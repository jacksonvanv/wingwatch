            ...
            if (Array.isArray(data.response) && data.response.length > 0) {
                for (const flight of data.response) {
                    ...
                    if (!flight.cs_airline_iata) {
                        ...
                        // Extract the first two characters from flight_iata
                        const flightIataPrefix = flight.flight_iata.substring(0, 2);
    
                        // Map the airline code to the airline name
                        const airlineName = airlineMapping[flightIataPrefix] || 'Unknown Airline';
                        ...
                        // Replace null values with '--' for gates, terminals, and times
                    ...
                    flightInfoContainer.innerHTML = `
                        ...
                        <div class="status ${flight.status.toLowerCase()}">${capitalizedFlightStatus}</div>
