

// JavaScript code for button interactions
const routeButton = document.getElementById('routeButton');
const routeOptions = document.getElementById('routeOptions');
const originDropdown = document.getElementById('origin');
const destinationDropdown = document.getElementById('destination');

// Function to show Route options
function showRouteOptions() {
    routeOptions.style.display = 'block';
    flightNumberOptions.style.display = 'none';
    airlineInputContainer.style.display = 'none'; // Hide airline input for route search
}


const checkStatusButton = document.querySelector('.check-status-button');

checkStatusButton.addEventListener('click', () => {
    // Redirect to the "routes.html" page
    window.location.href = 'routes.html';
});

const airports = [
"Chicago O'Hare International Airport (ORD)",
"Hartsfield-Jackson Atlanta International Airport (ATL)",
"Dallas/Fort Worth International Airport (DFW)",
"Los Angeles International Airport (LAX)",
"Charlotte Douglas International Airport (CLT)",
"Soekarno-Hatta International Airport (CGK)",
"George Bush Intercontinental Airport (IAH)",
"Seattle-Tacoma International Airport (SEA)",
"Tokyo International Airport (HND)",
"Frankfurt Airport (FRA)",
"Newark Liberty International Airport (EWR)",
"Amsterdam Airport Schiphol (AMS)",
"Indira Gandhi International Airport (DEL)",
"London Heathrow Airport (LHR)",
"Paris Charles de Gaulle Airport (CDG)",
"Guangzhou Baiyun International Airport (CAN)",
"San Francisco International Airport (SFO)",
"Shanghai Pudong International Airport (PVG)",
"Miami International Airport (MIA)",
"John F. Kennedy International Airport (JFK)",
"Detroit Metropolitan Wayne County Airport (DTW)",
"Beijing Capital International Airport (PEK)",
"Benito Juarez International Airport (MEX)",
"Logan International Airport (BOS)",
"Phoenix Sky Harbor International Airport (PHX)",
"Minneapolis-St Paul International Airport (MSP)",
"Munich International Airport (MUC)",
"Toronto Pearson International Airport (YYZ)",
"Philadelphia International Airport (PHL)",
"La Guardia Airport (LGA)",
"Adolfo Suarez Madrid-Barajas Airport (MAD)",
"Shenzhen Bao'an International Airport (SZX)",
"Dubai International Airport (DXB)",
"Chengdu Shuangliu International Airport (CTU)",
"Orlando International Airport (MCO)",
"El Dorado International Airport (BOG)",
"McCarran International Airport (LAS)",
"Salt Lake City International Airport (SLC)",
"Washington Dulles International Airport (IAD)",
"Barcelona-El Prat Airport (BCN)",
"Incheon International Airport (ICN)",
"Sheremetyevo International Airport (SVO)",
"Ronald Reagan Washington National Airport (DCA)",
"Hong Kong International Airport (HKG)",
"Ninoy Aquino International Airport (MNL)",
"Sao Paulo-Guarulhos International Airport (GRU)",
"Sydney International Airport (SYD)",
"Shanghai Hongqiao International Airport (SHA)",
"Chhatrapati Shivaji International Airport (BOM)",
"Suvarnabhumi Airport (BKK)",
"Doha International Airport (DIA)",
"Kuala Lumpur International Airport (KUL)",
"Kunming Changshui International Airport (KMG)",
"Xi'an Xianyang International Airport (XIY)",
"Singapore Changi Airport (SIN)",
"Vienna International Airport (VIE)",
"Vancouver International Airport (YVR)",
"Oslo Airport, Gardermoen (OSL)",
"Fort Lauderdale-Hollywood International Airport (FLL)",
"Leonardo da Vinci-Fiumicino Airport (FCO)",
"Zurich Airport (ZRH)",
"Copenhagen Airport (CPH)",
"Hangzhou Xiaoshan International Airport (HGH)",
"Melbourne Airport (MEL)",
"Chongqing Jiangbei International Airport (CKG)",
"Sabiha Goekcen International Airport (SAW)",
"Tan Son Nhat International Airport (SGN)",
"London Gatwick Airport (LGW)",
"Paris Orly Airport (ORY)",
"O.R. Tambo International Airport (JNB)",
"Lisbon Humberto Delgado Airport (LIS)",
"Taiwan Taoyuan International Airport (TPE)",
"Eleftherios Venizelos International Airport (ATH)",
"Portland International Airport (PDX)",
"Narita International Airport (NRT)",
"Calgary International Airport (YYC)",
"Berlin Brandenburg Airport (BER)",
"Montreal-Pierre Elliott Trudeau International Airport (YUL)",
"Stockholm Arlanda Airport (ARN)",
"Brussels Airport (BRU)",
"Don Mueang International Airport (DMK)",
"Brisbane Airport (BNE)",
"Nanjing Lukou International Airport (NKG)",
"Palma de Mallorca Airport (PMI)",
"Dublin Airport (DUB)",
"Domodedovo International Airport (DME)",
"Daniel K. Inouye International Airport (HNL)",
"Noi Bai International Airport (HAN)",
"Cancun International Airport (CUN)",
"San Diego International Airport (SAN)",
"Juanda International Airport (SUB)",
"Baltimore-Washington International Airport (BWI)",
"Milano Malpensa Airport (MXP)",
"Duesseldorf International Airport (DUS)",
"Nashville International Airport (BNA)",
"Fukuoka Airport (FUK)",
"King Khalid International (RUH)",
"Tampa International Airport (TPA)",
"Netaji Subhas Chandra Bose International Airport (CCU)",
"Warsaw Chopin Airport (WAW)",
"Chennai International Airport (MAA)",
"Osaka International Airport (ITM)",
"King Abdulaziz International Airport (JED)",
"Raleigh-Durham International Airport (RDU)",
"Auckland International Airport (AKL)",
"Wuhan Tianhe International Airport (WUH)",
"New Chitose Airport (CTS)",
"Comodoro Arturo Merino Benitez International Airport (SCL)",
"Ngurah Rai International Airport (DPS)",
"Manchester Airport (MAN)",
"Lambert-St. Louis International Airport (STL)",
"Tocumen International Airport (PTY)",
"Sultan Hasanuddin International Airport (UPG)",
"Ted Stevens Anchorage International Airport (ANC)",
"Helsinki-Vantaa Airport (HEL)",
"Luis Munoz Marin International Airport (SJU)",
"Austin-Bergstrom International Airport (AUS)",
"Urumqi Diwopu International Airport (URC)",
"Aeroport Geneve-Cointrin (GVA)",
"Kansai International Airport (KIX)",
"Viracopos International Airport (VCP)",
"Qingdao Liuting International Airport (TAO)",
"Pulkovo Airport (LED)",
"Jeju International Airport (CJU)",
"Brasilia International Airport (BSB)",
"Cairo International Airport (CAI)",
"Changsha Huanghua International Airport (CSX)",
"Chicago Midway International Airport (MDW)",
"Gimpo International Airport (GMP)",
"Hamburg Airport (HAM)",
"Nice Cote d'Azur International Airport (NCE)",
"Guadalajara International Airport (GDL)",
"Ben Gurion International Airport (TLV)",
"Naha Airport (OKA)",
"Cincinnati/Northern Kentucky International Airport (CVG)",
"Xiamen Gaoqi International Airport (XMN)",
"London Stansted Airport (STN)",
"Shenyang Taoxian International Airport (SHE)",
"Vnukovo International Airport (VKO)",
"Norman Y. Mineta San Jose International Airport (SJC)",
"Harbin Taiping International Airport (HRB)",
"Malaga Airport (AGP)",
"Pittsburgh International Airport (PIT)",
"Cleveland Hopkins International Airport (CLE)",
"Tianjin Binhai International Airport (TSN)",
"Indianapolis International Airport (IND)",
"Abu Dhabi International Airport (AUH)",
"Mactan-Cebu International Airport (CEB)",
"Sacramento International Airport (SMF)",
"Dalian Zhoushuizi International Airport (DLC)",
"Henri Coanda International Airport (OTP)",
"Louis Armstrong New Orleans International Airport (MSY)",
"Dallas Love Field (DAL)",
"Belize International Airport (BZE)",
"Lyon-Saint Exupery Airport (LYS)",
"John Glenn Columbus International Airport (CMH)",
"Tancredo Neves International Airport (CNF)",
"Kansas City International Airport (MCI)",
"Sanya Phoenix International Airport (SYX)",
"Kahului Airport (OGG)",
"Perth Airport (PER)",
"Kuwait International Airport (KWI)",
"Stuttgart Airport (STR)",
"Antalya Airport (AYT)",
"Southwest Florida International Airport (RSW)",
"Chubu Centrair International Airport (NGO)",
"Sardar Vallabhbhai Patel International Airport (AMD)",
"Gimhae International Airport (PUS)",
"William P Hobby Airport (HOU)",
"Edinburgh Airport (EDI)",
"John Wayne Airport-Orange County (SNA)",
"Jomo Kenyatta International Airport (NBO)",
"Jinan Yaoqiang International Airport (TNA)",
"Wellington International Airport (WLG)",
"General Mariano Escobedo International Airport (MTY)",
"Prague Vaclav Havel Airport (PRG)",
"Recife/Guararapes-Gilberto Freyre International Airport (REC)",
"Addis Ababa Bole International Airport (ADD)",
"Cape Town International Airport (CPT)",
"Bergen / Flesland (BGO)",
"Boryspil International Airport (KBP)",
"Kualanamu International Airport (KNO)",
"Alicante-Elche Airport (ALC)",
"Tribhuvan International Airport (KTM)",
"Marseille Provence Airport (MRS)",
"Nanning Airport (NNG)",
"London Luton Airport (LTN)",
"Jose Maria Cordova International Airport (MDE)",
"Jorge Newbery Airport (AEP)",
"Edmonton International Airport (YEG)",
"Cologne Bonn Airport (CGN)",
"Christchurch International Airport (CHC)",
"Gran Canaria Airport (LPA)",
"Toulouse-Blagnac Airport (TLS)",
"Mohammed V International Airport (CMN)",
"General Abelardo L. Rodriguez International Airport (TIJ)",
"Buenos Aires - Ministro Pistarini International Airport (EZE)",
"San Antonio International Airport (SAT)",
"Budapest Ferenc Liszt International Airport (BUD)",
"Taiyuan Wusu International Airport (TYN)",
"Milano Linate Airport (LIN)",
"Francisco de Sa Carneiro Airport (OPO)",
"Sultan Aji Muhamad Sulaiman Airport (BPN)",
"Muscat International Airport (MCT)",
"Sharjah International Airport (SHJ)",
"Salvador International Airport, Deputado Luis Eduardo Magalhaes (SSA)",
"Da Nang International Airport (DAD)",
"Metropolitan Oakland International Airport (OAK)",
"Ankara Esenboga International Airport (ESB)",
"Bahrain International Airport (BAH)",
"Penang International Airport (PEN)",
"Boise Air Terminal (BOI)",
"Cali / Alfonso Bonillaaragon (CLO)",
"Venice Marco Polo Airport (VCE)",
"Adelaide Airport (ADL)",
"Jacksonville International Airport (JAX)",
"General Mitchell International Airport (MKE)",
"Luxembourg Findel Airport (LUX)",
"Salgado Filho International Airport (POA)",
"Ottawa Macdonald-Cartier International Airport (YOW)",
"King Fahd International Airport (DMM)",
"Richmond International Airport (RIC)",
"Kagoshima Airport (KOJ)",
"Memphis International Airport (MEM)",
"Naples International Airport (NAP)",
"Julius Nyerere International Airport (DAR)",
"Birmingham International Airport (BHX)",
"Catania-Fontanarossa Airport (CTA)",
"Charleston International Airport (CHS)",
"Abeid Amani Karume International Airport (ZNZ)",
"Ibiza Airport (IBZ)",
"Bradley International Airport (BDL)",
"Mehrabad International Airport (THR)",
"Glasgow International Airport (GLA)",
"Cochin International Airport (COK)",
"Louisville International Airport (SDF)",
"Tolmachevo Airport (OVB)",
"Rio de Janeiro-Galeao International Airport (GIG)",
"Phuket International Airport (HKT)",
"London City Airport (LCY)",
"Riga International Airport (RIX)",
"EuroAirport Basel-Mulhouse-Freiburg (MLH)",
"Eppley Airfield (OMA)",
"Macau International Airport (MFM)",
"Chiang Mai International Airport (CNX)",
"Palm Beach International Airport (PBI)",
"Trondheim Airport, Vaernes (TRD)",
"Izmir Adnan Menderes International Airport (ADB)",
"Spokane International Airport (GEG)",
"Kota Kinabalu International Airport (BKI)",
"Hazrat Shahjalal International Airport (DAC)",
"Il Caravaggio International Airport (BGY)",
"Bandaranayake International Air Port (CMB)",
"Bologna Guglielmo Marconi Airport (BLQ)",
"Curitiba Airport (CWB)",
"Taipei Songshan Airport (TSA)",
"Tenerife North Airport (TFN)",
"Aeropuerto Internacional El Salvador (SAL)",
"Valencia Airport (VLC)",
"Queen Alia International Airport (AMM)",
"Bordeaux-Merignac Airport (BOD)",
"Sendai Airport (SDJ)",
"Bristol Airport (BRS)",
"Los Cabos International Airport (SJD)",
"Hohhot Baita International Airport (HET)",
"Sochi International Airport (AER)",
"John Paul II International Airport Krakow-Balice (KRK)",
"Beirut International Airport (BEY)",
"Reno/Tahoe International Airport (RNO)",
"Lynden Pindling International Airport (NAS)",
"Ontario International Airport (ONT)",
"Faro Airport (FAO)",
"Cyril E. King Airport (STT)",
"Fuzhou Changle International Airport (FOC)",
"Hang Nadim International Airport (BTH)",
"Buffalo Niagara International Airport (BUF)",
"Gerald R. Ford International Airport (GRR)",
"Sultan Mahmud Badaruddin II Airport (PLM)",
"Juan Santamaria International Airport (SJO)",
"Kona International Airport (KOA)",
"Stavanger Airport Sola (SVG)",
"McGhee Tyson Airport (TYS)",
"Pinto Martins - Fortaleza International Airport (FOR)",
"Falcone-Borsellino Airport (PMO)",
"Houari Boumediene Airport (ALG)",
"Nantes Atlantique Airport (NTE)",
"Halifax Stanfield International Airport (YHZ)",
"Bilbao Airport (BIO)",
"Savannah International Airport (SAV)",
"Heraklion International Airport (HER)",
"Winnipeg J. A. Richardson International Airport (YWG)",
"Kuching International Airport (KCH)",
"Albuquerque International Sunport (ABQ)",
"Chaudhary Charan Singh International Airport (LKO)",
"Belgrade Nikola Tesla Airport (BEG)",
"Lokpriya Gopinath Bordoloi International Airport (GAU)",
"Syamsudin Noor International Airport (BDJ)",
"Hannover Airport (HAJ)",
"Birmingham-Shuttlesworth International Airport (BHM)",
"Tenerife South Airport (TFS)",
"Seville San Pablo Airport (SVQ)",
"Licenciado Gustavo Diaz Ordaz International Airport (PVR)",
"Jaipur Airport (JAI)",
"Halim Perdanakusuma Airport (HLP)",
"Thessaloniki Airport (SKG)",
"Bodo Airport (BOO)",
"Tunis-Carthage Airport (TUN)",
"Lok Nayak Jayaprakash Airport (PAT)",
"Lanzarote Airport (ACE)",
"Greenville-Spartanburg International Airport (GSP)",
"Des Moines International Airport (DSM)",
"Supadio Airport (PNK)",
"Piarco International Airport (POS)",
"Gothenburg-Landvetter Airport (GOT)",
"Ekaterinburg Koltsovo Airport (SVX)",
"Palm Springs International Airport (PSP)",
"Sofia Airport (SOF)",
"Piedmont Triad International Airport (GSO)",
"Tromso Airport, Langnes (TOS)",
"Cairns International Airport (CNS)",
"Bob Hope Airport (BUR)",
"Las Americas International Airport (SDQ)",
"Viru Viru International Airport (VVI)",
"Murtala Muhammed International Airport (LOS)",
"Kaohsiung International Airport (KHH)",
"Larnaca International Airport (LCA)",
"Jacksons International Airport (POM)",
"Dane County Regional Airport (MSN)",
"Greater Rochester International Airport (ROC)",
"Zagreb Franjo Tugman Airport (ZAG)",
"James M Cox Dayton International Airport (DAY)",
"Jinnah International Airport (KHI)",
"Rafael Nunez International Airport (CTG)",
"Tulsa International Airport (TUL)",
"La Aurora International Airport (GUA)",
"Theodore Francis Green State Airport (PVD)",
"Jose Joaquin de Olmedo International Airport (GYE)",
"Sultan Syarif Kasim II International Airport (PKU)",
"Belfast International Airport (BFS)",
"Menara Airport (RAK)",
"Fresno Yosemite International Airport (FAT)",
"Malta International Airport (MLA)",
"Minsk National Airport (MSQ)",
"Brussels South Charleroi Airport (CRL)",
"Syracuse Hancock International Airport (SYR)",
"El Paso International Airport (ELP)",
"Adams Field (LIT)",
"Belem/Val de Cans-Julio Cezar Ribeiro International Airport (BEL)",
"Senai International Airport (JHB)",
"Bari Karol Wojtyla Airport (BRI)",
"Keflavik International Airport (KEF)",
"Fairbanks International Airport (FAI)",
"Portland International Jetport (PWM)",
"Pensacola International Airport (PNS)",
"Henry E Rohlsen Airport (STX)",
"Fuerteventura Airport (FUE)",
"Kotoka International Airport (ACC)",
"Punta Cana International Airport (PUJ)",
"Krasnodar International Airport (KRR)",
"Eduardo Gomes International Airport (MAO)",
"Francisco Bangoy International Airport (DVO)",
"Albany International Airport (ALB)",
"Prince Mohammad bin Abdulaziz Airport (MED)",
"Florence Peretola Airport (FLR)",
"Myrtle Beach International Airport (MYR)",
"Aberdeen International Airport (ABZ)",
"Benazir Bhutto Air Base (ISB)",
"Sarasota Bradenton International Airport (SRQ)",
"Vilnius International Airport (VNO)",
"Kumamoto Airport (KMJ)",
"Clark International Airport (CRK)",
"Gdansk Lech Walesa Airport (GDN)",
"Sangster International Airport (MBJ)",
"Westchester County Airport (HPN)",
"Pisa International Airport (PSA)",
"Colorado Springs Airport (COS)",
"Guilin Liangjiang International Airport (KWL)",
"Turin Airport (TRN)",
"El Alto International Airport (LPB)",
"El Tari Airport (KOE)",
"Princess Juliana International Airport (SXM)",
"Seychelles International Airport (SEZ)",
"Blue Grass Airport (LEX)",
"Billund Airport (BLL)",
"Sentani Airport (DJJ)",
"Columbia Metropolitan Airport (CAE)",
"Nagasaki Airport (NGS)",
"Rhodes International Airport (RHO)",
"Newcastle International Airport (NCL)",
"The Eastern Iowa Airport (CID)",
"Nuremberg Airport (NUE)",
"Billings Logan International Airport (BIL)",
"Menorca Airport (MAH)",
"Florianopolis-Hercilio Luz International Airport (FLN)",
"Ernesto Cortissoz International Airport (BAQ)",
"Harrisburg International Airport (MDT)",
"Split Airport (SPU)",
"Wichita Mid-Continent Airport (ICT)",
"Liverpool John Lennon Airport (LPL)",
"Jose Marti International Airport (HAV)",
"Sam Ratulangi International Airport (MDC)",
"Palonegro International Airport (BGA)",
"Townsville Airport (TSV)",
"Asheville Regional Airport (AVL)",
"Gallatin Field Airport (BZN)",
"Tallinn Airport (TLL)",
"Rogue Valley International - Medford Airport (MFR)",
"George Best Belfast City Airport (BHD)",
"Santorini Airport (JTR)",
"Yangon International Airport (RGN)",
"Springfield-Branson National Airport (SGF)",
"Del Bajio International Airport (BJX)",
"Key West International Airport (EYW)",
"Marechal Rondon International Airport (CGB)",
"Entebbe International Airport (EBB)",
"Velana International Airport (MLE)",
"Burlington International Airport (BTV)",
"Joe Foss Field (FSD)",
"Mahlon Sweet Field (EUG)",
"Long Beach Airport (LGB)",
"Leeds Bradford International Airport (LBA)",
"Jean Lesage International Airport (YQB)",
"Nnamdi Azikiwe International Airport (ABV)",
"Manuel Crescencio Rejon International Airport (MID)",
"Imam Khomeini International Airport (IKA)",
"Nadi International Airport (NAN)",
"Hobart International Airport (HBA)",
"Jorge Wilstermann (CBB)",
"Dalaman Airport (DLM)",
"Kilimanjaro International Airport (JRO)",
"Mombasa Airport (MBA)",
"Queen Beatrix International Airport (AUA)",
"Krasnoyarsk Yemelyanovo Airport (KJA)",
"Trivandrum International Airport (TRV)",
"Simon Bolivar International Airport (CCS)",
"Jackson-Evers International Airport (JAN)",
"Khrabrovo Airport (KGD)",
"Dr. Babasaheb Ambedkar International Airport (NAG)",
"Wilmington International Airport (ILM)",
"Ufa International Airport (UFA)",
"Allama Iqbal International Airport (LHE)",
"Oita Airport (OIT)",
"Roma Ciampino Airport (CIA)",
"Southampton Airport (SOU)",
"Almaty Airport (ALA)",
"Adana Sakirpasa Airport (ADA)",
"Verona Villafranca Airport (VRN)",
"Madeira Airport (FNC)",
"Cagliari Elmas Airport (CAG)",
"Hiroshima Airport (HIJ)",
"Ramon Villeda Morales International Airport (SAP)",
"Santa Barbara Municipal Airport (SBA)",
"Wroclaw-Copernicus Airport (WRO)",
"Midland International Airport (MAF)",
"Maceio/Zumbi dos Palmares International Airport (MCZ)",
"Coimbatore International Airport (CJB)",
"Dubrovnik Airport (DBV)",
"Bodrum Milas Airport (BJV)",
"Arthur Napoleon Raymond Robinson International Airport (TAB)",
"Niigata Airport (KIJ)",
"Montpellier-Mediterranee Airport (MPL)",
"Alejandro Velasco Astete International Airport (CUZ)",
"Pajas Blancas Airport (COR)",
"Corfu International Airport, Ioannis Kapodistrias (CFU)",
"Terrance B. Lettsome International Airport (EIS)",
"Olbia Costa Smeralda Airport (OLB)",
"Kazan International Airport (KZN)",
"East Midlands Airport (EMA)",
"Federal de Bachigualato International Airport (CUL)",
"Beauvais-Tille Airport (BVA)",
"Mashhad International Airport (MHD)",
"Darwin International Airport (DRW)",
"Mineralnye Vody Airport (MRV)",
"South Bend International Airport (SBN)",
"Heydar Aliyev International Airport (GYD)",
"Santiago de Compostela Airport (SCQ)",
"Calicut International Airport (CCJ)",
"Hector International Airport (FAR)",
"Baton Rouge Metropolitan Airport (BTR)",
"Robert Gabriel Mugabe International Airport (HRE)",
"Bremen Airport (BRE)",
"Phnom Penh International Airport (PNH)",
"Ponta Delgada-Joao Paulo II Airport (PDL)",
"Maputo International Airport (MPM)",
"Providenciales International Airport (PLS)",
"Hakodate Airport (HKD)",
"Roanoke Regional Airport-Woodrum Field (ROA)",
"Cibao International Airport (STI)",
"Port Bouet Airport (ABJ)",
"Hurghada International Airport (HRG)",
"Lille Airport (LIL)",
"Tallahassee Regional Airport (TLH)",
"Mangalore Airport (IXE)",
"General Ignacio Pesqueira Garcia International Airport (HMO)",
"Kigali International Airport (KGL)",
"Trabzon Airport (TZX)",
"Daegu International Airport (TAE)",
"St. John's International Airport (YYT)",
"Manchester Airport (MHT)",
"Sri Guru Ram Dass Jee International Airport (ATQ)",
"Brindisi - Salento Airport (BDS)",
"Shreveport Regional Airport (SHV)",
"Kashgar Airport (KHG)",
"Toncontin International Airport (TGU)",
"Antonio B. Won Pat International Airport (GUM)",
"Simon Bolivar International Airport (SMR)",
"Aalborg Airport (AAL)",
"Cork Airport (ORK)",
"Lamezia Terme International Airport (SUF)",
"Khon Kaen Airport (KKC)",
"Vladivostok International Airport (VVO)",
"Mobile Regional Airport (MOB)",
"Ajaccio Napoleon Bonaparte Airport (AJA)",
"Kos Airport (KGS)",
"Hato International Airport (CUR)",
"Fa'a'a International Airport (PPT)",
"Owen Roberts International Airport (GCM)",
"Orlando Sanford International Airport (SFB)",
"Sandefjord Airport, Torp (TRF)",
"Leipzig/Halle Airport (LEJ)",
"Bastia-Poretta (BIA)",
"Dresden Airport (DRS)",
"General Roberto Fierro Villalobos International Airport (CUU)",
"Tirana International Airport Nene Tereza (TIA)",
"Kurumoch International Airport (KUF)",
"Jersey Airport (JER)",
"San Luis Obispo County Airport (SBP)",
"Mikonos Airport (JMK)",
"Missoula International Airport (MSO)",
"General Heriberto Jara International Airport (VER)",
"Hosea Kutako International Airport (WDH)",
"Chania International Airport (CHQ)",
"Krabi Airport (KBV)",
"Katowice International Airport (KTW)",
"Cluj-Napoca International Airport (CLJ)",
"Erbil International Airport (EBL)",
"Norman Manley International Airport (KIN)",
"Ambon Pattimura Airport (AMQ)",
"Warsaw-Modlin Mazovia Airport (WMI)",
"Roshchino International Airport (TJM)",
"Cataratas International Airport (IGU)",
"Mendoza Aerodrome (MDZ)",
"Austin Straubel International Airport (GRB)",
"Lubbock Preston Smith International Airport (LBB)",
"Marechal Cunha Machado International Airport (SLZ)",
"Quad City International Airport (MLI)",
"Augusta Regional Airport (AGS)",
"Anapa Airport (AAQ)",
"Gustavo Rojas Pinilla International Airport (ADZ)",
"Poznan-Lawica Airport (POZ)",
"Surgut Airport (SGC)",
"Sir Seewoosagur Ramgoolam International Airport (MRU)",
"Liege Airport (LGG)",
"Aeropuerto Cerro Moreno (ANF)",
"Juan Manuel Galvez International Airport (RTB)",
"Chisinau International Airport (KIV)",
"Augusto Cesar Sandino International Airport (MGA)",
"Grantley Adams International Airport (BGI)",
"Snohomish County Airport (Paine Field) (PAE)",
"Abraham Gonzalez International Airport (CJS)",
"Alesund Airport, Vigra (AES)",
"Paphos International Airport (PFO)",
"Guernsey Airport (GCI)",
"Jackson Hole Airport (JAC)",
"Dortmund Airport (DTM)",
"Takamatsu Airport (TAK)",
"McAllen Miller International Airport (MFE)",
"Kristiansand Airport (KRS)",
"Queretaro Intercontinental Airport (QRO)",
"Lafayette Regional Airport (LFT)",
"Corpus Christi International Airport (CRP)",
"Lviv International Airport (LWO)",
"Phu Bai International Airport (HUI)",
"Perm International Airport (PEE)",
"Rotterdam The Hague Airport (RTM)",
"La Palma Airport (SPC)",
"Aguascalientes International Airport (AGU)",
"Monterey Regional Airport (MRY)",
"Khartoum International Airport (KRT)",
"Brunei International Airport (BWN)",
"Graz Airport (GRZ)",
"Cherry Capital Airport (TVC)",
"Carrasco International Airport (MVD)",
"Irkutsk Airport (IKT)",
"Zvartnots International Airport (EVN)",
"Bismarck Municipal Airport (BIS)",
"Carlos Rovirosa Perez International Airport (VSA)",
"Tangier Ibn Battouta Airport (TNG)",
"Chelyabinsk Airport (CEK)",
"Alexandria Borg El Arab Airport (HBE)",
"El Tepual Airport (PMC)",
"Khabarovsk Novy Airport (KHV)",
"Campo Grande International Airport (CGR)",
"Gustaf III Airport (SBH)",
"Carriel Sur Airport (CCP)",
"Angel Albino Corzo International Airport (TGZ)",
"Yeager Airport (CRW)",
"Agadir-Al Massira Airport (AGA)",
"Treviso Airport (TSF)",
"Lome-Tokoin Airport (LFW)",
"Daniel Oduber Quiros International Airport (LIR)",
"Rick Husband Amarillo International Airport (AMA)",
"St. Petersburg-Clearwater International Airport (PIE)",
"General Rafael Buelna International Airport (MZT)",
"Gulfport-Biloxi Regional Airport (GPT)",
"Gaziantep Oguzeli International Airport (GZT)",
"Ubon Ratchathani Airport (UBP)",
"Uytash Airport (MCX)",
"Djerba-Zarzis International Airport (DJE)",
"Strasbourg Airport (SXB)",
"Ivato International Airport (TNR)",
"Roland Garros Airport (RUN)",
"Oaxaca - Xoxocotlan International Airport (OAX)",
"Toussaint Louverture International Airport (PAP)",
"Rodriguez Ballon International Airport (AQP)",
"Hamid Karzai International Airport (KBL)",
"Brest Bretagne Airport (BES)",
"Kayseri Erkilet Airport (ASR)",
"Xichang Qingshan Airport (XIC)",
"Long Island Mac Arthur Airport (ISP)",
"Genoa Cristoforo Colombo Airport (GOA)",
"Cheongju International Airport (CJJ)",
"Port Vila - Bauerfield International Airport (VLI)",
"Muenster Osnabrueck International Airport (FMO)",
"Douala International Airport (DLA)",
"Zamboanga International Airport (ZAM)",
"Okayama Airport (OKJ)",
"Sharm el-Sheikh International Airport (SSH)",
"Camilo Daza International Airport (CUC)",
"V.C. Bird International Airport (ANU)",
"Tiruchirappalli International Airport (TRZ)",
"General Downing - Peoria International Airport (PIA)",
"Istanbul Atatuerk Airport (ISL)",
"Diego Aracena International Airport (IQQ)",
"Shiraz International Airport (SYZ)",
"Tsentralny Airport (OMS)",
"Wilkes-Barre/Scranton International Airport (AVP)",
"Ponciano Arriaga International Airport (SLP)",
"Fayetteville Municipal Airport (FAY)",
"Asturias Airport (OVD)",
"Skopje International Airport (SKP)",
"Bariloche Aerodrome (BRC)",
"Manuel Marquez de Leon International Airport (LAP)",
"Harstad/Narvik Airport, Evenes (EVE)",
"Silvio Pettirossi International Airport (ASU)",
"Bamako-Senou International Airport (BKO)",
"Salzburg Airport (SZG)",
"General Francisco J. Mujica International Airport (MLM)",
"Odessa International Airport (ODS)",
"General Rodolfo Sanchez Taboada International Airport (MXL)",
"Volgograd International Airport (VOG)",
"Timisoara Traian Vuia International Airport (TSR)",
"Rochester International Airport (RST)",
"Ljubljana Joze Pucnik Airport (LJU)",
"Es Senia Airport (ORN)",
"Isle of Man Airport (IOM)",
"Bishop International Airport (FNT)",
"Rockhampton Airport (ROK)",
"A Coruna Airport (LCG)",
"Wattay International Airport (VTE)",
"Lanseria International Airport (HLA)",
"Coronel FAP Francisco Secada Vignetta International Airport (IQT)",
"Frankfurt-Hahn Airport (HHN)",
"Voronezh International Airport (VOZ)",
"Fes-Saiss Airport (FEZ)",
"Shah Amanat International Airport (CGP)",
"Zakynthos International Airport (ZTH)",
"Nizhny Novgorod International Airport (GOJ)",
"Bram Fischer International Airport (BFN)",
"Lansing, Capital City Airport (LAN)",
"Lulea Airport (LLA)",
"Rennes - Saint-Jacques (RNS)",
"Oulu Airport (OUL)",
"Vance W. Amory International Airport (NEV)",
"Biarritz Pays Basque Airport (BIQ)",
"Siem Reap International Airport (REP)",
"Murmansk Airport (MMK)",
"Kyiv Zhuliany International Airport (IEV)",
"Bacha Khan International Airport (PEW)",
"Presidente Peron International Airport (NQN)",
"Vigo Airport (VGO)",
"Kangerlussuaq Airport (SFJ)",
"General Juan N. Alvarez International Airport (ACA)",
"Flamingo International Airport (BON)",
"Federico Garcia Lorca Airport (GRX)",
"Valley International Airport (HRL)",
"Salalah Airport (SLL)",
"Martin Miguel de Gueemes International Airport (SLA)",
"Kharkiv International Airport (HRK)",
"Robert L. Bradshaw International Airport (SKB)",
"Port Harcourt International Airport (PHC)",
"N'djili Airport (FIH)",
"Cozumel International Airport (CZM)",
"Greater Moncton International Airport (YQM)",
"Cheddi Jagan International Airport (GEO)",
"General Francisco Javier Mina International Airport (TAM)",
"Great Falls International Airport (GTF)",
"Pau Pyrenees Airport (PUF)",
"Melville Hall Airport (DOM)",
"Melilla Airport (MLN)",
"Varna Airport (VAR)",
"L.F. Wade International Airport (BDA)",
"Almeria Airport (LEI)",
"Cotonou Airport (COO)",
"Lilongwe International Airport (LLW)",
"Jerez Airport (XRY)",
"Francisco Sarabia International Airport (TRC)",
"Sarajevo International Airport (SJJ)",
"Idaho Falls Regional Airport (IDA)",
"Cataratas del Iguazu International Airport (IGR)",
"Monroe Regional Airport (MLU)",
"Ouagadougou Airport (OUA)",
"Reykjavik Airport (RKV)",
"Goloson International Airport (LCE)",
"Newport News-Williamsburg International Airport (PHF)",
"Durham Tees Valley Airport (MME)",
"Meadows Field Airport (BFL)",
"Rabat-Sale Airport (RBA)",
"Alta Airport (ALF)",
"Minot International Airport (MOT)",
"Aarhus Airport (AAR)",
"Kirkenes (KKN)",
"Shannon Airport (SNN)",
"Van Ferit Melen Airport (VAN)",
"Eldoret International Airport (EDL)",
"Samsun-Carsamba Airport (SZF)",
"Antonio Rivera Rodriguez Airport (VQS)",
"Durango International Airport (DGO)",
"Casper-Natrona County International Airport (CPR)",
"Kumasi Airport (KMS)",
"Bellingham International Airport (BLI)",
"Bornholm Airport (RNN)",
"Flagstaff Pulliam Airport (FLG)",
"Bacau International Airport (BCM)",
"Malmoe Airport (MMX)",
"Weeze Airport (NRN)",
"Nador International Airport (NDR)",
"Aden Adde International Airport (MGQ)",
"Maestro Wilson Fonseca Airport (STM)",
"Al Najaf International Airport (NJF)",
"Lincoln Airport (LNK)",
"Ixtapa-Zihuatanejo International Airport (ZIH)",
"Yuzhno-Sakhalinsk Airport (UUS)",
"Quatro de Fevereiro Airport (LAD)",
"Hermanos Serdan International Airport (PBC)",
"Basra International Airport (BSR)",
"Bratislava Airport (BTS)",
"Iasi International Airport (IAS)",
"Monastir Habib Bourguiba International Airport (MIR)",
"Zadar Airport (ZAD)",
"Eagle County Regional Airport (EGE)",
"Belgorod International Airport (EGO)",
"Rosario International Airport (ROS)",
"Honiara International Airport (HIR)",
"Laredo International Airport (LRD)",
"Yaounde (NSI)",
"Rzeszow-Jasionka Airport (RZE)",
"General Leobardo C. Ruiz International Airport (ZCL)",
"Brownsville-South Padre Island International Airport (BRO)",
"Umea Airport (UME)",
"Port Hedland International Airport (PHE)",
"Karlsruhe/Baden-Baden Airport (FKB)",
"Alghero-Fertilia Airport (AHO)",
"General Enrique Mosconi International Airport (CRD)",
"Libreville Airport (LBV)",
"Helena Regional Airport (HLN)",
"North Eleuthera Airport (ELH)",
"Ushuaia Aerodrome (USH)",
"Alice Springs Airport (ASP)",
"Buyant-Ukhaa International Airport (ULN)",
"Santiago Marino Caribbean International Airport (PMV)",
"Conakry International Airport (CKY)",
"Bahias de Huatulco International Airport (HUX)",
"Presidente Carlos Ibanez del Campo International Airport (PUQ)",
"Martinique Aime Cesaire International Airport (FDF)",
"Astrakhan Airport (ASF)",
"Innsbruck Airport (INN)",
"Nuuk Airport (GOH)",
"Talagi Airport (ARH)",
"Chacalluta Airport (ARI)",
"Diori Hamani International Airport (NIM)",
"La Crosse Municipal Airport (LSE)",
"Grand Bahama International Airport (FPO)",
"Turku Airport (TKU)",
"Atlantic City International Airport (ACY)",
"Cap. FAP Jose A. Quinones Gonzales International Airport (CIX)",
"Puerto Escondido International Airport (PXM)",
"Cardiff Airport (CWL)",
"Capitan FAP Carlos Martinez de Pinillos International Airport (TRU)",
"Lake Charles Regional Airport (LCH)",
"Frans Kaisiepo Airport (BIK)",
"Dubai World Central - Al Maktoum International Airport (DWC)",
"Pointe-a-Pitre Le Raizet Airport (PTP)",
"Paderborn Lippstadt Airport (PAD)",
"Amilcar Cabral International Airport (SID)",
"Djibouti-Ambouli International Airport (JIB)",
"Aswan International Airport (ASW)",
"Clayton J. Lloyd International Airport (AXA)",
"Tivat Airport (TIV)",
"Pula Airport (PUY)",
"Raja Haji Fisabilillah International Airport (TNJ)",
"Tamale Airport (TML)",
"Kirkwall Airport (KOI)",
"Angads Airport (OUD)",
"Stavropol Shpakovskoye Airport (STW)",
"Hewanorra International Airport (UVF)",
"Trieste - Friuli Venezia Giulia Airport (TRS)",
"Kaunas Airport (KUN)",
"Victoria Falls Airport (VFA)",
"Beira Airport (BEW)",
"Mytilene International Airport (MJT)",
"Santo-Pekoa International Airport (SON)",
"Exeter International Airport (EXT)",
"Hierro Airport (VDE)",
"Linz Airport (LNZ)",
"Sir Charles Kirkconnell International Airport (CYB)",
"Chileka International Airport (BLZ)",
"Santander Airport (SDR)",
"General Lucio Blanco International Airport (REX)",
"U-Tapao International Airport (UTP)",
"Vincenzo Florio Airport Trapani-Birgi (TPS)",
"Maurice Bishop International Airport (GND)",
"Podgorica Airport (TGD)",
"Erzurum Airport (ERZ)",
"Grand Forks International Airport (GFK)",
"Juba International Airport (JUB)",
"Kefallinia Airport (EFL)",
"Maya-Maya Airport (BZV)",
"Bulawayo Airport (BUQ)",
"Burgas Airport (BOJ)",
"Zaragoza Airport (ZAZ)",
"Luxor International Airport (LXR)",
"Kosice International Airport (KSC)",
"Los Mochis International Airport (LMM)",
"Yelizovo Airport (PKC)",
"Arcata-Eureka Airport (ACV)",
"Friedrichshafen Airport (FDH)",
"Tapachula International Airport (TAP)",
"Stockholm Skavsta Airport (NYO)",
"Samos Airport (SMI)",
"Chios Island National Airport (JKH)",
"Reus Airport (REU)",
"Toledo Express Airport (TOL)",
"Seletar Airport (XSP)",
"Norwich International Airport (NWI)",
"Macapa-Alberto Alcolumbre International Airport (MCP)",
"Ciudad Obregon International Airport (CEN)",
"Francisco C. Ada International Airport (SPN)",
"Calvi - Sainte-Catherine Airport (CLY)",
"Maastricht Aachen Airport (MST)",
"Kadala Airport (HTA)",
"Sulaymaniyah International Airport (ISU)",
"Orenburg / Tsentralny (REN)",
"Mohamed Boudiaf International Airport (CZL)",
"Ilulissat Airport (JAV)",
"Abruzzo Airport (PSR)",
"Sumburgh Airport (LSI)",
"George F. L. Charles Airport (SLU)",
"Perpignan-Rivesaltes Airport (PGF)",
"Rafael Hernandez Airport (BQN)",
"Sibiu International Airport (SBZ)",
"Fredericton International Airport (YFC)",
"Pantelleria Airport (PNL)",
"Szczecin-Goleniow Airport (SZZ)",
"Beslan Airport (OGZ)",
"Konya Airport (KYA)",
"Puerto Plata Airport (POP)",
"Dzaoudzi Pamandzi International Airport (DZA)",
"Ciudad del Carmen International Airport (CME)",
"Phitsanulok Airport (PHS)",
"Coastal Carolina Regional Airport (EWN)",
"Palanga International Airport (PLQ)",
"Mopah Airport (MKQ)",
"Marsh Harbour Airport (MHH)",
"Bandar Abbas International Airport (BND)",
"Astor Piazzolla International Airport (MDQ)",
"London Southend Airport (SEN)",
"Aktion Airport (PVK)",
"Tyler Pounds Regional Airport (TYR)",
"Lungi International Airport (FNA)",
"Stockholm-Bromma Airport (BMA)",
"Barnaul Airport (BAX)",
"Ignatyevo Airport (BQS)",
"Syktyvkar Airport (SCW)",
"Tabriz International Airport (TBZ)",
"Malabo Airport (SSG)",
"Tampere-Pirkkala Airport (TMP)",
"Capitan Oriel Lea Plaza Airport (TJA)",
"Vaasa Airport (VAA)",
"Maun Airport (MUB)",
"Ancona Falconara Airport (AOI)",
"Zaporizhia International Airport (OZH)",
"Banjul International Airport (BJL)",
"Saarbruecken Airport (SCN)",
"Boa Vista, Boa Vista Intl (BVB)",
"Bujumbura International Airport (BJM)",
"Ziguinchor Airport (ZIG)",
"Visby Airport (VBY)",
"Ostend-Bruges International Airport (OST)",
"Dnipropetrovsk International Airport (DNK)",
"Aeroport de Girona - Costa Brava (GRO)",
"Dushanbe Airport (DYU)",
"Caen - Carpiquet Airport (CFR)",
"Glasgow Prestwick Airport (PIK)",
"Roberts International Airport (ROB)",
"David Airport (DAV)",
"Rovaniemi Airport (RVN)",
"Rickenbacker International Airport (LCK)",
"Alexandroupoli Airport (AXD)",
"Gander International Airport (YQX)",
"Gibraltar International Airport (GIB)",
"Ireland West Airport Knock (NOC)",
"Playa de Oro International Airport (ZLO)",
"Prince Said Ibrahim International Airport (HAH)",
"Bournemouth Airport (BOH)",
"Yakutsk Airport (YKS)",
"Akureyri Airport (AEY)",
"Karpathos Airport (AOK)",
"Kemerovo Airport (KEJ)",
"Egal International Airport - Hargeisa (HGA)",
"Ulan-Ude Airport (UUD)",
"Skelleftea Airport (SFT)",
"Tarbes-Lourdes-Pyrenees Airport (LDE)",
"Chetumal International Airport (CTM)",
"San Sebastian Airport (EAS)",
"Bonriki International Airport (TRW)",
"Johan Adolf Pengel International Airport (PBM)",
"Skiathos Airport 'Alexandros Papadiamantis' (JSI)",
"N'Djamena International Airport (NDJ)",
"Resistencia International Airport (RES)",
"Paris-Le Bourget Airport (LBG)",
"Vagar Airport (FAE)",
"Pointe Noire Airport (PNR)",
"La Tontouta International Airport (NOU)",
"Governor's Harbour Airport (GHB)",
"Narsarsuaq Airport (UAK)",
"Kerry Airport (KIR)",
"Aba Tenna Dejazmach Yilma International Airport (DIR)",
"Atyrau Airport (GUW)",
"Coronel FAP Carlos Ciriani Santa Rosa International Airport (TCQ)",
"Central Nebraska Regional Airport (GRI)",
"Clermont-Ferrand Auvergne Airport (CFE)",
"Gobernador Horacio Guzman International Airport (JUJ)",
"Suceava / Salcea Airport (SCV)",
"Marsa Alam International Airport (RMF)",
"Humberside Airport (HUY)",
"Juan Gualberto Gomez International Airport (VRA)",
"Shymkent Airport (CIT)",
"Marcos A. Gelabert International Airport (PAC)",
"Upington Airport (UTN)",
"Sokol Airport (GDX)",
"Aktobe International Airport (AKX)",
"Banak Airport (LKL)",
"Yangyang International Airport (YNY)",
"Ashgabat International Airport (ASB)",
"Limnos Airport (LXS)",
"Aktau Airport (SCO)",
"Oradea International Airport (OMR)",
"Kars Airport (KSY)",
"Klagenfurt Airport (KLU)",
"Abakan Airport (ABA)",
"Magnitogorsk International Airport (MQF)",
"Walvis Bay Airport (WVB)",
"Kulusuk Airport (KUS)",
"Asmara International Airport (ASM)",
"Maseru / Moshoeshoe (MSU)",
"Fascene Airport (NOS)",
"Antwerp International Airport (ANR)",
"Cayenne-Rochambeau (CAY)",
"Minatitlan/Coatzacoalcos International Airport (MTT)",
"Bydgoszcz Ignacy Jan Paderewski Airport (BZG)",
"Samarkand Airport (SKD)",
"Piloto Civil Norberto Fernandez International Airport (RGL)",
"Kursk East Airport (URS)",
"Westerland / Sylt (GWT)",
"Federico Fellini International Airport (RMI)",
"La Isabela International Airport (JBQ)",
"Rarotonga International Airport (RAR)",
"Bangui Airport (BGF)",
"Rabah Bitat Airport (AAE)",
"Ing. Alberto Acuna Ongay International Airport (CPE)",
"Cheboksary Airport (CSY)",
"Lubumbashi International Airport (FBM)",
"Bissau Airport (OXB)",
"Licenciado Adolfo Lopez Mateos International Airport (TLC)",
"Antonio Maceo International Airport (SCU)",
"Ras Al Khaimah International Airport (RKT)",
"La Chinita Internacional (MAR)",
"Urgench Airport (UGC)",
"Qaarsut Airport (JQA)",
"Kavala International Airport (KVA)",
"Aasiaat Airport (JEG)",
"Tuzla International Airport (TZL)",
"Frank Pais International Airport (HOG)",
"Egilsstadir Airport (EGS)",
"Kalmar OEland Airport (KLR)",
"Gaya Airport (GAY)",
"Mallam Aminu Kano International Airport (KAN)",
"Ronneby Airport (RNB)",
"Amado Nervo International Airport (TPQ)",
"Kiruna Airport (KRN)",
"Kalamata Airport (KLX)",
"Hugo Chavez International Airport (CAP)",
"South Caicos Airport (XSC)",
"Sundsvall-Timra Airport (SDL)",
"Hihifo Airport (WLS)",
"Lublin Airport (LUZ)",
"Loreto International Airport (LTO)",
"Vanimo Airport (VAI)",
"Santa Maria Airport (SMA)",
"Denizli Cardak Airport (DNZ)",
"Sisimiut Airport (JHS)",
"Alfredo Vasquez Cobo International Airport (LET)",
"Eloy Alfaro International Airport (MEC)",
"Ioannina Airport (IOA)",
"Stella Maris Airport (SML)",
"King Hussein International Airport (AQJ)",
"Laramie Regional Airport (LAR)",
"Kutaisi International Airport (KUT)",
"Arturo Michelena International Airport (VLN)",
"Bocas del Toro Isla Colon International Airport (BOC)",
"Nalchik Airport (NAL)",
"Esbjerg Airport (EBJ)",
"Santa Maria Airport (SMX)",
"San Salvador Airport (ZSA)",
"Reggio Calabria Airport (REG)",
"Bratsk Airport (BTK)",
"Licenciado y General Ignacio Lopez Rayon Airport (UPN)",
"Jardines del Rey Airport (CCC)",
"Ivano-Frankivsk International Airport (IFO)",
"Mfuwe Airport (MFU)",
"Roman Tmetuchl International Airport (ROR)",
"Zahedan International Airport (ZAH)",
"Porto Santo Airport (PXO)",
"Kasane Airport (BBK)",
"Leos Janacek Airport Ostrava (OSR)",
"Tupelo Regional Airport (TUP)",
"Nevsehir Kapadokya Airport (NAV)",
"Ain Arnat Airport (QSF)",
"South Bimini Airport (BIM)",
"Kuressaare Airport (URE)",
"Chuuk International Airport (TKK)",
"Ivalo Airport (IVL)",
"Perugia San Francesco d'Assisi - Umbria International Airport (PEG)",
"Chicago Rockford International Airport (RFD)",
"Sonderborg Airport (SGD)",
"Assiut Airport (ATZ)",
"Mataveri International Airport (IPC)",
"Luebeck Airport (LBC)",
"S. Tome (TMS)",
"Pohnpei International Airport (PNI)",
"Kyzylorda Airport (KZO)",
"Petrozavodsk Airport (PES)",
"Arrachart Airport (DIE)",
"Mercedita Airport (PSE)",
"Oskemen Airport (UKK)",
"Changuinola/Captain Manuel Nino Airport (CHX)",
"Tlemcen Zenata (TLM)",
"Cherif Al Idrissi Airport (AHU)",
"Craiova Airport (CRA)",
"Bejaia Airport (BJA)",
"Laoag International Airport (LAO)",
"Valladolid Airport (VLL)",
"Bukhara Airport (BHK)",
"Nicolau Lobato Airport (DIL)",
"Chambery Airport (CMF)",
"Baia Mare International Airport (BAY)",
"Cuneo International Airport (CUF)",
"Amata Kabua International Airport (MAJ)",
"Neerlerit Inaat Airport (CNP)",
"Mariehamn Airport (MHQ)",
"Ouarzazate Airport (OZZ)",
"Banja Luka International Airport (BNX)",
"Satu Mare International Airport (SUJ)",
"Ouani Airport (AJN)",
"Fua'amotu International Airport (TBU)",
"St. Gallen-Altenrhein Airport (ACH)",
"Orsk Airport (OSW)",
"Tabatinga International Airport (TBT)",
"Karaganda Sary-Arka Airport (KGF)",
"Brjansk (BZK)",
"Rijeka Airport (RJK)",
"Cruzeiro do Sul International Airport (CZS)",
"Ohrid St. Paul the Apostle Airport (OHD)",
"General Servando Canales International Airport (MAM)",
"Quetzalcoatl International Airport (NLD)",
"Pardubice Airport (PED)",
"Essaouira Airport (ESU)",
"Norfolk Island Airport (NLK)",
"Erfurt Airport (ERF)",
"General Jose Antonio Anzoategui International Airport (BLA)",
"Port Gentil Airport (POG)",
"Tindal / Katherine (KTR)",
"Tunoshna Airport (IAR)",
"La Rochelle - Ile de Re Airport (LRH)",
"Kandahar Airport (KDH)",
"Ilorin International Airport (ILR)",
"Brno-Turany Airport (BRQ)",
"Touat-Cheikh Sidi Mohamed Belkebir Airport (AZR)",
"Ignacio Agramonte International Airport (CMW)",
"Dasoguz Airport (TAZ)",
"Lodz Wladyslaw Reymont Airport (LCJ)",
"Vitoria Airport (VIT)",
"Dole-Jura Airport (DLE)",
"Cap Skirring Airport (CSK)",
"Elista Airport (ESL)",
"Port Sudan New International Airport (PZU)",
"Shirak Airport (LWN)",
"General Pedro J. Mendez International Airport (CVM)",
"King County International Airport (BFI)",
"Piedras Negras International Airport (PDS)",
"Puerto Barrios Airport (PBR)",
"Kogalym International Airport (KGP)",
"Araxos Airport (GPA)",
"Sania Ramel Airport (TTU)",
"Targu Mures Transilvania Airport (TGM)",
"Corumba International Airport (CMG)",
"Nouadhibou International Airport (NDB)",
"Kisimayu Airport (KMU)",
"Sfax-Thyna International Airport (SFA)",
"Uruguaiana Aeroporto (URG)",
"Termez Airport (TMJ)",
"Paro Airport (PBH)",
"Tours Val de Loire Airport (TUF)",
"Goma International Airport (GOM)",
"Tamanrasset / Aguenna (TMR)",
"Upernavik Airport (JUV)",
"Damascus International Airport (DAM)",
"Poitiers-Biard Airport (PIS)",
"Aden International Airport (ADE)",
"Turkmenbashi Airport (KRW)",
"F. D. Roosevelt Airport (EUX)",
"Muan International Airport (MWX)",
"Maniitsoq Airport (JSU)",
"Pskov Airport (PKV)",
"Garoua Airport (GOU)",
"Bobo Dioulasso Airport (BOY)",
"Sainte Marie Airport (SMS)",
"Moulay Ali Cherif Airport (ERH)",
"Oued Irara Airport (HME)",
"Arvidsjaur Airport (AJR)",
"Biskra Airport (BSK)",
"Ugolny Airport (DYR)",
"Bouake Airport (BYK)",
"Pyongyang Sunan International Airport (FNJ)",
"Kassel-Calden Airport (KSF)",
"Francistown Airport (FRW)",
"Calabar (CBQ)",
"Samedan Airport (SMV)",
"Saratov Tsentralny Airport (RTW)",
"Mostepha Ben Boulaid Airport (BLJ)",
"Qaanaaq Airport (NAQ)",
"Liepaja East (LPX)",
"Karlovy Vary Airport (KLV)",
"Forli Airport (FRL)",
"Deauville - Saint-Gatien Airport (DOL)",
"Zinder Airport (ZND)",
"Mano Dayak International Airport (AJY)",
"Samana El Catey International Airport (AZS)",
"Nimes-Ales-Camargue-Cevennes Airport (FNI)",
"Chlef International Airport (CFK)",
"Stockholm-Vaesteras Airport (VST)",
"Tekirdag Corlu Airport (TEQ)",
"Kaduna Airport (KAD)",
"Osijek Airport (OSI)",
"Maiduguri International Airport (MIU)",
"Gan International Airport (GAN)",
"Pago Pago International Airport (PPG)",
"Funafuti International Airport (FUN)",
"Limon International Airport (LIO)",
"Vava'u International Airport (VAV)",
"Quimper-Pluguffan (UIP)",
"Gwadar International Airport (GWD)",
"Tozeur Nefta International Airport (TOE)",
"Christmas Island Airport (XCH)",
"Cocos Island Airport (CCK)",
"Al Ain International Airport (AAN)",
"Pitalito Airport (PTX)",
"Northway Airport (ORT)",
"Salak Airport (MVR)",
"Kostanay Airport (KSN)",
"Parma Airport (PMF)",
"St. Cloud Regional Airport (STC)",
"Yap International Airport (YAP)",
"Alpes-Isere Airport (GNB)",
"Mannheim City Airport (MHG)",
"La Romana International Airport (LRM)",
"Djanet (DJG)",
"Heringsdorf Airport (HDF)",
"Nea Anchialos National Airport (VOL)",
"General Jose Maria Yanez International Airport (GYM)",
"Alofi / Niue (IUE)",
"Zielona Gora Airport (IEG)",
"Cassidy International Airport (CXI)",
"Jijel Ferhat Abbas Airport (GJL)",
"Sokoto (SKO)",
"Sana'a International Airport (SAH)",
"Mihail Kogalniceanu International Airport (CND)",
"Mostar International Airport (OMO)",
"Poprad-Tatry Airport (TAT)",
"Franceville / Mvengue Airport (MVB)",
"Kassala Airport (KSL)",
"Ganja International Airport (KVD)",
"Benina International Airport (BEN)",
"Guarani International Airport (AGT)",
"Zhezkazgan Airport (DZN)",
"Cheikh Larbi Tebessa Airport (TEE)",
"Mopti Airport (MZI)",
"Saint-Nazaire - Montoir (SNR)",
"In Amenas Airport (IAM)",
"Treasure Cay Airport (TCB)",
"Nakhchivan International Airport (NAJ)",
"Pavlodar Airport (PWQ)",
"Hamburg Finkenwerder Airport (XFW)",
"Timbuktu Airport (TOM)",
"Gao (GAQ)",
"Augsburg Airport (AGB)",
"Abdelhafid Boussouf Bou Chekif Airport (TID)",
"Kokshetau Airport (KOV)",
"El Alamein International Airport (DBB)",
"Norrkoeping Airport (NRK)",
"OErebro Airport (ORB)",
"Taba International Airport (TCP)",
"Vilo Acuna Airport (CYO)",
"Joenkoeping Airport (JKG)",
"Odense Airport (ODE)",
"Karlstad Airport (KSD)",
"USAF Andersen Air Force Base (UAM)",
"Lawrence G. Hanscom Field (BED)",
"Bursa Yenisehir Airport (YEI)",
"Wevelgem Airport (KJK)",
"Chengdu Tianfu (TFU)",
"Jorge Chavez International Airport (LIM)",
"Denver International Airport (DEN)",
"Istanbul Airport (IST)",
"Hamad International Airport (DOH)",
"Adisutjipto International Airport (JOG)",
"Kempegowda International Airport (BLR)",
"Husein Sastranegara International Airport (BDO)",
"Santos Dumont Airport (SDU)",
"Eindhoven Airport (EIN)",
"Norfolk International Airport (ORF)",
"Akron-Canton Regional Airport (CAK)",
"Tbilisi International Airport (TBS)",
"Gold Coast Airport (OOL)",
"Easterwood Field (CLL)",
"Pristina International Airport (PIA) (PRN)",
"Greater Natal International Airport (NAT)",
"Teniente Benjamin Matienzo Airport (TUC)",
"Destin-Fort Walton Beach Airport (VPS)",
"Miami Executive Airport (TMB)",
"San Fernando Airport (FDO)",
"Appleton International Airport (ATW)",
"Chester County G O Carlson Airport (CTH)",
"Posadas Airport (PSS)",
"Comandante Espora Airport (BHI)",
"Memmingen Allgau Airport (FMM)",
"Harry Mwanga Nkumbula International Airport (LVI)",
"Purdue University Airport (LAF)",
"Eelde Airport (GRQ)",
"Bucharest Baneasa Aurel Vlaicu International Airport (BBU)",
"Seymour Airport (GPS)",
"Fort Wayne International Airport (FWA)",
"Canberra International Airport (CBR)",
"Auburn University Regional Airport (AUO)",
"Maringa Regional Airport (MGF)",
"Sauce Viejo Airport (SFN)",
"Corrientes International Airport (CNQ)",
"Enfidha - Hammamet International Airport (NBE)",
"Lyon-Bron Airport (LYN)",
"Chapelco - Aviador Carlos Campos Airport (CPC)",
"Brigadier Mayor Cesar Raul Ojeda Airport (LUQ)",
"Daytona Beach International Airport (DAB)",
"Hermes Quijada International Airport (RGA)",
"San Rafael Santiago Germano Airport (AFA)",
"Formosa Aerodrome (FMA)",
"Avalon Airport (AVV)",
"Melbourne International Airport (MLB)",
"Bangor International Airport (BGR)",
"Enrique Olaya Herrera Airport (EOH)",
"Parana Airport (PRA)",
"Komatsu Airport (KMQ)",
"Vicecomodoro Angel de la Paz Aragones Airport (SDE)",
"Capitan Vicente Almandos Almonacid Airport (IRJ)",
"Queenstown Airport (ZQN)",
"Kitakyushu Airport (KKJ)",
"Toulon-Hyeres Airport (TLN)",
"Kruger Mpumalanga International Airport (MQP)",
"Matsuyama Airport (MYJ)",
"Kobe Airport (UKB)",
"Misratah Airport (MRA)",
"Kelowna International Airport (YLW)",
"New Castle Airport (ILG)",
"Catamarca Airport (CTC)",
"Sunshine Coast Airport (MCY)",
"Duluth International Airport (DLH)",
"Santa Rosa Airport (RSA)",
"Lakeland Linder Regional Airport (LAL)",
"Pamplona Airport (PNA)",
"Debrecen International Airport (DEB)",
"Arnold Palmer Regional Airport (LBE)",
"Wonderboom Airport (PRY)",
"Kalibo International Airport (KLO)",
"Kushiro Airport (KUH)",
"John C. Munro Hamilton International Airport (YHM)",
"Glacier Park International Airport (FCA)",
"Joplin Regional Airport (JLN)",
"Anadolu Airport (AOE)",
"Zafer Airport (KZR)",
"Ithaca Tompkins Regional Airport (ITH)",
"Roberts Field (RDM)",
"Tokushima Airport (TKS)",
"Nelson Airport (NSN)",
"Aspen-Pitkin County Airport (ASE)",
"Kalamazoo-Battle Creek International Airport (AZO)",
"Leopold Sedar Senghor International Airport (DKR)",
"Plattsburgh International Airport (PBG)",
"Mackay Airport (MKY)",
"Leon Airport (LEN)",
"Rajiv Gandhi International Airport (HYD)",
"Essendon Airport (MEB)",
"Sioux Gateway Airport/Col. Bud Day Field (SUX)",
"Linkoeping City Airport (LPI)",
"University Park Airport (SCE)",
"Platov International Airport (ROV)",
"Thunder Bay International Airport (YQT)",
"Tottori Airport (TTJ)",
"Badajoz Airport (BJZ)",
"Manhattan Regional Airport (MHK)",
"Kitchener/Waterloo Airport (YKF)",
"Boca Raton Airport (BCT)",
"Majalengka Kertajati International Airport (KJT)",
"Lappeenranta Airport (LPP)",
"Yamagata Airport (GAJ)",
"Fort McMurray Airport (YMM)",
"Yamaguchi Ube Airport (UBJ)",
"Prince George Airport (YXS)",
"AEngelholm-Helsingborg Airport (AGH)",
"Chicago Executive Airport (PWK)",
"Wittman Regional Airport (OSH)",
"Port Macquarie Airport (PQQ)",
"Shenandoah Valley Regional Airport (SHD)",
"Cordoba Airport (ODB)",
"Okadama Airport (OKD)",
"Miho-Yonago Airport (YGJ)",
"Salisbury-Ocean City-Wicomico Regional Airport (SBY)",
"Laughlin Bullhead International Airport (IFP)",
"Izumo Airport (IZO)",
"Humberto Ghizzo Bortoluzzi Southern Regional Airport (JJG)",
"Zhukovsky International Airport (ZIA)",
"Brighton Shoreham Airport (ESH)",
"Donegal Airport (CFN)",
"Gwangju Airport (KWJ)",
"Castellon-Costa Azahar Airport (CDT)",
"Yeosu Airport (RSU)",
"Nakashibetsu Airport (SHB)",
"Nanaimo Airport (YCD)",
"Sliac Airport (SLD)",
"Lugano Airport (LUG)",
"Beziers Cap d'Agde Airport (BZR)",
"Windsor Airport (YQG)",
"Izhevsk Airport (IJK)",
"Williamsport-Lycoming County Airport (IPT)",
"Rio Cuarto Airport (RCU)",
"Fort Collins-Loveland Municipal Airport (FNL)",
"Malargue Airport (LGS)",
"Grande Prairie Airport (YQU)",
"Sydney/J.A. Douglas McCurdy Airport (YQY)",
"Waterford Airport (WAT)",
"Odate Noshiro Airport (ONJ)",
"Invercargill Airport (IVC)",
"Halmstad Airport (HAD)",
"Wood County Airport (PKB)",
"Northwest Florida Beaches International Airport (ECP)",
"Cherepovets Airport (CEE)",
"De Kooy (DHR)",
"London Oxford Airport (OXF)",
"Blackpool Airport (BLK)",
"Valdosta Regional Airport (VLD)",
"Dubbo City Regional Airport (DBO)",
"Pellston Regional Airport of Emmet County (PLN)",
"Tajima Airport (TJH)",
"Braunschweig-Wolfsburg Airport (BWE)",
"Cotswold Airport (GBA)",
"Vance Brand Airport (LMO)",
"Rivne International Airport (RWN)",
"McKinney Municipal Airport (TKI)",
"Chernivtsi International Airport (CWC)",
"Grabtsevo Airport (KLF)",
"Villa Reynolds Airport (VME)",
"Destin Executive Airport (DSI)",
"Ercan International Airport (ECN)",
"General Pico Airport (GPO)",
"Congonhas-Sao Paulo Airport (CGH)",
"Deelen Air Base (QAR)",
"Diyarbakir Airport (DIY)",
"Fujairah International Airport (FJR)",
"Memanbetsu Airport (MMB)",
"Nagoya Airport (NKM)",
"Grand Central Airport (GCJ)",
"Gagarin International Airport (GSV)",
"Hatay Airport (HTY)",
"Ordu Giresun Airport (OGU)",
"Tartu Airport (TAY)",
"Kristianstad Airport (KID)",
"Slave Lake Airport (YZH)",
"Elazig Airport (EZS)",
"Chennault International Airport (CWF)",
"Sanliurfa GAP Airport (GNY)",
"Mardin Airport (MQM)",
"Red Deer Regional Airport (YQF)",
"Vodochody Airport (VOD)",
"Balikesir Koca Seyit Airport (EDO)",
"Malatya Erhac Airport (MLX)",
"Sivas Airport (VAS)",
"Miramar Marine Corps Air Station (NKX)",
"Roskilde Airport (RKE)",
"Mount Isa Airport (ISA)",
"Altus Air Force Base (LTS)",
"Arnold Air Force Base (TUH)",
"Aviano Air Base (AVB)",
"Cannon Air Force Base (CVS)",
"Dover Air Force Base (DOV)",
"Duke Field (EGI)",
"Ellsworth Air Force Base (RCA)",
"Fairchild Air Force Base (SKA)",
"Grand Forks Air Force Base (RDR)",
"Hill Air Force Base (HIF)",
"Homestead Air Force Base (HST)",
"Laughlin Air Force Base (DLF)",
"Little Rock Air Force Base (LRF)",
"Macdill Air Force Base, Fl. (MCF)",
"Malmstrom Air Force Base (GFA)",
"Moody Air Force Base (VAD)",
"Mountain Home Air Force Base (MUO)",
"Offutt Air Force Base (OFF)",
"Patrick Air Force Base (COF)",
"Lakenheath Royal Air Force Base (LKZ)",
"Mildenhall Royal Air Force Base (MHZ)",
"Ramstein Air Base (RMS)",
"Robins Air Force Base (WRB)",
"Seymour-Johnson Air Force Base (GSB)",
"Spangdahlem Air Base (SPM)",
"Travis Air Force Base (SUU)",
"Tyndall Air Force Base (PAM)",
"Vance Air Force Base (END)",
"Whiteman Air Force Base (SZL)",
"Achmad Yani International Airport (SRG)",
"Scholes Field (GLS)",
"Batman Airport (BAL)",
"Edward Bodden Airfield (LYB)",
"Strausberg Airport (QPK)",
"Gunsan Airport (KUV)",
"Arad International Airport (ARW)",
"Sultan Ismail Petra Airport (KBR)",
"Hostomel Airport (GML)",
"Erzincan Airport (ERC)",
"Hat Yai International Airport (HDY)",
"Canakkale Airport (CKZ)",
"Santa Genoveva Airport (GYN)",
"Koh Samui Airport (USM)",
"Bayreuth (BYU)",
"Piestany Airport (PZY)",
"Grosse Ile Municipal Airport (ONZ)",
"Lombok International Airport (LOP)",
"Tucson International Airport (TUS)",
"Suleyman Demirel (ISE)",
"Minangkabau International Airport (PDG)",
"Adisumarmo International Airport (SOC)",
"Sligo Airport (SXL)",
"Billy Bishop Toronto City Airport (YTZ)",
"Sirnak Airport (NKT)",
"Lihue Airport (LIH)",
"Udon Thani International Airport (UTH)",
"Carlos Drummond de Andrade Airport (PLU)",
"Sultan Abdul Halim Airport (AOR)",
"Navegantes-Ministro Victor Konder International Airport (NVT)",
"Sultan Mahmud Airport (TGG)",
"Presidente Castro Pinto International Airport (JPA)",
"Mariscal Sucre International Airport (UIO)",
"Islamabad International Airport (ISB)",
"Southern Illinois Airport (MDH)",
"Mazar-i-Sharif Airfield (MZR)",
"Adiyaman Airport (ADF)",
"Londrina Aeroporto (LDB)",
"Miri Airport (MYY)",
"Prince Nayef bin Abdulaziz Regional Airport (ELQ)",
"Kastamonu Airport (KFS)",
"Igdir Airport (IGD)",
"Porto Seguro Airport (BPS)",
"Leite Lopes Airport (RAO)",
"Ponikve Airport (UZC)",
"Sinop Airport (NOP)",
"Teresina Airport (THE)",
"Radin Inten II International Airport (TKG)",
"Sultan Iskandar Muda International Airport (BTJ)",
"Aracaju Aeroporto (AJU)",
"Uberlandia-Ten. Cel. Av. Cesar Bombonato Airport (UDI)",
"Georgetown County Airport (GGE)",
"Campo de Marte Airport (MAE)",
"Speyer (ZQC)",
"Northwest Arkansas Regional Airport (XNA)",
"Puerto Princesa Airport (PPS)",
"Tipton Airport (FME)",
"Indiana County/Jimmy Stewart Field (IDI)",
"McGuire Air Force Base (WRI)",
"Enschede Airport Twente (ENS)",
"Nakhon Si Thammarat Airport (NST)",
"Tawau Airport (TWU)",
"Sao Jose do Rio Preto Airport (SJP)",
"Huntsville International Airport-Carl T Jones Field (HSV)",
"Depati Amir Airport (PGK)",
"Lauro Carneiro de Loyola Airport (JOI)",
"Joint Base Andrews (ADW)",
"Tinker Air Force Base (TIK)",
"Wright-Patterson Air Force Base (FFO)",
"Phoenix-Mesa Gateway Airport (AZA)",
"Lehigh Valley International Airport (ABE)",
"Governador Jorge Teixeira de Oliveira International Airport (PVH)",
"Asahikawa Airport (AKJ)",
"Luke Air Force Base (LUF)",
"Chattanooga Metropolitan Airport (CHA)",
"Kochi Ryoma Airport (KCZ)",
"Hilo International Airport (ITO)",
"Laguindingan International Airport (CGY)",
"Hyakuri Ab (IBR)",
"Miyako Airport (MMY)",
"Selçuk Efes Airport (IZM)",
"Vancouver Harbour Seaplane Base (CXH)",
"Sultan Haji Ahmad Shah Airport (KUA)",
"Szczytno-Szymany International Airport (SZY)",
"Usak Airport (USQ)",
"Chandler Field (AXN)",
"La Florida Airport (LSC)",
"Minot Air Force Base (MIB)",
"Aomori Airport (AOJ)",
"Manas International Airport (FRU)",
"Dala Airport (BLE)",
"Pune Airport (PNQ)",
"Charlottesville Albemarle Airport (CHO)",
"Victoria International Airport (YYJ)",
"Teterboro Airport (TEB)",
"Tashkent International Airport (TAS)",
"Haluoleo Airport (KDI)",
"Tokat Airport (TJK)",
"Batumi International Airport (BUS)",
"Nottingham Airport (NQT)",
"King Shaka International Airport (DUR)",
"Stewart International Airport (SWF)",
"Mt. Fuji Shizuoka Airport (FSZ)",
"Grider Field (PBF)",
"Orlando Bezerra de Menezes Airport (JDO)",
"Troyes-Barberey (QYR)",
"Shaw Air Force Base (SSC)",
"El Calafate Airport (FTE)",
"Fatmawati Soekarno Airport (BKS)",
"Nan Airport (NNT)",
"Gainesville Regional Airport (GNV)",
"Rapid City Regional Airport (RAP)",
"Jizan Regional Airport (GIZ)",
"Tomsk Bogashevo Airport (TOF)",
"Imperatriz (IMP)",
"Nantucket Memorial Airport (ACK)",
"Saga Airport (HSG)",
"Juwata International Airport (TRK)",
"Capitan FAP Guillermo Concha Iberico International Airport (PIU)",
"Hugo Cantergiani Regional Airport (CXJ)",
"Brisbane West Wellcamp Airport (WTB)",
"Caycuma (ONQ)",
"Selaparang Airport (AMI)",
"DeKalb Peachtree Airport (PDK)",
"Central Illinois Regional Airport at Bloomington-Normal (BMI)",
"Tokachi-Obihiro Airport (OBO)",
"Siirt Airport (SXZ)",
"Hoega Kusten Airport (KRF)",
"Montes Claros (MOC)",
"Trenton Mercer Airport (TTN)",
"Sonoma County Airport (STS)",
"Port Elizabeth International Airport (PLZ)",
"Luang Prabang International Airport (LPQ)",
"Tri-Cities Regional TN/VA Airport (TRI)",
"Tacloban Airport (TAC)",
"Comiso Airport (CIY)",
"Marine Corps Air Station Iwakuni (IWK)",
"Presidente Prudente Airport (PPB)",
"Grand Junction Regional Airport (GJT)",
"Mount Washington Regional Airport (HIE)",
"Sultan Azlan Shah Airport (IPH)",
"General Santos International Airport (GES)",
"Nakhon Phanom Airport (KOP)",
"Ulyanovsk Baratayevka Airport (ULV)",
"Gettysburg Regional Airport (GTY)",
"Evansville Regional Airport (EVV)",
"Cascavel Airport (CAC)",
"Martha's Vineyard Airport (MVY)",
"Professor Urbano Ernesto Stumpf Airport (SJK)",
"Eurico de Aguiar Salles Airport (VIX)",
"Amami Airport (ASJ)",
"Plan de Guadalupe International Airport (SLW)",
"Gelendzhik Airport (GDZ)",
"Tri-Cities Airport (PSC)",
"Jacarepagua-Roberto Marinho Airport (RRJ)",
"Neil Armstrong Airport (AXV)",
"Sultan Babullah Airport (TTE)",
"Pichoy Airport (ZAL)",
"Charlotte County Airport (PGD)",
"Durango-La Plata County Airport (DRO)",
"Morioka Hanamaki Airport (HNA)",
"Mount Gambier Airport (MGB)",
"Parana Airport (BFH)",
"Killeen-Fort Hood Regional Airport (GRK)",
"Regina International Airport (YQR)",
"Launceston Airport (LST)",
"Van Nuys Airport (VNY)",
"Baghdad International Airport (BGW)",
"McClellan-Palomar Airport (CLD)",
"Juneau International Airport (JNU)",
"Helsinki-Malmi Airport (HEM)",
"Haikou Meilan International Airport (HAK)",
"Republic Airport (FRG)",
"Branson Airport (BKG)",
"Albert J Ellis Airport (OAJ)",
"Noto Airport (NTQ)",
"Newquay Cornwall Airport (NQY)",
"Burke Lakefront Airport (BKL)",
"Campina Grande (CPV)",
"Erie International Airport (ERI)",
"Nizhnevartovsk Airport (NJC)",
"Inverness Airport (INV)",
"Elmira Corning Regional Airport (ELM)",
"Montrose Regional Airport (MTJ)",
"Hamilton Island Airport (HTI)",
"Mandalay International Airport (MDL)",
"University of Illinois Willard Airport (CMI)",
"Wayne County Airport (BJJ)",
"Fort Lauderdale Executive Airport (FXE)",
"Santa Fe Municipal Airport (SAF)",
"Central Wisconsin Airport (CWA)",
"Capitan de Corbeta Carlos A. Curbelo International Airport (PDP)",
"Dobbins Air Reserve Base (MGE)",
"Zhengzhou Xinzheng International Airport (CGO)",
"Shonai Airport (SYO)",
"Balikesir (BZI)",
"Wakkanai Airport (WKJ)",
"Charles B. Wheeler Downtown Airport (MKC)",
"Miami-Opa Locka Executive Airport (OPF)",
"Lynchburg Regional Airport (LYH)",
"Urmia Airport (OMH)",
"Pedro Otacilio Figueiredo Airport (VDC)",
"Greater Binghamton Airport (BGM)",
"Alexandria International Airport (AEX)",
"Tweed-New Haven Airport (HVN)",
"Ayers Rock Airport (AYQ)",
"Buri Ram Airport (BFV)",
"Matsumoto Airport (MMJ)",
"Hilton Head Airport (HHH)",
"Manuel Carlos Piar Guayana Airport (PZO)",
"Hachijojima Airport (HAC)",
"Inca Manco Capac International Airport (JUL)",
"Ningbo Lishe International Airport (NGB)",
"Juiz De Fora Airport (JDF)",
"Centennial Airport (APA)",
"Newcastle Airport (NTL)",
"Kuusamo Airport (KAO)",
"Jacinto Lara International Airport (BRM)",
"Lajes Field (TER)",
"Orlando Executive Airport (ORL)",
"Cincinnati Municipal Lunken Airport (LUK)",
"Mammoth Lakes Airport (MMH)",
"Caldas Novas Airport (CLV)",
"Hanty-Mansijsk (HMA)",
"Yuma International Airport (YUM)",
"Kuopio Airport (KUO)",
"Abraham Lincoln Capital Airport (SPI)",
"London International Airport (YXU)",
"Columbia Regional Airport (COU)",
"Valledupar / Alfonso Lopez (VUP)",
"Visakhapatnam Airport (VTZ)",
"Misawa Air Base (MSJ)",
"Pitt-Greenville Airport (PGV)",
"Taichung Airport (RMQ)",
"Fernando Luis Ribas Dominicci Airport (SIG)",
"Glenwood Springs Municipal Airport (GWS)",
"Palo Alto Airport of Santa Clara County (PAO)",
"Waco Regional Airport (ACT)",
"Niagara Falls International Airport (IAG)",
"Penza Airport (PEZ)",
"FAP Captain David Abenzur Rengifo International Airport (PCL)",
"Lakefront Airport (NEW)",
"Presidente Joao Batista Figueiredo Airport (OPS)",
"Robin Hood Airport Doncaster Sheffield (DSA)",
"Governador Valadares Airport (GVR, SBGV) (GVR)",
"Chumphon Airport (CJM)",
"Laguna Army Airfield (Yuma Proving Ground) (LGF)",
"Camarillo Airport (CMA)",
"Guiyang Longdongbao International Airport (KWE)",
"Farnborough Airport (FAB)",
"Addison Airport (ADS)",
"Shimoji-Shima Island (SHI)",
"Rendani Airport (MKW)",
"Gary/Chicago International Airport (GYY)",
"Cannes-Mandelieu (CEQ)",
"San Carlos Airport (SQL)",
"Florence Regional Airport (FLO)",
"Presidente Itamar Franco Airport (IZA)",
"Grozny Airport (GRV)",
"Sunan Shuofang International Airport (WUX)",
"Are OEstersund Airport (OSD)",
"Gazipasa-Alanya Airport (GZP)",
"M. Graham Clark Downtown Airport (PLK)",
"Montgomery Field Airport (MYF)",
"Naples Municipal Airport (APF)",
"Blimbingsari Airport (BWX)",
"Bern Airport (BRN)",
"Stockton Metropolitan Airport (SCK)",
"Gorno-Altaysk Airport (RGK)",
"Portsmouth International Airport at Pease (PSM)",
"Ketchikan International Airport (KTN)",
"Tri-State Airport (HTS)",
"Gillette-Campbell County Airport (GCC)",
"Wenzhou Longwan International Airport (WNZ)",
"Cajamarca (CJA)",
"Rotorua Airport (ROT)",
"Lijiang Sanyi Airport (LJG)",
"Rabil Airport (BVC)",
"Abbotsford Airport (YXX)",
"Charlottetown Airport (YYG)",
"Capitan FAP Pedro Canga Rodriguez Airport (TBP)",
"Kherson International Airport (KHE)",
"Kenneth Kaunda International Airport (LUN)",
"Ballina Byron Gateway Airport (BNK)",
"Spirit of St Louis Airport (SUS)",
"Pope Field (POB)",
"Gothenburg City Airport (GSE)",
"Millington, Millington Municipal Airport (NQA)",
"Altamira Airport (ATM)",
"Worcester Regional Airport (ORH)",
"Northeast Philadelphia Airport (PNE)",
"Svalbard Airport, Longyear (LYR)",
"Pokhara Airport (PKR)",
"Portland Hillsboro Airport (HIO)",
"Zhuhai Jinwan Airport (ZUH)",
"Jieyang Chaoshan Airport (SWA)",
"Szeged (QZD)",
"Selfridge Air National Guard Base (MTC)",
"Willow Run Airport (YIP)",
"Changchun Longjia International Airport (CGQ)",
"Albert Whitted Airport (SPG)",
"Srinagar International Airport (SXR)",
"Kapalua Airport (JHM)",
"Jack Northrop Field/Hawthorne Municipal Airport (HHR)",
"Nanchang Changbei International Airport (KHN)",
"Santa Maria Airport (RIA)",
"Yonaguni Airport (OGN)",
"Houston, Sugar Land Municipal / Hull Field Airport (SGR)",
"Ashtabula County Airport (JFN)",
"Martin State Airport (MTN)",
"Kinmen Airport (KNH)",
"Kish International Airport (KIH)",
"Northeast Florida Regional Airport (UST)",
"Penghu Magong Airport (MZG)",
"Linden Airport (LDJ)",
"Joensuu Airport (JOE)",
"Lancaster Airport (LNS)",
"Marietta, Cobb County-McCollum Field Airport (RYY)",
"Lipetsk Airport (LPK)",
"East London Airport (ELS)",
"The Florida Keys Marathon Airport (MTH)",
"Oakland County International Airport (PTK)",
"Hagerstown Regional Airport (HGR)",
"Syukuran Aminuddin Amir Airport (LUW)",
"Kisumu Airport (KIS)",
"Livermore Municipal Airport (LVK)",
"Hawke's Bay Airport (NPE)",
"Youngstown-Warren Regional Airport (YNG)",
"Muskegon County Airport (MKG)",
"Chino Airport (CNO)",
"Bartolomeu Lisandro Airport (CAW)",
"Chippewa Valley Regional Airport (EAU)",
"Falcon Field (MSC)",
"Yellowknife Airport (YZF)",
"Iwami Airport (IWJ)",
"Kamloops Airport (YKA)",
"Half Moon Bay Airport (HAF)",
"Blaise Diagne International Airport (DSS)",
"Paros National Airport (PAS)",
"Puerto Maldonado (PEM)",
"Salekhard Airport (SLY)",
"Brive-Souillac Airport (BVE)",
"Hualien Airport (HUN)",
"Mathis Field (SJT)",
"Saransk Airport (SKX)",
"Vanguardia Airport (VVC)",
"Renton Municipal Airport (RNT)",
"London Biggin Hill Airport (BQH)",
"Zamperini Field (TOA)",
"Erwin, Harnett County Airport (HRJ)",
"RAF Northolt (NHT)",
"Bowman Field (LOU)",
"Redding Municipal Airport (RDD)",
"Napa County Airport (APC)",
"East Hampton Airport (HTO)",
"Morgantown Municipal Airport (MGW)",
"Leeuwarden Air Base (LWR)",
"Hayward Executive Airport (HWD)",
"Magas Airport (IGT)",
"Volkel Airport (UDE)",
"Golden Triangle Regional Airport (GTR)",
"El Monte Airport (EMT)",
"Midtjyllands Airport (KRP)",
"Kirovsk-Apatity Airport (KVK)",
"Vero Beach Municipal Airport (VRB)",
"Ukhta Airport (UCT)",
"Essex County Airport (CDW)",
"Reid-Hillview of Santa Clara County Airport (RHV)",
"Gunnison County Airport (GUC)",
"Hamilton International Airport (HLZ)",
"Smith Reynolds Airport (INT)",
"Buttonville Municipal Airport (YKZ)",
"Shijiazhuang Zhengding International Airport (SJW)",
"King Khalid Air Base (KMX)",
"Reading Municipal Airport (RDG)",
"Leesburg Executive Airport (JYO)",
"Gillespie Field (SEE)",
"Monmouth Executive Airport (BLM)",
"Grand Case-Esperance Airport (SFG)",
"Sitka Rocky Gutierrez Airport (SIT)",
"Jyvaeskylae Airport (JYV)",
"Yariguies Airport (EJA)",
"Nis Constantine the Great Airport (INI)",
"Kurgan Airport (KRO)",
"Coronel FAP Alfredo Mendivil Duarte Airport (AYP)",
"Parauapebas Carajas Airport (CKS)",
"Quanzhou Jinjiang International Airport (JJN)",
"Hanimaadhoo Airport (HAQ)",
"Gregg County Airport (GGG)",
"Anápolis Airport (APS)",
"Taitung Airport (TTT)",
"Topeka Regional Airport (FOE)",
"Yantai Penglai International Airport (YNT)",
"Lewiston-Nez Perce County Airport (LWS)",
"Limbang Airport (LMN)",
"Sudbury Airport (YSB)",
"Sheppard Air Force Base/Wichita Falls Municipal Airport (SPS)",
"French Valley Airport (RBK)",
"Twin Falls Airport (TWF)",
"Tampa, Peter O Knight Airport (TPF)",
"Telluride Regional Airport (TEX)",
"Brainerd Lakes Regional Airport (BRD)",
"Kajaani Airport (KAJ)",
"Southwest Georgia Regional Airport (ABY)",
"Praia International Airport (RAI)",
"Yinchuan Hedong International Airport (INC)",
"Osh Airport (OSS)",
"Tauranga Airport (TRG)",
"Cap. FAP Victor Montes Arias Airport (TYL)",
"Saint Louis Downtown - Parks Airport (CPS)",
"Lebanon Municipal Airport (LEB)",
"Greenville Downtown Airport (GMU)",
"Brown Field Municipal Airport (SDM)",
"Sacramento Executive Airport (SAC)",
"Ponta Pora International Airport (PMG)",
"Lake Tahoe Airport (TVL)",
"Jack Brooks Regional Airport (BPT)",
"Texarkana Regional Airport (TXK)",
"Molde Airport, Aro (MOL)",
"Parnaiba Aeroporto (PHB)",
"Provo Municipal Airport (PVU)",
"Norwood Memorial Airport (OWD)",
"Block Island State Airport (BID)",
"Walla Walla Regional Airport (ALW)",
"Baguio-Loakan Airport (BAG)",
"Page Municipal Airport (PGA)",
"Richmond Executive-Chesterfield County Airport (FCI)",
"Yellowstone Regional Airport (COD)",
"Malindi Airport (MYD)",
"Provincetown Municipal Airport (PVC)",
"Tambov Donskoye Airport (TBW)",
"Dayton-Wright Brothers Airport (MGY)",
"Buchanan Field (CCR)",
"Blumenau Airport (BNU)",
"Pangborn Field (EAT)",
"Phoenix Goodyear Airport (GYR)",
"Southern California Logistics Airport (VCV)",
"Waukesha County Airport (UES)",
"Barkley Regional Airport (PAH)",
"W K Kellogg Airport (BTL)",
"Rostock-Laage Airport (RLG)",
"Yakima Air Terminal Airport (YKM)",
"Greenbrier Valley Airport (LWB)",
"Waukegan National Airport (UGN)",
"Penggung Airport (CBN)",
"Venice Municipal Airport (VNC)",
"Ji-Parana Airport (JPR)",
"Araraquara Airport (AQA)",
"Albury Airport (ABX)",
"Oxnard Airport (OXR)",
"Whiteman Airport (WHP)",
"Ratmalana Airport (RML)",
"Pocatello Regional Airport (PIH)",
"Bemidji-Beltrami County Airport (BJI)",
"Connemara Regional Airport (NNR)",
"Pocos De Caldas (POO)",
"Cuyahoga County Airport (CGF)",
"Canyonlands Field (CNY)",
"Eagle Creek Airpark (EYE)",
"Fort Worth Meacham International Airport (FTW)",
"Bagram (OAI)",
"Montgomery County Airpark (GAI)",
"Amakusa Airport (AXJ)",
"Athens-Ben Epps Airport (AHN)",
"San Bernardino International Airport (SBD)",
"Gloucestershire Airport (GLO)",
"Visalia Municipal Airport (VIS)",
"Sion Airport (SIR)",
"Cesaria Evora Airport (VXE)",
"Chandler Municipal Airport (CHD)",
"Jiuzhai Huanglong Airport (JZH)",
"NATO Air Base Geilenkirchen (GKE)",
"West Houston Airport (IWS)",
"Kokkola-Pietarsaari Airport (KOK)",
"Havryshivka International Airport (VIN)",
"Dong Hoi Airport (VDH)",
"Hervey Bay Airport (HVB)",
"Dutchess County Airport (POU)",
"Braganca Paulista Airport (BJP)",
"Chico Municipal Airport (CIC)",
"Batajnica Air Base (BJY)",
"City of Derry Airport (LDY)",
"Vologda Airport (VGD)",
"Philadelphia Seaplane Base (PSQ)",
"Sault Ste. Marie Airport (YAM)",
"Naxos Airport (JNX)",
"Ocala International Airport (OCF)",
"Limoges-Bellegarde (LIG)",
"Merrill Field Airport (MRI)",
"Stinson Municipal Airport (SSF)",
"Yoshkar-Ola Airport (JOK)",
"Weihai Airport (WEH)",
"Groton-New London Airport (GON)",
"Pori Airport (POR)",
"Lawrence J. Timmerman Airport (MWC)",
"Saint-Etienne-Boutheon Airport (EBU)",
"Waterkloof Air Force Base (WKF)",
"Paranagua Airport (PNG)",
"Uralsk Airport (URA)",
"Rodez-Marcillac Airport (RDZ)",
"Four Corners Regional Airport (FMN)",
"New Bedford Regional Airport (EWB)",
"Sacramento Mather Airport (MHR)",
"Islay Airport (ILY)",
"Changzhou Benniu Airport (CZX)",
"Zhytomyr Airport (ZTR)",
"Garden City Regional Airport (GCK)",
"Nausori International Airport (SUV)",
"Mc Clellan Airfield (MCC)",
"Wagga Airport (WGA)",
"Mildura Airport (MQL)",
"Decatur Airport (DEC)",
"Lake Havasu City Airport (HII)",
"Rishiri Airport (RIS)",
"Kushok Bakula Rimpochhe Airport (IXL)",
"Frankfurt Egelsbach Airport (QEF)",
"Suarlee Airport (QNM)",
"Riverside Municipal Airport (RAL)",
"Chippewa County International Airport (CIU)",
"Southwest Oregon Regional Airport (OTH)",
"Kristiansund Airport, Kvernberget (KSU)",
"Tainan Airport (TNN)",
"Owensboro-Daviess County Regional Airport (OWB)",
"Okmulgee Regional Airport (OKM)",
"General Wm J Fox Airfield (WJF)",
"Svolvaer Airport, Helle (SVJ)",
"Huangshan Tunxi International Airport (TXN)",
"Kropyvnytskyi Airport (KGO)",
"Cambridge Airport (CBG)",
"Kodiak Benny Benson State Airport (ADQ)",
"Jandakot Airport (JAD)",
"Pierre Regional Airport (PIR)",
"Armidale Airport (ARM)",
"Val-d'Or Airport (YVO)",
"Notodden Airport (NTB)",
"Blackbushe Airport (BBS)",
"Valence-Chabeuil Airport (VAF)",
"Southern Wisconsin Regional Airport (JVL)",
"Fond du Lac County Airport (FLD)",
"Westfield-Barnes Regional Airport (BAF)",
"Poltava - Suprunivka Airport (PLV)",
"Dinard-Pleurtuit-Saint-Malo Airport (DNR)",
"Bagotville Airport (YBG)",
"Padua Airport (QPA)",
"Maafaru International Airport (NMF)",
"Yellowstone Airport (WYS)",
"Bartow Municipal Airport (BOW)",
"Isfahan International Airport (IFN)",
"Oceanside Municipal Airport (OCN)",
"Whitsunday Island Airport (WSY)",
"Plymouth Municipal Airport (PYM)",
"Ohio State University Airport (OSU)",
"Portage la Prairie Southport Airport (YPG)",
"Friday Harbor Airport (FRD)",
"Alcantari International Airport (SRE)",
"Fort St. John Airport (YXJ)",
"Yichang Sanxia Airport (YIH)",
"Gomel Airport (GME)",
"Sitia Airport (JSH)",
"Adirondack Regional Airport (SLK)",
"Klamath Falls Airport (LMT)",
"Xishuangbanna Gasa Airport (JHG)",
"Madison Municipal Airport (MDN)",
"MBS International Airport (MBS)",
"Montgomery County Airport (CXO)",
"Weston Airport (WST)",
"Miskolc Airport (MCQ)",
"Wiarton Airport (YVV)",
"Santa Paula Airport (SZP)",
"Penticton Airport (YYF)",
"Las Cruces International Airport (LRU)",
"Flores Airport (FLW)",
"Syros Island National Airport (JSY)",
"Trento-Mattarello Airport (ZIA)",
"Aiken Municipal Airport (AIK)",
"Kingman Airport (IGM)",
"Alferez FAP David Figueroa Fernandini Airport (HUU)",
"Ikaria Airport (JIK)",
"Lannion (LAI)",
"Radom Airport (RDO)",
"Park Township Airport (HLM)",
"DuPage Airport (DPA)",
"Sebring Regional Airport (SEF)",
"Clearwater Air Park (CLW)",
"Vorkuta Airport (VKT)",
"David Wayne Hooks Memorial Airport (DWH)",
"Verona Brescia Airport (VBS)",
"Coeur d'Alene Airport (COE)",
"Kimberley Airport (KIM)",
"Brest Airport (BQT)",
"Leros Municipal Airport (LRS)",
"Heviz-Balaton Airport (SOB)",
"Lima Allen County Airport (AOH)",
"Merced Municipal Airport (MCE)",
"Virginia Tech/Montgomery Executive Airport (BCB)",
"Ahwaz Airport (AWZ)",
"El Tehuelche Airport (PMY)",
"Quincy Regional Airport (UIN)",
"Linyi Shubuling Airport (LYI)",
"Michigan City Municipal Airport-Phillips Field (MGC)",
"Kyzyl Airport (KYZ)",
"Maimun Saleh Airport (SBG)",
"Lawrence Municipal Airport (LWM)",
"Logrono-Agoncillo Airport (RJL)",
"Petersburg James A. Johnson Airport (PSG)",
"Sao Joao del-Rei Airport (JDR)",
"Hemet-Ryan Airport (HMT)",
"Semipalatinsk Airport (PLX)",
"Chautauqua County-Jamestown Airport (JHW)",
"Tunica Municipal Airport (UTM)",
"Black Forest Airport (LHA)",
"Xuzhou Guanyin Airport (XUZ)",
"Grand Strand Airport (CRE)",
"Tefe Airport (TFF)",
"Yanji Chaoyangchuan Airport (YNJ)",
"Schenectady County Airport (SCH)",
"Khajuraho Airport (HJR)",
"Sabetta International Airport (SBT)",
"Lolland Falster Airport (MRW)",
"Stornoway Airport (SYY)",
"Yancheng Nanyang Airport (YNZ)",
"Willoughby Lost Nation Municipal Airport (LNN)",
"Texas Gulf Coast Regional Airport (LJN)",
"Dyess Air Force Base (DYS)",
"Taizhou Luqiao Airport (HYN)",
"Zephyrhills Municipal Airport (ZPH)",
"Nantong Airport (NTG)",
"Kryvyi Rih/Lozuvatka International Airport (KWG)",
"Kiel / Holtenau Civilian (KEL)",
"Yangzhou Taizhou Airport (YTY)",
"Polokwane International Airport (PTG)",
"Fort Worth Alliance Airport (AFW)",
"Noumea Magenta Airport (GEA)",
"Lianyungang Airport (LYG)",
"Mianyang Nanjiao Airport (MIG)",
"Wilmington Airpark (ILN)",
"Jose Leonardo Chirino Airport (CZE)",
"Khudjand International Airport (LBD)",
"Land's End Airport (LEQ)",
"Taganrog Yuzhny Airport (TGK)",
"Porter County Regional Airport (VPZ)",
"Ogdensburg International Airport (OGS)",
"Pembroke Airport (YTA)",
"Smyrna Airport (MQY)",
"Doylestown Airport (DYL)",
"Altenburg Nobitz (AOC)",
"Calexico International Airport (CXL)",
"Oshawa Airport (YOO)",
"Kerman Airport (KER)",
"Shawnee Regional Airport (SNL)",
"Rochester Airport (RCS)",
"Sevastopol International Airport (UKS)",
"Orleans - Saint-Denis-de-l'Hotel Airport (ORE)",
"Eastern Oregon Regional Airport (PDT)",
"Pierrefonds Airport (ZSE)",
"Rasht Airport (RAS)",
"Barra Airport (BRR)",
"Manjimup Airport (MJP)",
"Ibadan Airport (IBA)",
"New Tanegashima Airport (TNE)",
"Angers - Loire Airport (ANE)",
"Sohag International Airport (HMB)",
"Smiths Falls-Montague Airport (YSH)",
"Varrelbusch Airport (VAC)",
"Ravenna Airport (RAN)",
"Simferopol International Airport (SIP)",
"Tsushima Airport (TSJ)",
"La Plata Airport (LPG)",
"Eagle River Union Airport (EGV)",
"Sawyer International Airport (MQT)",
"Sumy Airport (UMY)",
"Sulphur Springs Municipal Airport (SLR)",
"Dandong Langtou Airport (DDG)",
"Kenosha Regional Airport (ENW)",
"St Mary's Airport (ISC)",
"Capital City Airport (HAR)",
"Lidkoeping-Hovby Airport (LDK)",
"Metz-Nancy-Lorraine Airport (ETZ)",
"Sindal Airport (CNL)",
"San Luis Valley Regional Airport (ALS)",
"Lake Simcoe Regional Airport (YLK)",
"Space Coast Regional Airport (TIX)",
"West Michigan Regional Airport (BIV)",
"Cape May County Airport (WWD)",
"Salinas Municipal Airport (SNS)",
"Tokunoshima Airport (TKN)",
"Coventry Airport (CVT)",
"Jackson County Airport (JXN)",
"Hanover County Municipal Airport (OFP)",
"Carson Airport (CSN)",
"Virginia Highlands Airport (VJI)",
"Marana Regional Airport (AVW)",
"Okinoerabu Airport (OKE)",
"Tamworth Airport (TMW)",
"McNary Field (SLE)",
"Ocean City Municipal Airport (OCE)",
"Hickory Regional Airport (HKY)",
"Oberpfaffenhofen Airport (OBF)",
"Kalgoorlie-Boulder Airport (KGI)",
"Nellis Air Force Base (LSV)",
"Tulcea Airport (TCE)",
"Kostroma Sokerkino Airport (KMW)",
"Sheridan County Airport (SHR)",
"La Grange, Callaway Airport (LGC)",
"Akron Fulton International Airport (AKC)",
"Lethbridge Airport (YQL)",
"Millville Municipal Airport (MIV)",
"Crystal Airport (MIC)",
"Cherkasy International Airport (CKC)",
"Kikai Airport (KKX)",
"Nakhon Ratchasima Airport (NAK)",
"Leesburg International Airport (LEE)",
"Mackinac Island Airport (MCD)",
"Casa Grande Municipal Airport (CGZ)",
"Gyor-Per International Airport (QGY)",
"Burgos Villafria Airport (RGS)",
"Pecs - Pogany Airport (PEV)",
"Dasht-e Naz Airport (SRY)",
"Jingdezhen Airport (JDZ)",
"Pitt Meadows Airport (YPK)",
"Abel Santamaria Airport (SNU)",
"Carroll County Regional Airport/Jack B Poage Field (DMW)",
"Mansfield Lahm Regional Airport (MFD)",
"Riverton Regional Airport (RIW)",
"Felts Field (SFF)",
"Concord Municipal Airport (CON)",
"Chilliwack Airport (YCW)",
"Tinson Pen Aerodrome (KTP)",
"Sarnia Chris Hadfield Airport (YZR)",
"Brandywine Airport (OQN)",
"Johnston Regional Airport (JNX)",
"Carl R Keller Field Airport (PCW)",
"Kaerdla Airport (KDL)",
"Nut Tree Airport (VCB)",
"Datong Airport (DAT)",
"Minamidaitojima (MMD)",
"Savonlinna Airport (SVL)",
"Ian Fleming International Airport (OCJ)",
"Meriden Markham Municipal Airport (MMK)",
"Beverly Municipal Airport (BVY)",
"Bend Municipal Airport (BDN)",
"Lucca-Tassignano Airport (LCV)",
"San Fernando Airport (SFE)",
"Southwest Michigan Regional Airport (BEH)",
"Blythe Airport (BLH)",
"Xiangfan Airport (XFN)",
"Auburn Municipal Airport (AUN)",
"Boralday Airport (BXJ)",
"Lahti Vesivehmaa Airport (QLF)",
"Smith Field (SMD)",
"Shreveport Downtown Airport (DTN)",
"Mo i Rana Airport, Rossvoll (MQN)",
"Seinajoki-Ilmajok Airport (SJY)",
"Union County Airport (MRT)",
"Atibaia (ZBW)",
"Ryan Field (RYN)",
"Whangarei Airport (WRE)",
"Galway Airport (GWY)",
"Lumberton Regional Airport (LBT)",
"Medicine Hat Airport (YXH)",
"Zilina Airport (ILZ)",
"Chichester/Goodwood Airport (QUG)",
"Navoi Airport (NVI)",
"Pocono Mountains Municipal Airport (MPO)",
"Trollhaettan-Vaenersborg Airport (THN)",
"Manitowoc County Airport (MTW)",
"Mefford Field (TLR)",
"Kalaeloa Airport (JRF)",
"Paso Robles Municipal Airport (PRB)",
"Verona-Boscomantico Airfield (QBS)",
"Kastoria National Airport (KSO)",
"Plant City Airport (PCM)",
"Council Bluffs Municipal Airport (CBF)",
"Grant County International Airport (MWH)",
"Springdale Municipal Airport (SPZ)",
"Swansea Airport (SWS)",
"Jonesboro Municipal Airport (JBR)",
"Santa Ynez Airport (SQA)",
"St Mary's County Regional Airport (LTW)",
"Bonn-Hangelar (BNJ)",
"Prince Rupert Airport (YPR)",
"Luiz dal Canalle Filho Airport (TOW)",
"Khmelnytskyi Airport (HMJ)",
"Riviera Airport (ALL)",
"Son Bonet Aerodrome (SBO)",
"Toowoomba Airport (TWB)",
"Lydd Airport (LYX)",
"Sanford, Sanford-Lee County Regional Airport (TTA)",
"Barra do Garcas Airport (BPG)",
"Fitchburg Municipal Airport (FIT)",
"Huai'an Lianshui Airport (HIA)",
"Lan Yu (KYD)",
"Delaware Municipal Airport - Jim Moore Field (DLZ)",
"Nanyang Jiangying Airport (NNY)",
"Cenej Airport (QND)",
"Griffith Airport (GFF)",
"Goshen Municipal Airport (GSH)",
"Toledo Executive Airport (TDZ)",
"Auburn Lewiston Municipal Airport (LEW)",
"Davenport Municipal Airport (DVN)",
"Heligoland Airport (HGL)",
"Lorain County Regional Airport (LPR)",
"Lismore Airport (LSY)",
"Ohio University Airport-Snyder Field (ATO)",
"Kerama Airport (KJP)",
"Vigan Mindoro Airport (VGN)",
"Hof-Plauen Airport (HOQ)",
"Kitadaito Airport (KTD)",
"Salerno Costa d'Amalfi Airport (QSR)",
"Bentonville Municipal Airport (VBT)",
"Clovis Municipal Airport (CVN)",
"Norderney (NRD)",
"Yuncheng Guangong Airport (YCU)",
"Mudanjiang Hailang International Airport (MDG)",
"Sunriver Airport (SUO)",
"Inhambane Airport (INH)",
"Springfield-Beckley Municipal Airport (SGH)",
"Mariupol International Airport (MPW)",
"Rochelle Municipal Airport (RPJ)",
"Buffalo-Lancaster Regional Airport (BQR)",
"Warrenton-Fauquier Airport (HWY)",
"Petropavlosk South Airport (PPK)",
"Aurillac Airport (AUR)",
"Monett Municipal Airport (HFJ)",
"Columbia Airport (COA)",
"Castle Airport (MER)",
"Wangerooge (AGE)",
"Albacete Airport (ABC)",
"Capital City Airport (FFT)",
"Horseshoe Bay Resort Airpark (DZB)",
"Astoria Regional Airport (AST)",
"Changde Airport (CGD)",
"Yulin Yuyang Airport (UYN)",
"Los Alamos Airport (LAM)",
"Morrisville-Stowe State Airport (MVL)",
"Jining Qufu Airport (JNG)",
"Vardo Airport, Svartnes (VAW)",
"Sikeston Memorial Municipal Airport (SIK)",
"Walter J. Koladza Airport (GBR)",
"Conway-Horry County Airport (HYW)",
"Lompoc Airport (LPC)",
"Midland Airpark (MDD)",
"Rocky Mount-Wilson Regional Airport (RWI)",
"Perth / Scone (PSL)",
"Newport Municipal Airport (ONP)",
"Williamson-Sodus Airport (SDC)",
"Dawson Creek Airport (YDQ)",
"Eisenach-Kindel Airport (EIB)",
"Tangshan Sannuehe Airport (TVS)",
"Donaueschingen / Villingen (ZQL)",
"Madera Municipal Airport (MAE)",
"Richland Airport (RLD)",
"St Clair County International Airport (PHN)",
"Melfa / Accomack Airport (MFV)",
"Handan Airport (HDG)",
"Kasos Island Public Airport (KSJ)",
"Lensk Airport (ULK)",
"Lake City Gateway Airport (LCQ)",
"Zanesville Municipal Airport (ZZV)",
"Frankfort Municipal Airport (FKR)",
"Hengchun Airport (HCN)",
"Greater Portsmouth Regional Airport (PMH)",
"Nikolayevsk-na-Amure Airport (NLI)",
"Rizhao Shanzihe Airport (RIZ)",
"Worms Airfield (ZQV)",
"Chizhou Jiuhuashan Airport (JUH)",
"Tabarka-Ain Draham International Airport (TBJ)",
"Edward F Knapp State Airport (MPV)",
"Sea-Airport Cuxhaven/Nordholz (FCN)",
"Bitburg Airport (BBJ)",
"Nauru International Airport (INU)",
"Wheeling Ohio County Airport (HLG)",
"Newark Heath Airport (VTA)",
"Bembridge Airport (BBP)",
"Front Royal-Warren County Airport (FRR)",
"Mount Pleasant Municipal Airport (MOP)",
"Lyudao (Green Island) Airport (GNI)",
"Harry Clever Field Airport (PHD)",
"Dyersburg Municipal Airport (DYR)",
"Holloman Air Force Base (HMN)",
"Husavik Airport (HZK)",
"Cantrell Field Airport (CWS)",
"Campbeltown Airport (CAL)",
"Central Jersey Regional Airport (JVI)",
"Huesca-Pirineos Airport (HSK)",
"Inverness Airport (INF)",
"Boire Field (ASH)",
"Pattaya Airpark (PYX)",
"Brooks Field (RMY)",
"Polyarny Airport (PYJ)",
"Corvo Airport (CVU)",
"Quzhou Airport (JUZ)",
"Ralph Wien Memorial Airport (OTZ)",
"Andahuaylas Airport (ANS)",
"Fitch H. Beach Airport (FPK)",
"Hagfors Airport (HFS)",
"Kauhava Airport (KAU)",
"Monroe County Airport (MVC)",
"Myitkyina Airport (MYT)",
"La Porte Municipal Airport (LPO)",
"Dazhou Heshi Airport (DAX)",
"Marshfield Municipal Airport (MFI)",
"Mason County Airport (LDM)",
"Miami University Airport (OXD)",
"Thomas C Russell Field (ALX)",
"Gafsa - Ksar International Airport (GAF)",
"Taldykorgan Airport (TDK)",
"Lenawee County Airport (ADG)",
"Davis Field Airport (MKO)",
"Bildudalur Airport (BIU)",
"Enontekio Airport (ENF)",
"Huntington Municipal Airport (HHG)",
"Carroll County-Tolson Airport (TSO)",
"Penn Valley Airport (SEG)",
"Harrison County Airport (ASL)",
"Dannelly Field (MGM)",
"Torsby Airport (TYF)",
"Belfast Municipal Airport (BST)",
"Safford Regional Airport (SAD)",
"Thule Air Base (THU)",
"Dillingham Airport (DLG)",
"Winona Municipal Airport (ONA)",
"Chautauqua County-Dunkirk Airport (DKK)",
"Ken Jones Aerodrome (POT)",
"Three Rivers Municipal Dr. Haines Airport (HAI)",
"Fulton County Airport (RCR)",
"Hastings Municipal Airport (HSI)",
"Mission Field (LVM)",
"Saint-Brieuc - Armor Airport (SBK)",
"Anshan Teng'ao Airport (AOG)",
"Branch County Memorial Airport (OEB)",
"Grants Pass Airport (GTP)",
"Mifflin County Airport (RED)",
"Aguni Airport (AGJ)",
"Jiujiang Lushan Airport (JIU)",
"Dijon-Longvic Air Base (DIJ)",
"Fulton County Airport (USE)",
"Galion Municipal Airport (GQQ)",
"Madison County Airport (UYF)",
"Le Mars Municipal Airport (LRJ)",
"Cameron Memorial Airport (EZZ)",
"Lawrence County Airpark (HTW)",
"Van Wert County Airport (VNW)",
"Stralsund Barth Airport (BBH)",
"Achinsk Airport (ACS)",
"Daviess County Airport (DCY)",
"Defiance Memorial Airport (DFI)",
"Crookston Municipal Airport-Kirkwood Field (CKN)",
"Frank Wiley Field (MLS)",
"Oroville Municipal Airport (OVE)",
"San Carlos Apache Airport (GLB)",
"Darke County Airport (VES)",
"Heide-Buesum Airport (HEI)",
"Henry County Airport (PHT)",
"Kauhajoki Airport (KHJ)",
"Linfen Qiaoli Airport (LFQ)",
"Lizard Island Airport (LZR)",
"Bellefontaine Regional Airport (EDJ)",
"Cold Bay Airport (CDB)",
"Westover Air Reserve Base / Metropolitan Airport (CEF)",
"Cambridge Municipal Airport (CDI)",
"Grimsey Airport (GRY)",
"Chi Mei Airport (CMJ)",
"St. Landry Parish Airport (OPL)",
"Beatrice Municipal Airport (BIE)",
"Langlade County Airport (AIG)",
"Adak Airport (ADK)",
"Papa Westray Airport (PPW)",
"Lakefield Airport (CQA)",
"Delaware County Regional Airport (MIE)",
"Broken Bow Municipal Airport (BBW)",
"Hyderabad Airport (HDD)",
"Malden Regional Airport (MAW)",
"Putnam County Airport (OWX)",
"Modesto City-County Airport (MOD)",
"Pike County Airport (EOP)",
"Qaqortoq Heliport (JJU)",
"Wang-an Airport (WOT)",
"Baise Bama Airport (AEB)",
"Bowman Regional Airport (BWM)",
"Highland County Airport (HOC)",
"Guyuan Liupanshan Airport (GYU)",
"Lowcountry Regional Airport (RBW)",
"Saint Helena Airport (HLE)",
"Caledonia County Airport (LLX)",
"Kaufana Airport (EUA)",
"Liupanshui Yuezhao Airport (LPF)",
"Brevig Mission Airport (KTS)",
"Artigas International Airport (ATI)",
"Paamiut Airport (JFR)",
"Shashi Airport (SHS)",
"Tomas de Heres Airport (CBL)",
"Tortuguero Airport (TTQ)",
"Afore Airport (AFR)",
"Afton Municipal Airport (AFO)",
"Afutara Aerodrome (AFT)",
"Afyon Airport (AFY)",
"Agartala Airport (IXA)",
"Agatti Airport (AGX)",
"Agaun Airport (AUP)",
"Agen-la-Garenne (AGF)",
"Aggeneys Airport (AGZ)",
"Aghajari Airport (AKW)",
"Agnew Airport (AGW)",
"Agostinho Neto Airport (NTO)",
"Agra Airport (AGR)",
"Agri Airport (AJI)",
"Agropecuaria Castanhais Airport (ITI)",
"Aguaclara Airport (ACL)",
"Aguas Claras Airport (OCV)",
"Ahe Airport (AHE)",
"Ahmed Al Jaber Air Base (XIJ)",
"Ahuas Airport (AHS)",
"Ai Ais (AIW)",
"Aiambak Airport (AIH)",
"Ailinglaplap Airok Airport (AIC)",
"Ailuk Airfield (AIM)",
"Ouargla (OGX)",
"Ainsworth Municipal Airport (ANW)",
"Aiome Airport (AIE)",
"Aioun el Atrouss Airport (AEO)",
"Aishalton Airport (AHL)",
"Aitape Airport (ATP)",
"Aitutaki Airport (AIT)",
"Aiyura Airport (AYU)",
"AJ Eisenberg Airport (ODW)",
"APT Pranoto Airport (AAP)",
"Akanu Ibiam International Airport (ENU)",
"Akhiok Airport (AKK)",
"Akiachak Airport (KKI)",
"Akiak Airport (AKI)",
"Akieni Airport (AKE)",
"Akita Airport (AXT)",
"Akjoujt (AJJ)",
"Aklavik/Freddie Carmichael Airport (LAK)",
"Akola Airport (AKD)",
"Aksu Airport (AKU)",
"Akulivik Airport (AKV)",
"Akunnaaq Heliport (QCU)",
"Akure Airport (AKR)",
"Akutan Airport (KQA)",
"Akwa Ibom International Airport (QUO)",
"Al Abraq International Airport (LAQ)",
"Al Ahsa Airport (HOF)",
"Wadi Ain Airport (WDA)",
"Al Asad Airbase (IQA)",
"Al-Baha Domestic Airport (ABT)",
"Al Bayda Airport (BYD)",
"Albuq Airport (BUK)",
"Al Dhafra Air Base (DHF)",
"Al Ghaidah International Airport (AAY)",
"Al Jouf Domestic Airport (AJF)",
"Al Minhad Air Base (NHD)",
"Al Qaisumah/Hafr Al Batin Airport (AQI)",
"Al Fallujah (TQD)",
"Al Thaurah (SOR)",
"Al Udeid Air Base (XJD)",
"Al Wajh Domestic Airport (EJH)",
"Alakanuk Airport (AUK)",
"Alamogordo-White Sands Regional Airport (ALM)",
"Alamos Airport (XAL)",
"Albany Airport (ALH)",
"Alberni Valley Regional Airport (YPB)",
"Albert - Picardie Airport (BYF)",
"Albert Lea Municipal Airport (AEL)",
"Alberto Carnevalli Airport (MRD)",
"Alberto Delgado Airport (TND)",
"Sogamoso Airport (SOX)",
"Albertus Airport (FEP)",
"Albertville (XAV)",
"Albi-Le Sequestre (LBI)",
"Albina Airstrip (ABN)",
"Alcides Fernandez Airport (ACD)",
"Aldan Airport (ADH)",
"Alderney Airport (ACI)",
"Alegrete Novo Airport (ALQ)",
"Aleknagik /New Airport (WKK)",
"Alenquer Airport (ALT)",
"Aleppo International Airport (ALP)",
"Alert Bay Airport (YAL)",
"Alert Airport (YLT)",
"Alerta Airport (ALD)",
"Alexander Bay Airport (ALJ)",
"Alexander Field South Wood County Airport (ISW)",
"Alexandra (ALR)",
"Alexandria Homestead Airport (AXL)",
"Las Lomitas (LLS)",
"Alferez FAP Alfredo Vladimir Sara Bauer Airport (AOP)",
"Algeciras Heliport (AEI)",
"Algona Municipal Airport (AXG)",
"Ali-Sabieh Airport (AII)",
"Nasiriyah Ali Air Base (XNH)",
"Alice International Airport (ALI)",
"Alitak Seaplane Base (ALZ)",
"Allah Valley Airport (AAV)",
"Allahabad Airport (IXD)",
"Allakaket Airport (AET)",
"Alldays Airport (ADY)",
"Allegheny County Airport (AGC)",
"Allen Army Air Field (BIG)",
"Allen C Perkinson Municipal Airport (BKT)",
"Alliance Municipal Airport (AIA)",
"Ailigandi Airport (AIL)",
"Alluitsup Paa Heliport (LLU)",
"Alma Airport (YTF)",
"Almeirim Airport (GGF)",
"Almirante Marcos A. Zar Airport (REL)",
"Almirante Padilla Airport (RCH)",
"Along Airport (IXV)",
"Chitre Airport (CTD)",
"Alowe Airport (AWE)",
"Alpena County Regional Airport (APN)",
"Alpha Airport (ABH)",
"Alpine Airstrip (DQH)",
"Alpine - Casparis Municipal Airport (ALE)",
"Alroy Downs Airport (AYD)",
"Altai Airport (LTI)",
"Altay Airport (AAT)",
"Alpe d'Huez Airport (AHZ)",
"Alto Molocue Airport (AME)",
"Alto Palena Airport (WAP)",
"Alto Parnaiba (APY)",
"Alton Downs (AWN)",
"Altoona-Blair County Airport (AOO)",
"Altus Municipal Airport (AXS)",
"Alula Airport (ALU)",
"Alverca Airport (AVR)",
"Alxa Left Banner Airport (AXF)",
"Alxa Right Banner Airport (RHT)",
"Zintan Airport (ZIS)",
"Am Timan Airport (AMC)",
"Ama Airport (AMF)",
"Amahai Airport (AHI)",
"Amalfi Airport (AFI)",
"Amanab Airport (AMU)",
"Amarais Airport (CPQ)",
"Merzifon (MZH)",
"Amata Airport (AMT)",
"Amazon Bay (AZB)",
"Ambalabe Airport (WAI)",
"Ambanja Airport (IVA)",
"Amabatolahy Airport (AHY)",
"Ambatomainty Airport (AMY)",
"Ambatondrazaka Airport (WAM)",
"Ambilobe Airport (AMB)",
"Ambler Airport (ABL)",
"Ambohibary Airport (OHB)",
"Amborovy Airport (MJN)",
"Amboseli Airport (ASV)",
"Ambriz Airport (AZZ)",
"Ambunti Airport (AUJ)",
"Amderma Airport (AMV)",
"Amedee Army Airfield (AHC)",
"Amery Municipal Airport (AHH)",
"Ames Municipal Airport (AMW)",
"Amgu Airport (AEM)",
"Amman Civil Airport (ADJ)",
"Ammaroo Airport (AMX)",
"Ammassivik Heliport (QUW)",
"Amook Bay Seaplane Base (AOS)",
"Amos/Magny Airport (YEY)",
"Ampampamena Airport (IVA)",
"Ampanihy Airport (AMP)",
"Ampara Airport (ADP)",
"Anaa Airport (AAA)",
"Anaco Airport (AAO)",
"Anacortes Airport (OTS)",
"Anahim Lake Airport (YAA)",
"Anaktuvuk Pass Airport (AKP)",
"Analalava Airport (HVA)",
"Andakombe Airport (ADC)",
"Andamooka Airport (ADO)",
"Andapa (ZWA)",
"Andavadoaka Airport (DVD)",
"Anderson County Airport (AND)",
"Anderson Municipal Airport (AID)",
"Andi Jemma Airport (MXB)",
"Andizhan Airport (AZN)",
"Andorra la Vella Heliport (ALV)",
"Andover Airfield (ADV)",
"Andoya Andenes Airport (ANX)",
"Andravida Airport (PYR)",
"Andre Maggi Airport (OTT)",
"Andres Miguel Salazar Marcano Airport (ICC)",
"Andriamena Airport (WAD)",
"Andros Town Airport (ASD)",
"Andulo Airport (ANL)",
"Aneityum Airport (AUY)",
"Angama Mara Airport (ANA)",
"Angel Fire Airport (AXX)",
"Angelina County Airport (LFK)",
"Anggi Airport (AGD)",
"Anglesey Airport (VLY)",
"Antonio Enes (ANO)",
"Angoon Seaplane Base (AGN)",
"Angoram Airport (AGG)",
"Angouleme - Cognac International Airport (ANG)",
"Anguganak Airport (AKG)",
"Angus Downs Stn (ANZ)",
"Aniak Airport (ANI)",
"Animas Air Park (AMK)",
"Aniwa Airport (AWD)",
"Ankaizina Airport (WBE)",
"Ankang Airport (AKA)",
"Ankavandra Airport (JVA)",
"Ankazoabo Airport (WAK)",
"Ann Arbor Municipal Airport (ARB)",
"Ann Airport (VBA)",
"Annai Airport (NAI)",
"Annanberg Airport (AOB)",
"Annecy-Meythet (NCY)",
"Annemasse (QNJ)",
"Annette Island Airport (ANN)",
"Anniston Regional Airport (ANB)",
"Annobon Airport (NBN)",
"Anqing Tianzhushan Airport (AQG)",
"Ansbach-Petersdorf (QOB)",
"Anshun Huangguoshu Airport (AVA)",
"Anthony Municipal Airport (ANY)",
"Anthony Lagoon Airport (AYL)",
"Antlers Municipal Airport (ATE)",
"Antoine de Saint Exupery Airport (OES)",
"Antonio Guerreiro Airport (GMS)",
"Pasto / Antonio Narin (PSO)",
"Antonio (Nery) Juarbe Pol Airport (ARE)",
"Antonio Roldan Betancourt Airport (APO)",
"Antrim County Airport (ACB)",
"Antsalova Airport (WAQ)",
"Antsirabato Airport (ANM)",
"Antsirabe Airport (ATJ)",
"Antsoa Airport (WBO)",
"SLAF Anuradhapura (ACJ)",
"Anvik Airport (ANV)",
"Anyang Airport (AYN)",
"Aosta Airport (AOT)",
"Apalachicola Municipal Airport (AAF)",
"Apalapsili Airport (AAS)",
"Apataki Airport (APK)",
"Apolo Airport (APB)",
"Apple Valley Airport (APV)",
"April Point Seaplane Base (YQJ)",
"April River Airport (APR)",
"Ar Horqin Airport (AEQ)",
"Aracati Airport (ARX)",
"Aracati Dragao do Mar Airport (ARX)",
"Aracatuba Airport (ARU)",
"Aragarcas (ARS)",
"Aragip Airport (ARP)",
"Araguaiana Airport (0)",
"Araguaina Airport (AUX)",
"Arak Airport (AJK)",
"Aramac Airport (AXC)",
"Arandis Airport (ADI)",
"Aransas County Airport (RKP)",
"Aranuka Airport (AAK)",
"Arapahoe Municipal Airport (AHF)",
"Arapiraca Airport (APQ)",
"Arapongas Airport (APX)",
"Arar Domestic Airport (RAE)",
"Araracuara Airport (ACR)",
"Ararat Airport (ARY)",
"Arathusa Safari Lodge Airport (ASS)",
"Aratika Airport (RKA)",
"La Araucania International Airport (ZCO)",
"Arawa Airport (RAW)",
"Arba Minch Airport (AMH)",
"Arboletas (ARO)",
"Arctic Bay Airport (YAB)",
"Arctic Village Airport (ARC)",
"Ardabil Airport (ADU)",
"Ardmore Municipal Airport (ADM)",
"Ardmore Airport (AMZ)",
"Ardmore Downtown Executive Airport (AHD)",
"Arenal Airport (FON)",
"Arezzo Airfield (QZO)",
"Argyle Downs Airport (AGY)",
"Argyle Airport (GYL)",
"Argyle International Airport (SVD)",
"Aribinda Airport (XAR)",
"Puerto Arica Airport (ACM)",
"Aripuana Airport (AIR)",
"Ariquemes Airport (AQM)",
"Arkalyk North Airport (AYK)",
"Arkansas International Airport (BYH)",
"Arlit Airport (RLT)",
"Arly Airport (ARL)",
"Armando Schwarck Airport (LPJ)",
"Armstrong Airport (YYW)",
"Aropa Airport (KIE)",
"Arorae Airport (AIS)",
"Arrabury Airport (AAB)",
"Arraias Airport (AAI)",
"Arras-Roclincourt (QRV)",
"Arroyo Barril Airport (EPS)",
"Arso Airport (ARJ)",
"Arsuk Heliport (JRK)",
"Artesia Municipal Airport (ATS)",
"Arthur N Neu Airport (CIN)",
"Arthur's Town Airport (ATC)",
"Arua Airport (RUA)",
"Arugam Bay Lagoon Waterdrome (AYY)",
"Arusha Airport (ARK)",
"Arutua Airport (AXR)",
"Arvaikheer Airport (AVK)",
"Arviat Airport (YEK)",
"Yiershi Airport (YIE)",
"Asaba International Airport (ABB)",
"Asapa (APP)",
"Asau Airport (AAU)",
"Ascencion de Guarayos Airport (ASC)",
"Aseki Airport (AEK)",
"Ashburton Aerodrome (ASG)",
"Ashland Municipal Airport-Sumner Parker Field (AHM)",
"Ashley Municipal Airport (ASY)",
"Asirim (ASZ)",
"Asosa Airport (ASO)",
"Assab International Airport (ASA)",
"Assouindé Airport (ABJ)",
"Astana Nursultan Nazarbayev International Airport (NQZ)",
"Astypalaia Airport (JTY)",
"Ataq Airport (AXK)",
"Atar Airport (ATR)",
"Atauro Airport (AUT)",
"Atbara Airport (ATB)",
"Atbasar Airport (ATX)",
"Ati (ATV)",
"Atikokan Municipal Airport (YIB)",
"Atizapan-Jimenez Cantu Airport (AZP)",
"Atka Airport (AKB)",
"Atkamba Airport (ABP)",
"Atkinson Municipal Airport (PTS)",
"Atlantic Municipal Airport (AIO)",
"Atmautluak Airport (ATT)",
"Atqasuk Edward Burnell Sr. Memorial Airport (ATK)",
"Atsinanana Airport (ILK)",
"Attawapiskat Airport (YAT)",
"Attapeu International Airport (AOU)",
"Attu Heliport (QGQ)",
"Aua Island Airport (AUI)",
"Aubenas-Ardeche meridionale (OBS)",
"Augusta State Airport (AUG)",
"Augustus Downs Airport (AUD)",
"Aukland Hospital Helipad (XNZ)",
"Aumo Airport (AUV)",
"Aupaluk Airport (YPJ)",
"Aur Island Airport (AUL)",
"Aurangabad Airport (IXU)",
"Aurora Municipal Airport (AUZ)",
"Aurukun Airport (AUU)",
"Austin Airport (ASQ)",
"Austin Municipal Airport (AUM)",
"Austin Executive Airport (EDC)",
"Austral Downs Airport (AWP)",
"Auvergne Airport (AVG)",
"Auxerre-Branches (AUF)",
"Auxiliary Airfield (GBI)",
"Befandriana Airport (WBD)",
"Avare-Arandu Airport (QVP)",
"Avelino Vieira Airport (AAG)",
"Avenger Field (SWW)",
"Base de Aviacao de Taubate Airport (QHP)",
"Avignon-Caumont (AVN)",
"Avon Park Executive Airport (AVO)",
"Haimarao Airfield (AVU)",
"Awaba Airport (AWB)",
"Awang Airport (CBO)",
"Awasa Airport (AWA)",
"Axum Airport (AXU)",
"Ayacucho (AYC)",
"Ayapel Airport (AYA)",
"Ayawasi Airport (AYW)",
"Ayr Airport (AYR)",
"Azaza Airport (GSU)",
"Babanakira Airport (MBU)",
"Babo Airport (BXB)",
"Baco Airport (BCO)",
"Bacolod Silay International Airport (BCD)",
"Bade Airport (BXD)",
"Badu Island Airport (BDD)",
"Bafoussam Airport (BFX)",
"Bagan Nyaung U Airport (NYU)",
"Bagani Airport (BQI)",
"Bagdad Airport (BGT)",
"Bagdogra Airport (IXB)",
"Baglung Airport (BGL)",
"Bahawalnagar Airport (WGB)",
"Bahawalpur Airport (BHV)",
"Bahia Cupica (BHF)",
"Ilheus Jorge Amado Airport (IOS)",
"Bahia de los Angeles Airport (BHL)",
"Bahia Negra Airport (BFA)",
"Bahia Pina Airport (BFQ)",
"Bahir Dar Airport (BJR)",
"Bahja (BJQ)",
"Bahregan (IAQ)",
"Baicheng Chang'an Airport (DBC)",
"Baidoa Airport (BIB)",
"Baie-Comeau Airport (YBC)",
"Baie-Johan-Beetz Seaplane Base (YBJ)",
"Basse-Terre Baillif Airport (BBR)",
"Baimuru Airport (VMU)",
"Baindoung (BDZ)",
"Bairnsdale Airport (BSJ)",
"Baitadi Airport (BIT)",
"Bajawa Soa Airport (BJW)",
"Bajhang Airport (BJH)",
"Bajura Airport (BJU)",
"Bakalalan Airport (BKM)",
"Bakel (BXE)",
"Baker Municipal Airport (BKE)",
"Baker Lake Airport (YBK)",
"Bakkafjoerdur Airport (BJD)",
"Bakouma Airport (BMF)",
"Joint Base Balad (XQC)",
"Balakovo Airport (BWO)",
"Balcanoona Airport (LCN)",
"Baldwin County Airport (MLJ)",
"Baleela (BJE)",
"Balesin Island Airport (BSI)",
"Balgo Hill Airport (BQW)",
"Bali (BAJ)",
"Bali Airport (BLC)",
"Balimo Airport (OPU)",
"Balkanabat Airport (BKN)",
"Balkhash Airport (BXH)",
"Balalae Airport (BAS)",
"Ballera Airport (BBL)",
"Balmaceda Airport (BBA)",
"Balranald Airport (BZD)",
"Balsas Airport (BSS)",
"Baltrum (BMR)",
"Balurghat Airport (RGH)",
"Bam Airport (BXR)",
"Bamarni (BMN)",
"Bambari Airport (BBY)",
"Bambu Airport (BCP)",
"Bamburi Airport (BMQ)",
"Bamenda (BPC)",
"Bamfield Seaplane Base (YBF)",
"Bamiyan Airport (BIN)",
"Bamu (BMZ)",
"Ban Huoeisay Airport (HOE)",
"Banaina Airport (NAF)",
"Bancasi Airport (BXU)",
"Bandanaira Airport (NDA)",
"Bandar Lengeh Airport (BDH)",
"Bandeirantes Airport (0)",
"Bandirma Airport (BDM)",
"Bandon State Airport (BDY)",
"Bandundu Airport (FDU)",
"Banff Airport (YBA)",
"Banfora Airport (BNR)",
"Bangassou (BGU)",
"Bangoka International Airport (FKI)",
"Baniyala Airport (BYX)",
"Banmaw Airport (BMO)",
"Banning Municipal Airport (BNG)",
"Bannu Airfield (BNP)",
"Bantry Aerodrome (BYT)",
"Panzhihua Bao'anying Airport (PZI)",
"Baoshan Yunduan Airport (BSD)",
"Baotou Airport (BAV)",
"Bapi (BPD)",
"Bar River Airport (YEB)",
"Bar Yehuda Airfield (MTZ)",
"Baracoa Airport (MGN)",
"Barakoma Airport (VEV)",
"Baramita Airport (BMJ)",
"Warm Spring Bay Seaplane Base (BNF)",
"Barcaldine Airport (BCI)",
"Barcelonnette - Saint-Pons Airport (BAE)",
"Barcelos Airport (BAZ)",
"Bardera Airport (BSY)",
"Bardufoss Airport (BDU)",
"Bareilly Airport (BEK)",
"Barimunya Airport (BYP)",
"Barinas Airport (BNS)",
"Bario Airport (BBN)",
"Barisal Airport (BZL)",
"Kekaha, Pacific Missile Test Facility Barking Sands (BKH)",
"Barkly Downs Airport (BKP)",
"Barksdale Air Force Base (BAD)",
"Barnstable Municipal Airport-Boardman/Polando Field (HYA)",
"Barnwell Regional Airport (BNL)",
"Barra (BQQ)",
"Barra del Colorado Airport (BCL)",
"Barra do Corda Airport (BDC)",
"Tortuquero Airport (TTQ)",
"Barranca De Upia (BAC)",
"Barranco Minas Airport (NBB)",
"Barreiras Airport (BRA)",
"Barreirinhas Airport (BRB)",
"Barriles (TOQ)",
"Barrow Island Airport (BWB)",
"Barrow County Airport (WDR)",
"Barrow/Walney Island Airport (BWF)",
"Barstow Daggett County Airport (DAG)",
"Barter Island LRRS Airport (BTI)",
"Bartica A Airport (GFO)",
"Bartlesville Municipal Airport (BVO)",
"Bartlett Cove Seaplane Base (BQV)",
"Bartletts Airport (BSZ)",
"Baruun Urt Airport (UUN)",
"Basango Mboliasa Airport (KRZ)",
"Basankusu Airport (BSU)",
"Ramagundam Basantnagar Airport (RMD)",
"Basco Airport (BSO)",
"Basongo Airport (BAN)",
"Meknes (MEK)",
"Lattakia (LTK)",
"Bata Airport (BSG)",
"Batagay Airport (BQJ)",
"Batangafo Airport (BTG)",
"Batavia Downs Airport (BVW)",
"Bateen Airport (AZI)",
"Batesville Regional Airport (BVX)",
"Bathpalathang Airport (BUT)",
"Bathurst Airport (BHS)",
"Bathurst Island Airport (BRT)",
"Bathurst Airport (ZBF)",
"Batom Airport (BXM)",
"Batouri Airport (OUR)",
"Batsfjord Airport (BJF)",
"Battambang Airport (BBM)",
"Batticaloa Airport (BTC)",
"Battle Mountain Airport (BAM)",
"Batu Licin Airport (BTW)",
"Batuna (BPF)",
"Baudette International Airport (BDE)",
"La Baule-Escoublac (LBY)",
"Baures Airport (BVL)",
"Bauru-Arealva Airport (JTC)",
"Bauru Airport (BAU)",
"Bawan (BWJ)",
"Harun Thohir Airport (BXW)",
"Bay City Municipal Airport (BBC)",
"Bayankhongor Airport (BYN)",
"Bayannur Tianjitai Airport (RLK)",
"Baytown Airport (HPY)",
"Bazaruto Island Airport (BZB)",
"Bazhong Enyang Airport (BZX)",
"Beagle Bay Airport (BEE)",
"Beale Air Force Base (BAB)",
"Bear Creek 3 Airport (BCC)",
"Bearskin Lake Airport (XBE)",
"Beatty Airport (BTY)",
"Beaufort County Airport (BFT)",
"Beaumont Municipal Airport (BMT)",
"Beaune-Challanges (XBV)",
"Beauregard Regional Airport (DRI)",
"Beaver County Airport (BFP)",
"Beaver Creek Airport (YXQ)",
"Beaver Airport (WBQ)",
"Bechar (CBH)",
"Bedford Downs (BDW)",
"Bedourie Airport (BEU)",
"Bedwell Harbour Water Aerodrome (YBW)",
"Beech Factory Airport (BEC)",
"Beer-Sheva (BEV)",
"Begishevo Airport (NBC)",
"Hyderabad Old Airport (BPM)",
"Beica Airport (BEI)",
"Beihai Fucheng Airport (BHY)",
"Beihan Airport (BHN)",
"Beijing Daxing International Airport (PKX)",
"Beijing Nanyuan Airport (NAY)",
"Beira Lake Sea Plane Base (BYV)",
"Beja Airport (BYJ)",
"Bekily (OVA)",
"Belaga Airport (BLG)",
"Belgaum Airport (IXG)",
"Belaya Gora Airport (BGN)",
"Beledweyne Airport (BLW)",
"Beles (PWI)",
"Bell Island Hot Springs Seaplane Base (KBE)",
"Bella Bella (Campbell Island) Airport (ZEL)",
"Bella Coola Airport (QBC)",
"Bella Bella/Shearwater Water Aerodrome (YSX)",
"Bella Union Airport (BUV)",
"Bella Yella Airport (BYL)",
"Bellary Airport (BEP)",
"Bellburn (BXF)",
"Bellona Airport (BNY)",
"Belluno Airport (BLX)",
"Belmonte Airport (BVM)",
"Belmullet Aerodrome (BLY)",
"Belo sur Tsiribihina Airport (BMD)",
"Beloretsk Airport (BCX)",
"Beloyarskiy Airport (EYK)",
"Beluga Airport (BVU)",
"Bemichi Airport (BCG)",
"Bemolanga Airport (BZM)",
"Ben Slimane Airport (GMD)",
"Rosh Pina Airport (RPN)",
"Benalla Airport (BLN)",
"Benbecula Airport (BEB)",
"Bendigo Airport (BXG)",
"Bengbu Airport (BFU)",
"Monbaca Benguela (BUG)",
"Benguera Island Airport (BCW)",
"Beni Airport (BNC)",
"Beni Mellal Airport (BEM)",
"Benin City (BNI)",
"Benito Salas Airport (NVA)",
"Benjamin Rivera Noriega Airport (CPX)",
"Bensbach Airport (BSP)",
"Benson Municipal Airport (BBB)",
"Lahat Pagar Alam Airport (PXA)",
"Benton Field (BZF)",
"Bentota River Sea Plane Base (BJT)",
"Dolac Airport (IVG)",
"Berbera Airport (BBO)",
"Berberati Airport (BBT)",
"Berens River Airport (YBV)",
"Berezovo Airport (EZV)",
"Bergerac-Roumaniere (EGC)",
"Beringin Airport (GXA)",
"Berlevag Airport (BVG)",
"Berlin Regional Airport (BML)",
"Bermejo Airport (BJO)",
"Bermuda Dunes Airport (UDD)",
"Bert Mooney Airport (BTM)",
"Bertoua Airport (BTA)",
"Beru Airport (BEZ)",
"Besakoa Airport (BSV)",
"Besalampy Airport (BPY)",
"Besancon - La Veze Aerodrome (QBQ)",
"Bethel Airport (BET)",
"Bethel Seaplane Base (JBT)",
"Betioky Airport (BKU)",
"Baubau Betoambari Airport (BUW)",
"Betoota Airport (BTX)",
"Betou Airport (BTB)",
"Bettles Airport (BTT)",
"Beverley Springs Airport (BVZ)",
"Bewani Airport (BWP)",
"Bezmer Air Base (JAM)",
"Bhadrapur Airport (BDP)",
"Bhagatanwala Airport (BHW)",
"Bharatpur Airport (BHR)",
"Bathinda Airport (BUP)",
"Bhavnagar Airport (BHU)",
"Bhojpur Airport (BHP)",
"Bhuj Airport (BHJ)",
"Biala Podlaska (BXP)",
"Bialla Airport (BAA)",
"Biangabip Airport (BPK)",
"Biaru Airport (BRP)",
"Bicycle Lake Army Airfield (BYS)",
"Bidar Airport (IXX)",
"Bielefeld Windelsbleiche Airport (BFE)",
"Bielsko-Bialo Kaniow Airfield (QEO)",
"Big Bay Seaplane Base (YIG)",
"Big Bear City Airport (RBF)",
"Big Bell Airport (BBE)",
"Big Creek Airport (BGK)",
"Big Lake Airport (BGQ)",
"Big Mountain Air Force Station (BMX)",
"Big Spring McMahon-Wrinkle Airport (HCA)",
"Big Trout Lake Airport (YTL)",
"Biggs Army Air Field (BIF)",
"Bijie Airport (BFJ)",
"Biju Patnaik International Airport (BBI)",
"Bilaspur Airport (PAB)",
"Billiluna (BIW)",
"Billy Mitchell Airport (HNC)",
"Zugapa Airport (UGU)",
"Bimini North Seaplane Base (NSB)",
"Binaka Airport (GNS)",
"Bingoel Airport (BGG)",
"Biniguni Airport (XBN)",
"Bintulu Airport (BTU)",
"Birao (IRO)",
"Biratnagar Airport (BIR)",
"Birch Creek Airport (KBC)",
"Bird Island Airport (BDI)",
"Birdsville Airport (BVI)",
"Birjand Airport (XBJ)",
"Birsa Munda Airport (IXR)",
"Bisbee Municipal Airport (BSQ)",
"Bisbee-Douglas International Airport (DUG)",
"Bisha Airport (BHH)",
"Bishe Kola Air Base (BSM)",
"Bhisho Airport (BIY)",
"Bislig Airport (BPH)",
"Bitam Airport (BMM)",
"Bizant Airport (BZP)",
"Black Hills Airport (SPF)",
"Black Tickle Airport (YBI)",
"Blackall Airport (BKQ)",
"Blackwater Airport (BLT)",
"Blakely Island Airport (BYW)",
"Blanding Municipal Airport (BDG)",
"Blida Airport (QLD)",
"Blois-Le Breuil (XBQ)",
"Bloodvein River Airport (YDV)",
"Bloomfield Airport (BFC)",
"Blosser Municipal Airport (CNK)",
"Blubber Bay Seaplane Base (XBB)",
"Blue Canyon-Nyack Airport (BLU)",
"Blue Lagoon Seaplane Base (BXL)",
"Bluefields Airport (BEF)",
"Blytheville Municipal Airport (HKA)",
"Bo Airport (KBS)",
"Boalang Airport (BJG)",
"Boang (BOV)",
"Bob Baker Memorial Airport (IAN)",
"Bob Quinn Lake Airport (YBO)",
"Bob Sikes Airport (CEW)",
"Bodaybo Airport (ODO)",
"Bodinumu Airport (BNM)",
"Bodrum-Imsik Airport (BXN)",
"Boende Airport (BNB)",
"Bogande Airport (XBG)",
"Bogorodskoye Airport (BQG)",
"Bohol-Panglao International Airport (TAG)",
"Boigu Airport (GIC)",
"Bojnord Airport (BJB)",
"Boke (BKJ)",
"Bokondini Airport (BUI)",
"Bokoro Airport (BKR)",
"Bokpyin Airport (VBP)",
"Bol Airport (BWK)",
"Bol Berim (OTC)",
"Bole Alashankou Airport (BPL)",
"Joint Base Anacostia-Bolling Heliport (BOF)",
"Bollon Airport (BLS)",
"Bolwarra Airport (BCK)",
"Bolzano Airport (BZO)",
"Lucas do Rio Verde Airport (LVR)",
"Bom Jesus Da Lapa (LAZ)",
"Boma Airport (BOA)",
"Bomai (BMH)",
"Shelbyville Municipal Airport (SYI)",
"Bonaventure Airport (YVB)",
"Bongo (BGP)",
"Bongor Airport (OGR)",
"Bonito Airport (BYO)",
"Bonnyville Airport (YBY)",
"Bontang Airport (BXT)",
"Boolgeeda Airport (OCM)",
"Boone Municipal Airport (BNW)",
"Boone County Airport (HRO)",
"Booue Airport (BGB)",
"Boquira Airport (0)",
"Bora-Bora Airport (BOB)",
"Borama Airport (BXX)",
"Borba Airport (RBB)",
"Bordj Badji Mokhtar Airport (BMW)",
"Borgarfjoerdur Eystri Airport (BGJ)",
"Borkum (BMK)",
"Borrego Valley Airport (BXS)",
"Borroloola Airport (BOX)",
"Bender Qassim International Airport (BSA)",
"Bossangoa Airport (BSN)",
"Bosset Airport (BOT)",
"Bost Airport (BST)",
"Boswell Bay Airport (BSW)",
"Botopasi Airport (BTO)",
"Botucatu Airport (QCJ)",
"Bou Saada Airport (BUJ)",
"Bouar Airport (BOP)",
"Bouarfa Airport (UAR)",
"Bouca Airport (BCF)",
"Boufarik Airport (QFD)",
"Boulder City Municipal Airport (BLD)",
"Boulder Municipal Airport (WBU)",
"Boulia Airport (BQL)",
"Boulsa Airport (XBO)",
"Boun Neua Airport (PCQ)",
"Bouna Airport (BQO)",
"Boundary Bay Airport (YDT)",
"Boundary Airport (BYA)",
"Boundiali Airport (BXI)",
"Boundji Airport (BOE)",
"Bourges Airport (BOU)",
"Bourke Airport (BRK)",
"Bousso Airport (OUT)",
"Boutilimit (OTL)",
"Bowen Airport (ZBO)",
"Bowerman Airport (HQM)",
"Bowers Field (ELN)",
"Bowling Green-Warren County Regional Airport (BWG)",
"Bozoum Airport (BOZ)",
"Brackett Field Airport (POC)",
"Bradford Regional Airport (BFD)",
"Bradshaw Army Airfield (BSF)",
"Braga Airport (BGZ)",
"Bragado (QRF)",
"Braganca Airport (BGC)",
"Brak Airport (BCQ)",
"Brampton Island Airport (BMP)",
"Brandon Municipal Airport (YBR)",
"Brawley Municipal Airport (BWC)",
"Breiddalsvik Airport (BXV)",
"Bremerton National Airport (PWT)",
"Breves Airport (BVS)",
"Brewarrina Airport (BWQ)",
"Brewster Field (HDE)",
"Bria Airport (BIV)",
"Maranhao / Carolina Airport (CLN)",
"Divinopolis Airport (DIQ)",
"Vilhena Aeroporto (BVH)",
"Palmas-Brigadeiro Lysias Rodrigues Airport (PMW)",
"Esquel Airport (EQS)",
"Brigadier Hector Eduardo Ruiz Airport (CSZ)",
"Brigham City Airport (BMC)",
"Brighton Downs Airport (BHT)",
"Archerfield Aerodrome (ACF)",
"Britton Municipal Airport (TTO)",
"Broadus Airport (BDX)",
"Brochet Airport (YBT)",
"Brockville Regional Tackaberry Airport (XBR)",
"Broken Hill Airport (BHQ)",
"Roland-Desourdy Airport (ZBM)",
"Bronnoysund Bronnoy Airport (BNN)",
"Brookhaven Airport (WSH)",
"Brookings Municipal Airport (BKX)",
"Brookings Airport (BOK)",
"Broome International Airport (BME)",
"Browns Airport (EPG)",
"Brownwood Regional Airport (BWD)",
"Bruce Campbell Field (DXE)",
"Brunette Downs (BTD)",
"Brunswick Executive Airport (NHZ)",
"Brunswick Golden Isles Airport (BQK)",
"Lawrenceville/Brunswick Municipal Airport (LVL)",
"Brus Laguna Airport (BHG)",
"Bryant Army Heliport (FRN)",
"Bryce Canyon Airport (BCE)",
"Budardalur Airport (BQD)",
"Palopo Lagaligo Airport (LLO)",
"Bubaque Airport (BQE)",
"Buchanan Airport (UCN)",
"Bucholz Army Airfield (KWA)",
"Buckeye Municipal Airport (BXK)",
"Buckland Airport (BKC)",
"Buckley Air Force Base (BFK)",
"Buenos Aires Airport (BAI)",
"Buffalo Narrows Airport (YVT)",
"Buffalo Range Airport (BFO)",
"Samburu South Airport (UAS)",
"Bugulma Airport (UUA)",
"Buin Airport (UBI)",
"Buka Airport (BUA)",
"Bukavu Kavumu Airport (BKY)",
"Targovishte Bukhovtsi Airport (TGV)",
"Bukoba Airport (BKZ)",
"Umnugobitour (UGT)",
"Bulgan Airport, Khovd (HBU)",
"Bulgan Airport (UGA)",
"Buli Airport (PGQ)",
"Bulimba Airport (BIP)",
"Bull Harbour Seaplane Base (YBH)",
"Bullfrog Basin Airport (BFG)",
"Bulolo Airport (BUL)",
"Bultfontein Airport (UTE)",
"Bumba Airport (BMB)",
"Bumi Hills Airport (BZH)",
"Bunbury Airport (BUY)",
"Bundaberg Airport (BDB)",
"Bundi Airport (BNT)",
"Bunia Airport (BUX)",
"Bunsil (BXZ)",
"Bunyu Airport (BYQ)",
"Buochs Airport (BXO)",
"Pogogul Airport (UOL)",
"Buon Ma Thuot Airport (BMV)",
"Buraimi (RMB)",
"Burao Airport (BUO)",
"Burevestnik Airport (BVV)",
"Burg Feuerstein Airport (URD)",
"Buriti Alegre Airport (0)",
"Buritirama Airport (0)",
"Burketown Airport (BUC)",
"Burley Municipal Airport (BYI)",
"Burnie Wynyard Airport (BWT)",
"Burns Municipal Airport (BNO)",
"Burns Lake Airport (YPZ)",
"Bursa Airport (BTZ)",
"Burwash Airport (YDB)",
"Bushehr Airport (BUZ)",
"Busselton Airport (BQB)",
"Buta Zega Airport (BZU)",
"Butare Airport (BTQ)",
"Butaritari Airport (BBG)",
"Butler Memorial Airport (BUM)",
"Butler County Regional Airport (HAO)",
"Butts Army Air Field (FCS)",
"C David Campbell Field-Corsicana Municipal Airport (CRS)",
"CFB Valcartier (YOY)",
"Ca Mau Airport (CAH)",
"Caballococha Airport (LHC)",
"Cabaniss Field (NGW)",
"Cabinda Airport (CAB)",
"Cable Airport (CCB)",
"Ingeniero Jacobacci Airport (IGB)",
"Cabo Frio Airport (CFB)",
"Cabo Juan Roman Airfield (WPA)",
"Cabo Rojo Airport (CBJ)",
"Cacador Airport (CFC)",
"Caçapava do Sul Airport (0)",
"Caceres (CCX)",
"Cache Creek Airport (YZA)",
"Cachoeira do Sul Airport (QDB)",
"Cachoeiro do Itapemirim Airport (CDI)",
"Cacique Aramare Airport (PYH)",
"Cacoal Airport (OAL)",
"Tarapoto (TPP)",
"Cafunfo Airport (CFF)",
"Cagayan North International Airport (LLC)",
"Cagayan de Sulu Airport (CDY)",
"Cahors Lalbenque Airport (ZAO)",
"Caia Airport (CMZ)",
"Caicara del Orinoco Airport (CXA)",
"Caiguna Airport (CGV)",
"Cairns Army Airfield (Fort Rucker) (OZR)",
"Cairo Airport (CIR)",
"Baucau Airport (BCH)",
"Calabozo Airport (CLZ)",
"Calais-Dunkerque Airport (CQF)",
"Calbayog Airport (CYP)",
"Caleta Olivia Airport (CVI)",
"Caloundra Airport (CUD)",
"Calverton Executive Airpark (CTO)",
"Cam Ranh International Airport (CXR)",
"Camaxilo Airport (CXM)",
"Camballin Airport (MYO)",
"Cambridge Bay Airport (YCB)",
"Cambridge-Dorchester Airport (CGE)",
"Camden Airport (CDU)",
"Camfield Airport (CFI)",
"Camiguin Airport (CGM)",
"Camilo Ponce Enriquez Airport (LOH)",
"Camiri Airport (CAM)",
"Camocim Airport (CMC)",
"Camooweal Airport (CML)",
"Camp Bastion Air Base (OAZ)",
"Camp Nifty Airport (NIF)",
"Fort Campbell U. S. Army Airfield (HOP)",
"Campbell River Airport (YBL)",
"Campbell River Waterdrome (YHH)",
"Campbellpore Airport (CWP)",
"Campo do Meio Airport (0)",
"Pirassununga (QPS)",
"Campo Mourao (CBW)",
"Can Tho International Airport (VCA)",
"Canaima Airport (CAJ)",
"Canal Bajo Carlos - Hott Siebert Airport (ZOS)",
"Canandaigua Airport (IUA)",
"Cananea Airfield (CNA)",
"Canarana Airport (CQA)",
"Candala Airport (CXN)",
"Candle 2 Airport (CDL)",
"Canefield Airport (DCF)",
"Canela Airport (QCN)",
"Cangamba Airport (CNZ)",
"Cangapara Airport (FLB)",
"Cangyuan Washan Airport (CWJ)",
"Porto Alegre (QNS)",
"Canobie Airport (CBY)",
"Canouan Airport (CIW)",
"Kanton Airport (CIS)",
"Canton Municipal Airport (CTK)",
"Capanda Airport (KNP)",
"Boston Harbour Seaplane Base (BNH)",
"Cape Barren Island Airport (CBI)",
"Cape Dorset Airport (YTE)",
"Cape Flattery Airport (CQP)",
"Cape Girardeau Municipal Airport (CGI)",
"Cape Gloucester Airport (CGC)",
"Cape Lisburne LRRS Airport (LUR)",
"Cape Newenham LRRS Airport (EHM)",
"Cape Orford (CPI)",
"Cape Palmas (CPA)",
"Cape Pole Seaplane Base (CZP)",
"Cape Rodney (CPN)",
"Cape Romanzof LRRS Airport (CZF)",
"Cape Sarichef Airport (WSF)",
"Cape Vogel Airport (CVL)",
"Madinaty Capital International Airport (CCE)",
"Cobija (CIJ)",
"Guayaramerin Airport (GYA)",
"Capitan German Quiroga Guardia Airport (SRJ)",
"San Ignacio De Velasco (SNG)",
"Puerto Suarez (PSZ)",
"Riberalta (RIB)",
"Capitan Av. Vidal Villagomez Toledo Airport (VAH)",
"Capitan Jose Daniel Vazquez Airport (ULA)",
"Capitan FAP Renan Elias Olivera Airport (PIO)",
"Capitan Fuentes Martinez Airport Airport (WPR)",
"Captain Nicolas Rojas Airport (POI)",
"Capt. J. Montenegro Airport (PDM)",
"Auguste George Airport (NGD)",
"Captain Jack Thomas/El Dorado Airport (EDK)",
"Apucarana Airport (APU)",
"Captain Rogelio Castillo National Airport (CYW)",
"Capurgana Airport (CPB)",
"Caquetania (CQT)",
"Car Nicobar Air Force Base (CBD)",
"Caransebes Airport (CSB)",
"Carauari Airport (CAF)",
"Caravelas Airport (CRQ)",
"Carazinho Airport (QRE)",
"Carbon County Airport (PUC)",
"Carcassonne Airport (CCF)",
"Caribou Municipal Airport (CAR)",
"Carimagua Airport (CCO)",
"Carlisle Airport (CAX)",
"Carlos Manuel de Cespedes Airport (BYM)",
"Carlos Miguel Gimenez Airport (PIL)",
"Cruz Alta Airport (CZB)",
"Carlton Hill Airport (CRY)",
"Carmelita Airport (CMM)",
"Carmen De Patagones Airport (CPG)",
"Carnarvon Airport (CVQ)",
"Carnot Airport (CRF)",
"Carora Airport (VCR)",
"Carosue Dam Airport (WCD)",
"Carpentaria Downs Airport (CFP)",
"La Union Airport (LUI)",
"Carti Airport (CTE)",
"Cartwright Airport (YRF)",
"Caruaru Airport (CAU)",
"Caruru Airport (CUO)",
"Carutapera Airport (CTP)",
"Cascade Airport (ICS)",
"Cascade Locks-Stevenson State Airport (CZK)",
"Cascais Municipal Aerodrome (CAT)",
"Casigua El Cubo Airport (CUV)",
"Casiguran Airport (CGG)",
"Casino Airport (CSI)",
"Cassilandia (CSS)",
"Castaway Island Airstrip (CST)",
"Le Castellet (CTT)",
"Castellon de la Plana Airport (CDT)",
"West Kootenay Regional Airport (YCG)",
"Castlereigh Reservoir Waterdrome (NUF)",
"Castres-Mazamet Airport (DCM)",
"Castro Airport (QAC)",
"Casuarito Airport (CSR)",
"Cat Bi International Airport (HPH)",
"Cat Lake Airport (YAC)",
"Catacamas El Aguacate Airport (CAA)",
"Catalao Airport (TLZ)",
"Catalina Air-Sea Terminal Heliport (SPQ)",
"Catalina Airport (AVX)",
"Catarman National Airport (CRM)",
"Cattaraugus County-Olean Airport (OLE)",
"Cattle Creek Airport (CTR)",
"Catumbela Airport (CBT)",
"Cauayan Airport (CYZ)",
"Caucaya Puerto Leguizamo Airport (LQM)",
"Cauquira Airport (CDD)",
"Cavern City Air Terminal (CNM)",
"Caviahue Airport (CVH)",
"Cayana Airstrip (AAJ)",
"Caye Caulker Airport (CUK)",
"Caye Chapel Airport (CYC)",
"Cazombo Airport (CAV)",
"Cecil Airport (VQQ)",
"Cedar City Regional Airport (CDC)",
"Ceduna Airport (CED)",
"Celaque Airport (GAC)",
"Celle (ZCN)",
"Cengiz Topel Airport (KCO)",
"Centenário do Sul Airport (0)",
"Center Island Airport (CWS)",
"Centerville Municipal Airport (GHM)",
"Central Airport (CEM)",
"Central Maine Airport of Norridgewock (OWK)",
"Centralia Municipal Airport (ENL)",
"Centralia/James T. Field Memorial Aerodrome (YCE)",
"Century City Heliport (CCD)",
"Ceres Airport (0)",
"Ceres Aerodrome (CRR)",
"Cerro Largo International Airport (MLZ)",
"Cesar Lim Rodriguez Airport (RZP)",
"Cessna Aircraft Field Airport (CEA)",
"Cessnock Airport (CES)",
"Ceuta Heliport (JCU)",
"CFB Borden Heliport (YBN)",
"CFB Cold Lake (YOD)",
"CFB Gagetown (YCX)",
"CFB Greenwood (YZX)",
"CFB Trenton (YTR)",
"Nakhon Si Thammarat Airport (NST)",
"Puntarenas Chacarita Airport (JAP)",
"Chachapoyas Airport (CHH)",
"Ambato / Chachoan (ATF)",
"Chadron Municipal Airport (CDR)",
"Chafei Amsei Airport (BAT)",
"Chaiten Airport (WCH)",
"Chakcharan Airport (CCN)",
"Chalkyitsik Airport (CIK)",
"Challis Airport (CHL)",
"Chalon-Champforgeuil (XCD)",
"Chalons Vatry Airport (XCR)",
"Chamonate Airport (CPO)",
"Chan Gurney Municipal Airport (YKN)",
"Chanaral Airport (CNR)",
"Chandalar Lake Airport (WCR)",
"Chandigarh Airport (IXC)",
"Changbaishan Airport (NBS)",
"Changhai Airport (CNI)",
"Changzhi Airport (CIH)",
"Mandalay Chanmyathazi Airport (VBC)",
"Chanute Martin Johnson Airport (CNU)",
"Chaoyang Airport (CHG)",
"Chibougamau/Chapais Airport (YMT)",
"Chaparral Airport (CPL)",
"Chapleau Airport (YLD)",
"Charata Airport (CNT)",
"Charleville Airport (CTL)",
"Charleville-Mezieres (XCZ)",
"Charlevoix Airport (YML)",
"Charlo Airport (YCL)",
"Charlotte Amalie Harbor Seaplane Base (SPB)",
"Charters Towers Airport (CXT)",
"Chartres-Metropole airport (QTJ)",
"Chase Field Industrial Airport (NIR)",
"Chateau-Thierry - Belleau (XCY)",
"Chatellerault-Targe (XCX)",
"Chatham Seaplane Base (CYM)",
"Chatham-Kent Airport (XCM)",
"Chatham Island (CHT)",
"Cheadle Airport (TIL)",
"Chefornak Airport (CYF)",
"Chehalis-Centralia Airport (CLS)",
"Chelinda Airport (CEH)",
"Chena Hot Springs Airport (CEX)",
"Chenega Bay Airport (NCN)",
"Chengde Puning Airport (CDE)",
"Zhalantun Chengjisihan Airport (NZL)",
"Cheraw Municipal Airport/Lynch Bellinger Field (HCW)",
"Cherbourg-Maupertus Airport (CER)",
"Chernofski Harbor Seaplane Base (KCN)",
"Cherokee County Airport (JKV)",
"Cherrabah Airport (CRH)",
"Cherrabun Airfield (CBC)",
"Cherskiy Airport (CYX)",
"Chesterfield Inlet Airport (YCS)",
"Chetwynd Airport (YCQ)",
"Chevak Airport (VAK)",
"Chevery Airport (YHR)",
"Cheyenne Airport (CYS)",
"Chiang Rai International Airport (CEI)",
"Chiayi Airport (CYI)",
"Chichen Itza International Airport (CZA)",
"Chickasha Municipal Airport (CHK)",
"Chicken Airport (CKX)",
"Chifeng Yulong Airport (CIF)",
"Chignik Bay Seaplane Base (KBW)",
"Chignik Airport (KCG)",
"Chignik Lagoon Airport (KCL)",
"Chignik Lake Airport (KCQ)",
"Chigorodo Airport (IGO)",
"Chilas Airport (CHB)",
"Childress Municipal Airport (CDS)",
"Chile Chico Airport (CCH)",
"Tsylos Park Lodge Aerodrome (CJH)",
"Chillagoe Airport (LLG)",
"Chiloquin State Airport (CHZ)",
"Chimbu Airport (CMU)",
"Chimoio Airport (VPY)",
"Chimore Airport (CCA)",
"China Bay Airport (TRR)",
"China Straits (SQT)",
"Chinchilla Airport (CCL)",
"Chinde (INE)",
"Chingozi Airport (TET)",
"Chipata Airport (CIP)",
"Chipinge (CHJ)",
"Chiquimula Airport (CIQ)",
"Chisana Airport (CZN)",
"Chisasibi Airport (YKU)",
"Chistochina Airport (CZO)",
"Chitato Airport (PGI)",
"Chitina Airport (CXC)",
"Chitral Airport (CJL)",
"Chkalovsky Airport (CKL)",
"Choibalsan Airport (COQ)",
"Choiseul Bay Airport (CHY)",
"Chokurdakh Airport (CKH)",
"Chokwe Airport (TGS)",
"Cholet-Le Pontreau Airport (CET)",
"Chomley Seaplane Base (CIV)",
"Chongqing Wushan Airport (WSK)",
"La Chorrera Airport (LCR)",
"Chos Malal Airport (HOS)",
"Christiansted Harbor Seaplane Base (SSB)",
"Christmas Creek Airport (CXQ)",
"Chu Lai International Airport (VCL)",
"Chuathbaluk Airport (CHU)",
"Chub Cay Airport (CCZ)",
"Chuja Heliport (JCJ)",
"Chulman Airport (NER)",
"Chungribu Airport (CVB)",
"Churchill Falls Airport (ZUM)",
"Churchill Airport (YYQ)",
"Chute-Des-Passes Airport (YWQ)",
"Gastao Mesquita Airport (GGH)",
"Cibeureum Airport (TSY)",
"Cicia Airport (ICI)",
"Cigli Air Base (IGL)",
"Aydin Airport (CII)",
"Cimitarra Airport (CIM)",
"Circle Hot Springs Airport (CHP)",
"Circle City Airport (IRC)",
"Cirilo Queiroz Airport (AMJ)",
"Ciudad Acuna New International Airport (ACN)",
"Ciudad Constitucion Airport (CUA)",
"Ciudad Mante National Airport (MMC)",
"Claremont Municipal Airport (CNH)",
"Clarence A. Bain Airport (MAY)",
"Clarks Point Airport (CLP)",
"Outlaw Field (CKV)",
"Clayton Municipal Airpark (CAO)",
"Clermont Airport (CMQ)",
"Cleve Airport (CVC)",
"Cliff Hatfield Memorial Airport (CLR)",
"Clifton Hills Airstrip (CFH)",
"Clinton Regional Airport (CLK)",
"Clinton Municipal Airport (CWI)",
"Clinton-Sherman Airport (CSM)",
"Clintonville Municipal Airport (CLI)",
"Cloncurry Airport (CNJ)",
"Clorinda Airport (CLX)",
"Club Makokola Airport (CMK)",
"Cluff Lake Airport (XCL)",
"Cluny Airport (CZY)",
"Clyde River Airport (YCY)",
"Coari Airport (CIZ)",
"Coatepeque Airport (CTF)",
"Coban (CBV)",
"Cobar Airport (CAZ)",
"Cochise County Airport (CWX)",
"Cochrane Airfield (LGR)",
"Cochrane Airport (YCN)",
"Cochstedt Airport (CSO)",
"Coconut Island Airport (CNC)",
"Codazzi Airport (DZI)",
"Barbuda Codrington Airport (BBQ)",
"Coen Airport (CUQ)",
"Coffeyville Municipal Airport (CFV)",
"Coffman Cove Seaplane Base (KCC)",
"Coffs Harbour Airport (CFS)",
"Cognac-Chateaubernard Air Base (CNG)",
"Colac Airport (XCO)",
"Colatina Airport (QCH)",
"Coldfoot Airport (CXF)",
"Coleman Municipal Airport (COM)",
"Coleman A. Young Municipal Airport (DET)",
"Coles County Memorial Airport (MTO)",
"Colinas Airport (0)",
"Coll Airport (COL)",
"Collarenebri Airport (CRB)",
"College Park Airport (CGS)",
"Collie Airport (CIE)",
"Collins Bay Airport (YKC)",
"Collinsville Airport (KCE)",
"Colmar-Houssen Airport (CMR)",
"La Coloma Airport (LCL)",
"Colonel Hill Airport (CRI)",
"Lago Musters Airport (OLN)",
"Colonsay Airstrip (CSA)",
"Colorado Creek Airport (KCR)",
"Akron-Washington County Airport (AKO)",
"Columbia County Airport (HCC)",
"The Dalles Municipal Airport (DLS)",
"Columbus Air Force Base (CBM)",
"Columbus Municipal Airport (CLU)",
"Columbus-Lowndes County Airport (UBS)",
"Columbus Airport (CSG)",
"Columbus Municipal Airport (OLU)",
"Jericoacoara Airport (JJD)",
"Comandante FAP German Arias Graziani Airport (ATA)",
"Bage Aeroporto (BGX)",
"Combolcha Airport (DSE)",
"Comilla Airport (CLA)",
"Comodoro Pedro Zanni Airport (PEH)",
"Comodoro Pierrestegui Airport (COC)",
"Comox Valley Airport (YQQ)",
"Compton/Woodley Airport (CPM)",
"Con Dao Airport (VCS)",
"Conceicao do Araguaia Airport (CDJ)",
"Concepcion Airport (CEP)",
"Concord-Padgett Regional Airport (USA)",
"Concordia Airport (CCI)",
"Condobolin Airport (CBX)",
"Condron Army Airfield (WSD)",
"Confresa Airport (CFO)",
"South Andros Airport (TZN)",
"Conklin (Leismer) Airport (CFM)",
"Constanza (COZ)",
"Contadora Airport (OTD)",
"Converse County Airport (DGW)",
"Coober Pedy Airport (CPD)",
"Cooch Behar Airport (COH)",
"Cooinda Airport (CDA)",
"Cooktown Airport (CTN)",
"Coolah Airport (CLH)",
"Coolawanyah Station Airport (COY)",
"Coolibah (COB)",
"Cooma-Snowy Mountains Airport (OOM)",
"Coonabarabran Airport (COJ)",
"Coonamble Airport (CNB)",
"Coondewanna Airport (CJF)",
"Cooperstown-Westville Airport (COP)",
"Coorabie Airport (CRJ)",
"Cootamundra Airport (CMD)",
"Ruinas de Copan Airport (RUY)",
"Coposa Airport (CPP)",
"Copper Center 2 Airport (CZC)",
"Coral Harbour Airport (YZS)",
"Corazon De Jesus Airport (CZJ)",
"Corcoran Airport (CRO)",
"Cordillo Downs Airport (ODL)",
"Cordova Municipal Airport (CKU)",
"Corisco International Airport (OCS)",
"Corixá Airport (0)",
"Corn Island (RNI)",
"Cornelio Procopio Airport (CKO)",
"Cornwall Regional Airport (YCC)",
"Coromandel Aerodrome (CMV)",
"Coronation Airport (YCT)",
"Edmundo Carvajal Airport (XMS)",
"Soto Cano Air Base (XPL)",
"Chapada Diamantina Airport (LEC)",
"Corowa Airport (CWW)",
"Corozal Airport (CZH)",
"Corpus Christi, Corpus Christi Naval Air Station/Truax Field (NGP)",
"Corryong Airport (CYG)",
"Cortes Island Airport (YCF)",
"Cortez-Montezuma County Airport (CEZ)",
"Cortland County Airport-Chase Field (CTX)",
"Corvallis Municipal Airport (CVO)",
"Costa Esmeralda Airport (ECI)",
"Costa Marques Airport (CQS)",
"Coto 47 Airport (OTR)",
"Cotopaxi International Airport (LTX)",
"Cotulla-La Salle County Airport (COT)",
"Coulter Field (CFD)",
"Council Airport (CIL)",
"Courchevel Airport (CVF)",
"Courtenay Airpark (YCA)",
"Covenas Airport (CVE)",
"Cowarie Airport (CWR)",
"Cowell Airport (CCW)",
"Cowley Airport (YYM)",
"Cowra Airport (CWT)",
"Cox Field Airport (PRX)",
"Cox's Bazar Airport (CXB)",
"Coyoles (CYL)",
"Cradock (CDO)",
"Craig Seaplane Base (CGA)",
"Craig Cove Airport (CCV)",
"Craig-Moffat Airport (CIG)",
"Craig Field Airport (SEM)",
"Cram Field (BUB)",
"Cranbrook/Canadian Rockies International Airport (YXC)",
"Crane Island Airfield (CKR)",
"Crane County Airport (CCG)",
"Crateus (JCS)",
"Cravo Norte (RAV)",
"Creech Air Force Base (INS)",
"Creil (CSF)",
"Cresswell Downs Airport (CSD)",
"Buckhorn Ranch Airport (CSE)",
"Creston Municipal Airport (CSQ)",
"Creston Aerodrome (CFQ)",
"Croisette Heliport (JCA)",
"Croker Island Airport (CKI)",
"Crooked Creek Airport (CKD)",
"Cross City Airport (CTY)",
"Cross Lake (Charlie Sinclair Memorial) Airport (YCR)",
"Crossville Memorial Airport (CSV)",
"Crotone Airport (CRV)",
"Croydon Airport (CDQ)",
"Cruz Bay Seaplane Base (SJF)",
"Cuamba Airport (FXO)",
"Cuatro Vientos Airport (ECV)",
"Cudal Airport (CUG)",
"Cue Airport (CUY)",
"Cuito Cuanavale Airport (CTI)",
"Culberson County Airport (VHN)",
"Culion Airstrip (CUJ)",
"Cumana Antonio Jose de Sucre Airport (CUM)",
"La Cumbre Airport (LCM)",
"Cunnamulla Airport (CMA)",
"Cupul National Airport (TZM)",
"Curitibanos Airport (QCR)",
"Mina do Brejui (QCP)",
"Currillo (CUI)",
"Brady Curtis Field (BBD)",
"Cururupu Airport (CPU)",
"Curuzu Cuatia Airport (UZU)",
"Cushing Municipal Airport (CUH)",
"Cut Bank Municipal Airport (CTB)",
"Cutral Co Airport (CUT)",
"Cuyo Airport (CYU)",
"Rudniki (CZW)",
"Aix-Les Milles (QXB)",
"Alencon Valframbert Airport (XAN)",
"Amiens-Glisy (QAM)",
"Arcachon - La Teste-de-Buch (XAC)",
"D. Casimiro Szlapelis Airport (ARR)",
"Dabo Airport (DAO)",
"Dabo Airport (SIQ)",
"Dabolim Airport (GOI)",
"Dabra Airport (DRH)",
"Dade-Collier Training and Transition Airport (TNT)",
"Dadu Airport (DDU)",
"Dadu Airport (DDU)",
"Daet (DTE)",
"Dahl Creek Airport (DCK)",
"Dahlgren Naval Surface Warfare Center Airport (DGN)",
"Dajarra Airport (DJR)",
"Dakhla Airport (DAK)",
"Dakhla Airport (VIL)",
"Dalanzadgad Airport (DLZ)",
"Dalbandin Airport (DBA)",
"Dalby Airport (DBY)",
"Dalgaranga Gold M (DGD)",
"Dalhart Municipal Airport (DHT)",
"Dali Airport (DLU)",
"Dallas Executive Airport (RBD)",
"Dalnegorsk Airport (DHG)",
"Dalnerechensk Airport (DLR)",
"Daloa Airport (DJO)",
"Dalton Municipal Airport (DNN)",
"Daly River Airport (DVR)",
"Daly Waters Airfield (DYW)",
"Daman Airport (NMB)",
"Damazin Airport (RSS)",
"Dambula Sea Plane Base (DBU)",
"Danbury Municipal Airport (DXR)",
"Dandugama Sea Plane Base (DGM)",
"Dangriga Airport (DGA)",
"Daniel Field (DNL)",
"Dansville Municipal Airport (DSV)",
"Danville Regional Airport (DAN)",
"Daocheng Yading Airport (DCY)",
"Daporijo Airport (DEP)",
"Darchula Airport (DAP)",
"Dare County Regional Airport (MEO)",
"Dargaville Aerodrome (DGR)",
"Dargaville Hospital Heliport (DGR)",
"Darnley Island Airport (NLF)",
"Daru Airport (DAU)",
"Darwaz Airport (DAZ)",
"Conselheiro Lafaiete Airport (QDF)",
"Datadawai Airport (DTD)",
"Dathina (DAH)",
"Daugavpils International Airport (DGP)",
"Daup Airport (DAF)",
"Lt. Col W.G. (Billy) Barker VC Airport (YDN)",
"Davenport Downs Airport (DVP)",
"David C. Saudale Airport (RTI)",
"Davis Monthan Air Force Base (DMA)",
"Davison Army Airfield (DAA)",
"Dawei Airport (TVY)",
"Dawson Community Airport (GDV)",
"Dawson City Airport (YDA)",
"Dax-Seyresse (XDA)",
"Daydream Island Heliport (DDI)",
"Dayong Airport (DYG)",
"Dazu (DZU)",
"Deadhorse Airport (SCC)",
"Deadman's Cay Airport (LGI)",
"Dean River Airport (YRD)",
"Dease Lake Airport (YDL)",
"Debepare (DBP)",
"Debremarcos (DBM)",
"Debre Tabor Airport (DBT)",
"Decatur County Industrial Air Park (BGE)",
"Decatur Shores Airport (DTR)",
"Decimomannu Military Airfield (DCI)",
"Decorah Municipal Airport (DEH)",
"Dedougou Airport (DGU)",
"Deer Harbor Seaplane Base (DHB)",
"Deer Lake Regional Airport (YDF)",
"Deer Lake Airport (YVZ)",
"Deering Airport (DRG)",
"Dehong Mangshi Airport (LUM)",
"Jolly Grant Airport (DED)",
"Deir ez-Zor Airport (DEZ)",
"Deline Airport (YWJ)",
"Delingha Airport (HXD)",
"Delissaville Airport (DLV)",
"Dalma Airport (ZDY)",
"Delta County Airport (ESC)",
"Delta Downs Airport (DDN)",
"Delta Municipal Airport (DTA)",
"Delta Junction Airport (DJN)",
"Dembidollo Airport (DEM)",
"Deming Municipal Airport (DMN)",
"Deniliquin Airport (DNQ)",
"Denis Island Airport (DEI)",
"Denison Municipal Airport (DNS)",
"Depati Parbo Airport (KRC)",
"Deputatskiy Airport (DPT)",
"Deqing Moganshan Airport (DEQ)",
"Dera Ghazi Khan Airport (DEA)",
"Dera Ismail Khan Airport (DSK)",
"Derby Airport (DRB)",
"Derby Field (LOL)",
"Derim Airport (DER)",
"Desert Rock Airport (DRA)",
"Desierto de Atacama Airport (CPO)",
"La Desirade Airport (DSD)",
"Desroches Airport (DES)",
"Detroit Lakes Airport (DTL)",
"Devi Ahilyabai Holkar International Airport (IDR)",
"Devils Lake Municipal Airport (DVL)",
"Devonport Airport (DPO)",
"Dewadaru Airport (KWB)",
"Old Town / Dewitt Field (OLD)",
"Dezful Airport (DEF)",
"Dhaalu Atoll Airport (DDD)",
"Dhanbad Airport (DBD)",
"Dhangarhi Airport (DHI)",
"Dharavandhoo Airport (DRV)",
"Diamantina Airport (DTI)",
"Diamantina Lakes Airport (DYM)",
"Diamantino Airport (DMT)",
"Dianopolis Airport (DNO)",
"Diapaga Airport (DIP)",
"Diavik Airport (DVK)",
"Dibba Airport (BYB)",
"Dibrugarh Airport (DIB)",
"Dickinson Municipal Airport (DIK)",
"Diebougou Airport (XDE)",
"Naval Support Facility Diego Garcia (NKW)",
"Diego Jimenez Torres Airport (FAJ)",
"Dien Bien Phu Airport (DIN)",
"Digby/Annapolis Regional Airport (YDG)",
"Dikson Airport (DKS)",
"Dilasag Airport (DSG)",
"Dillant-Hopkins Airport (EEN)",
"Dillingham Airfield (HDH)",
"Dillon County Airport (DLL)",
"Dillon Airport (DLN)",
"Dillon's Bay Airport (DLY)",
"Dimapur Airport (DMU)",
"Dimbokro Airport (DIM)",
"Dimmit County Airport (CZT)",
"Dinangat (DNU)",
"Petersburg, Dinwiddie County Airport (PTB)",
"Diomicio Freitas/Forquilhinha Airport (CCM)",
"Dipolog Airport (DPL)",
"Diqing Shangri-La Airport (DIG)",
"Dirico Airport (DRC)",
"Dirranbandi Airport (DRN)",
"Diu Airport (DIU)",
"Dix-Sept Rosado Airport (MVF)",
"Dixie Airport (DXD)",
"Diyawanna Oya Seaplane Base (DWO)",
"Djambala Airport (DJM)",
"Djibo Airport (XDJ)",
"Djougou Airport (DJA)",
"Djoemoe Airstrip (DOE)",
"Djupivogur Airport (DJU)",
"Doany (DOA)",
"Docker River Airport (DKV)",
"Dodge County Airport (UNU)",
"Dodge City Regional Airport (DDC)",
"Dodoima Airport (DDM)",
"Dodoma Airport (DOD)",
"Doini (DOI)",
"Dolbeau-Saint-Felicien Airport (YDO)",
"Dolna Mitropoliya Air Base (PVN)",
"Dolpa Airport (DOP)",
"Domine Edward Osok Airport (SOQ)",
"Domingo Faustino Sarmiento Airport (UAQ)",
"Donaldson Center Airport (GDC)",
"Kattiniq/Donaldson Airport (YAU)",
"Dong Tac Airport (TBB)",
"Dongara Airport (DOX)",
"Fuyuan Airport (FYJ)",
"Dongola Airport (DOG)",
"Dongsha Island Airport (DSX)",
"Dongying Shengli Airport (DOY)",
"Donnelly Airport (YOE)",
"Donoi Airport (ULZ)",
"Doomadgee Airport (DMD)",
"Doogan Airport (DNG)",
"Door County Cherryland Airport (SUE)",
"Dori Airport (DOR)",
"Dornoch Airport (DOC)",
"Dorobisoro (DOO)",
"Dorunda Airport (DRD)",
"Dos Lagunas Airport (DON)",
"Dothan Regional Airport (DHN)",
"Douglas Municipal Airport (DGL)",
"Douglas Lake Airport (DGF)",
"Dourados Airport (DOU)",
"Downsview Airport (YZD)",
"Downtown Manhattan Heliport (JRB)",
"Valera Airport (VLV)",
"Dr. Arturo Umberto Illia Airport (GNR)",
"Dr Augusto Roberto Fuster International Airport (PJC)",
"Dr. Ferdinand Lumban Tobing Airport (FLZ)",
"Dr. Juan C. Angara Airport (BQA)",
"Dr Juan Plate Airport (VMI)",
"Dr. Luis Maria Argana International Airport (ESG)",
"Dracena Airport (QDC)",
"Drake Bay Airport (DRK)",
"Drake Field (FYV)",
"Temple, Draughon-Miller Central Texas Regional Airport (TPL)",
"Drayton Valley Industrial Airport (YDC)",
"Drietabbetje (DRJ)",
"Drift River Airport (DRF)",
"Drumduff Airport (DFP)",
"Drummond Island Airport (DRE)",
"Drummond Airport (DRU)",
"Dryden Regional Airport (YHD)",
"Drysdale River (DRY)",
"Dschang Airport (DSC)",
"Dubai Creek Golf Club Seaplane Base (DCG)",
"Dubois Municipal Airport (DBS)",
"Du Bois-Jefferson County Airport (DUJ)",
"Dubuque Regional Airport (DBQ)",
"Dulkaninna Airport (DLK)",
"Dumatumbun Airport (LUV)",
"Dunbar Airport (DNB)",
"Duncan/Quam Airport (DUQ)",
"Duncan Town Airport (DCT)",
"Dundee Airport (DND)",
"Dundo Airport (DUE)",
"Dunedin Airport (DUD)",
"Dunhuang Airport (DNH)",
"Dunk Island Airport (DKI)",
"Dunsmuir Municipal Airport-Mott Airport (MHS)",
"Dunwich Airport (SRR)",
"Duqm International Airport (DQM)",
"Durham Downs Airport (DHD)",
"Durrie Airport (DRR)",
"Dutch Bay Seaplane Base (DBK)",
"Duxford Airport (QFO)",
"Dwangwa Airport (DWA)",
"Dysart Airport (DYA)",
"Dywer Airbase (DWR)",
"Eagle Air Park (BZT)",
"Eagle Airport (EAA)",
"Eagle Lake Airport (ELA)",
"Eaker Field (DUA)",
"Eareckson Air Station (SYA)",
"Earl Henry Airport (BWL)",
"Earlton (Timiskaming Regional) Airport (YXR)",
"East 34th Street Heliport (TSS)",
"Eastern Sierra Regional Airport (BIH)",
"Eastern Slopes Regional Airport (FRY)",
"Eastern West Virginia Regional Airport (MRB)",
"Eastland Municipal Airport (ETN)",
"Eastmain River Airport (ZEM)",
"Easton Airport (ESN)",
"Easton State Airport (ESW)",
"Ebadon Airport (EBN)",
"Ebolowa Airport (EBW)",
"Ebon Airfield (EBO)",
"Echuca Airport (ECH)",
"Ed-Air Airport (OTN)",
"South Lewis County Airport (TDO)",
"Eday Airport (EOI)",
"CFB Edmonton (YED)",
"Edna Bay Airport (EDA)",
"Edson Airport (YET)",
"Eduardo Falla Solano Airport (SVI)",
"Yellow River (XYR)",
"Galena Airport (GAL)",
"Edwards Air Force Base (EDW)",
"Eek Airport (EEK)",
"Efogi Airport (EFG)",
"Egegik Airport (EGX)",
"Eielson Air Force Base (EIL)",
"Eil Airport (HCM)",
"Ein Yahav Airfield (EIY)",
"Einasleigh Airport (EIH)",
"Eirunepe Airport (ERN)",
"Taolai Airport (EJN)",
"Ekati Airport (YOA)",
"Ekibastuz Airport (EKB)",
"Ekuk Airport (KKU)",
"Ekwok Airport (KEK)",
"El Arish International Airport (AAC)",
"Olanchito El Arrayan Airport (OAN)",
"El Avellano Airport (FRT)",
"El Bagre Airport (EBG)",
"El Bayadh Airport (EBH)",
"El Bolson Airport (EHL)",
"El Borma Airport (EBM)",
"El Carano Airport (UIB)",
"El Centro, Naval Air Facility (NJK)",
"El Charco Airport (ECR)",
"El Daein Airport (ADV)",
"El Debba Airport (EDB)",
"Eldorado Airport (ELO)",
"El Dorado Airport (EOR)",
"El Eden Airport (AXM)",
"El Embrujo Airport (PVA)",
"El Encanto Airport (ECO)",
"El Fasher Airport (ELF)",
"El Golea Airport (ELG)",
"El Gora Airport (EGH)",
"El Kharga Airport (UVL)",
"El Lencero Airport (JAL)",
"El Loa Airport (CJC)",
"El Maiten Airport (EMX)",
"El Minya (EMY)",
"El Mirador Heliport (EMR)",
"El Mirador Airport (PUX)",
"El Naranjo Airport (ENJ)",
"El Nido Airport (ENI)",
"Alexandria / Nouzha Airport (ALY)",
"El Obeid Airport (EBD)",
"Shark El Oweinat International Airport (GSQ)",
"El Palomar Airport (EPA)",
"El Peten Airport (TKM)",
"El Porvenir Airport (PVE)",
"El Real Airfield (ELE)",
"El Recreo (ELJ)",
"El Reno Regional Airport (RQO)",
"El Tajin National Airport (PAZ)",
"El Tor (ELT)",
"El Trompillo Airport (SRZ)",
"Aeropuerto Arsenio Valderrama (ARQ)",
"El Tuqui (OVL)",
"El Yopal Airport (EYP)",
"Elcho Island Airport (ELC)",
"Elelim Airport (ELR)",
"Elenak Airport (EAL)",
"Elfin Cove Seaplane Base (ELV)",
"Elim Airport (ELI)",
"Eliptamin (EPT)",
"Eliye Springs Airport (EYS)",
"Elizabeth City Regional Airport (ECG)",
"Elizabeth Field (FID)",
"Elk City Regional Business Airport (ELK)",
"Elkedra Airport (EKD)",
"Elkhart Municipal Airport (EKI)",
"Jennings Randolph Field (EKN)",
"Elko Regional Airport (EKO)",
"Ellamar Seaplane Base (ELW)",
"Ellington Field (EFD)",
"Elliot Lake Municipal Airport (YEL)",
"Ellisras Matimba Airport (ELL)",
"Elmendorf Air Force Base (EDF)",
"Elorza Airport (EOZ)",
"Elrose Airport (ERQ)",
"Ely Municipal Airport (LYU)",
"Ely Airport-Yelland Field (ELY)",
"Emalamo Sanana Airport (SQN)",
"Embessa Airport (EMS)",
"Embraer Unidade Gaviao Peixoto Airport (0)",
"Emden Airport (EME)",
"Emerald Airport (EMD)",
"Emirau Airport (EMI)",
"Emmen Air Base (EML)",
"Emmonak Airport (EMK)",
"Emo River Airstrip (EMO)",
"Empangeni Airport (EMG)",
"Emporia Municipal Airport (EMP)",
"En Nahud Airport (NUD)",
"Enarotali Airport (EWI)",
"Encanta Moça Airport (0)",
"Lapangan Terbang Ipi (ENE)",
"Eneabba Airport (ENB)",
"Enejit Airport (EJT)",
"Engati (EGA)",
"Enid Woodring Regional Airport (WDG)",
"Enewetak Airport (ENT)",
"Enrique Adolfo Jimenez Airport (ONX)",
"Ensenada Airport (ESE)",
"Enshi Airport (ENH)",
"Enterprise Municipal Airport (ETS)",
"Entrance Island Seaplane Base (HBH)",
"Enua Airport (AIU)",
"Enyu Airfield (BII)",
"Ephrata Municipal Airport (EPH)",
"Epinal - Mirecourt Airport (EPL)",
"Eqalugaarsuit Heliport (QFG)",
"Erandique (EDQ)",
"Erave Airport (ERE)",
"Erdenet Airport (ERT)",
"Erechim Airport (ERM)",
"Erenhot Saiwusu International Airport (ERL)",
"Erigavo Airport (ERA)",
"Erldunda Airport (EDD)",
"Ernabella Airport (ERB)",
"Eros Airport (ERS)",
"Errol Airport (ERR)",
"Erume Airport (ERU)",
"Esa'ala Airport (ESA)",
"Mariscal Sucre Airport (MYC)",
"Eskilstuna Airport (EKT)",
"Eskisehir Airport (ESK)",
"Alexandria Esler Regional Airport (ESF)",
"Brava Island (BVR)",
"Esperance Airport (EPR)",
"La Esperanza (LEZ)",
"Espinosa (ESI)",
"Essen/Muelheim Airport (ESS)",
"Estevan Regional Aerodrome (YEN)",
"Estherville Municipal Airport (EST)",
"Etadunna Airport (ETD)",
"Ankara Etimesgut Air Base (ANK)",
"Eucla Airport (EUC)",
"Eugene F. Correira International Airport (OGL)",
"Eugenio Maria de Hostos Airport (MAZ)",
"Eunice Airport (UCE)",
"Eureka Airport (EUE)",
"Eureka Aerodrome (YEU)",
"Eva Downs Airport (EVD)",
"Evans Head Aerodrome (EVH)",
"Evanston-Uinta County Burns Field (EVW)",
"Eveleth-Virginia Municipal Airport (EVM)",
"San Jose Airfield (EUQ)",
"Everett-Stewart Regional Airport (UCY)",
"Evreux-Fauville Air Base (EVX)",
"Ewer Airport (EWE)",
"Ewo Airport (EWO)",
"Excursion Inlet Seaplane Base (EXI)",
"Exmouth Gulf Airport (EXM)",
"Exuma International Airport (GGT)",
"Faaite Airport (FAC)",
"Fabio Alberto Leon Bentley Airport (MVP)",
"Fada N'Gourma Airport (FNG)",
"Fagurholsmyri Airport (FAG)",
"Fahud Airport (FAU)",
"Fair Isle Airport (FIE)",
"Fairbury Municipal Airport (FBY)",
"Fairfield Municipal Airport (FFL)",
"Fairmont Municipal Airport (FRM)",
"Fairmont Hot Springs Airport (YCZ)",
"Fairview Airport (ZFW)",
"Faisalabad International Airport (LYP)",
"Fakahina Airport (FHZ)",
"Fakarava Airport (FAV)",
"Fakfak Torea Airport (FKQ)",
"Faleolo International Airport (APW)",
"Fallon Municipal Airport (FLX)",
"Naval Air Station Fallon (NFL)",
"Falls International Airport (INL)",
"False Island Seaplane Base (FAK)",
"False Pass Airport (KFP)",
"False River Regional Airport (HZR)",
"Fane Airport (FNE)",
"Fangatau Airport (FGU)",
"Farafangana Airport (RVA)",
"Farah Airport (FAH)",
"Faranah Airport (FAA)",
"Farewell Airport (FWL)",
"Faribault Municipal Airport (FBL)",
"Farmington Regional Airport (FAM)",
"Faro Airport (ZFA)",
"Fasa Airport (FAZ)",
"Faskrudsfjoerdur Airport (FAS)",
"Faya-Largeau Airport (FYT)",
"Fayetteville Municipal Airport (FYM)",
"Fayzabad Airport (FBD)",
"Fazenda Cataco Airport (0)",
"Fazenda Constância Airport (0)",
"Fazenda das Represas Airport (0)",
"Fazenda Dona Olga Airport (CSW)",
"Porto Alegre (PBX)",
"Fazenda Santa Maria Airport (0)",
"Fazenda São Francisco Airport (0)",
"Fazenda São João Airport (0)",
"Fazenda Três Barras Airport (0)",
"Santa Fe Do Sul (SFV)",
"Fazenda Tucunare (AZL)",
"Fazenda Vassoural Airport (0)",
"Fderik Airport (FGD)",
"Feijó Airport (FEJ)",
"Santa Lucia Air Force Base Num 1 (NLU)",
"Felker Army Airfield (FAF)",
"Fera/Maringe Airport (FRE)",
"Feramin Airport (FRQ)",
"Fergana International Airport (FEG)",
"Fergus Falls Municipal/Einar Mickelson Field (FFM)",
"Ferkessedougou Airport (FEK)",
"Fernando de Noronha Airport (FEN)",
"Fetlar Airport (FEA)",
"Fianarantsoa Airport (WFI)",
"Ficksburg Airport (FCB)",
"Figari - Sud-Corse (FSC)",
"Filadelfia Airport (FLM)",
"Fillmore Municipal Airport (FIL)",
"Fincha Airport (FNH)",
"Findlay Airport (FDY)",
"Finke Airport (FIK)",
"Finley Airport (FLY)",
"Finschhafen Airport (FIN)",
"First Flight Airport (FFA)",
"Fitiuta Airport (FTI)",
"Fitzroy Crossing Airport (FIZ)",
"Flabob Airport (RIR)",
"Flamingo Airport (FMG)",
"Flat Airport (FLT)",
"Fleetlands Heliport (PME)",
"Flensburg-Schaeferhaus Airport (FLF)",
"Fletcher Field (CKM)",
"Flin Flon Airport (YFO)",
"Flinders Island Airport (FLS)",
"Flora Valley (FVL)",
"Florence Municipal Airport (FMU)",
"La Florida Airport (TCO)",
"Floro Airport (FRO)",
"Flotta Airport (FLH)",
"Floyd Bennett Memorial Airport (GFL)",
"Ruegen Airport (GTI)",
"Kitzingen Airport (KZG)",
"Neustadt-Glewe Airfield (QGW)",
"Flying Cloud Airport (FCM)",
"Fond-Du-Lac Airport (ZFD)",
"Fontanges Airport (YFG)",
"Fonte Boa Airport (FBA)",
"Foothills Regional Airport (MRN)",
"Forbes Airport (FRB)",
"Ford Airport (IMT)",
"Forde Airport, Bringeland (FDE)",
"Bringeland Airport (FDE)",
"Forest City Municipal Airport (FXY)",
"Forestville Airport (YFE)",
"Formiga Airport (0)",
"Forrest City Municipal Airport (FCY)",
"Forrest Airport (FOS)",
"Forssa Airport (QVE)",
"Forster (Wallis Is) Airport (FOT)",
"Fort Albany Airport (YFA)",
"Fort Bragg Airport (FOB)",
"Fort Bridger Airport (FBR)",
"Fort Chipewyan Airport (YPY)",
"Fort Dodge Municipal Airport (FOD)",
"Fort Frances Municipal Airport (YAG)",
"Fort Good Hope Airport (YGH)",
"Fort Hope Airport (YFH)",
"Fort Liard Airport (YJF)",
"Fort MacKay/Albian Aerodrome (JHL)",
"Fort MacKay/Firebag Aerodrome (YFI)",
"Fort MacKay/Horizon Airport (HZP)",
"Fort Madison Municipal Airport (FMS)",
"Fort McMurray/Mildred Lake Airport (NML)",
"Fort Mcpherson Airport (ZFM)",
"Northern Rockies Regional Airport (YYE)",
"Fort Reliance Seaplance Base (YFL)",
"Fort Resolution Airport (YFR)",
"Fort Scott Municipal Airport (FSK)",
"Fort Severn Airport (YER)",
"Fort Simpson Airport (YFS)",
"Fort Smith Regional Airport (FSM)",
"Fort Smith Airport (YSM)",
"Fort Stockton, Fort Stockton-Pecos County Airport (FST)",
"Fort Sumner Municipal Airport (FSU)",
"Fort Yukon Airport (FYU)",
"Fortescue Dave Forrest Airport (KFE)",
"Foshan Shadi Airport (FUO)",
"Fossil Downs Stn (FSL)",
"Fougamou Airport (FOU)",
"Foula Airport (FOA)",
"Koundja (FOM)",
"Fox Glacier Heliport (FGL)",
"Foya (FOY)",
"The Francis S Gabreski Airport (FOK)",
"Francisco B. Reyes Airport (USU)",
"Paulo Abdala Airport (FBE)",
"Francisco Carle Airport (JAU)",
"Francisco De Orellana Airport (OCC)",
"Francisco Primo de Verdad National Airport (LOM)",
"Itumbiara Airport (ITR)",
"Cerro Sombrero (SMB)",
"Marilia Airport (MII)",
"Franklin County Airport (UOS)",
"Franklin Municipal-John Beverly Rose Airport (FKN)",
"Frans Sales Lega Airport (RTG)",
"Franz Josef Aerodrome (WHO)",
"Frederick Municipal Airport (FDK)",
"Frederick Municipal Airport (FDR)",
"Freeman Municipal Airport (SER)",
"Fregate Island Airport (FRK)",
"Fremont County Airport (CNE)",
"Fremont Municipal Airport (FET)",
"French Lick Municipal Airport (FRH)",
"Fresno-Chandler Downtown Airport (FCH)",
"Fria Airport (FIG)",
"La Fria Airport (LFR)",
"Friday Harbor Seaplane Base (FBS)",
"Frieda River Airport (FAQ)",
"Friedman Memorial Airport (SUN)",
"Fritzlar (FRZ)",
"Frutillar Airport (FRT)",
"Fuerte Olimpo Airport (OLK)",
"Fukue Airport (FUJ)",
"Fukui Airport (FKJ)",
"Fukushima Airport (FKS)",
"Fulleborn Airport (FUB)",
"Fullerton Municipal Airport (FUL)",
"Fulton County Airport-Brown Field (FTY)",
"Fuma (FUM)",
"Funadhoo Airport (FND)",
"Fundación Airport (FDA)",
"Funter Bay Seaplane Base (FNR)",
"Furnace Creek Airport (DTH)",
"Futaleufu Airport (FFU)",
"Futuna Airport (FTA)",
"Fuvahmulaku Island Airport (FVM)",
"Fuyang Xiguan Airport (FUG)",
"Fuyun Koktokay Airport (FYN)",
"Gabala International Airport (GBB)",
"Gabbs Airport (GAB)",
"Gabès Matmata International Airport (GAE)",
"Gachsaran Airport (GCH)",
"Gag Island Airport (GAV)",
"Gage Airport (GAG)",
"Gagnoa Airport (GGN)",
"Gahcho Kue Aerodrome (GHK)",
"Gainesville Municipal Airport (GLE)",
"Galbraith Lake Airport (GBH)",
"Galcaio Airport (GLK)",
"Galegu Airport (DNX)",
"Galesburg Municipal Airport (GBG)",
"Gaellivare Airport (GEV)",
"Gallup Municipal Airport (GUP)",
"Gamal Abdel Nasser Airport (TOB)",
"Galela Gamarmalamu Airport (GLX)",
"Gamba Airport (GAX)",
"Gambell Airport (GAM)",
"Gambella Airport (GMB)",
"Gamboa Airport (WCA)",
"Gamboma Airport (GMM)",
"Gamboola Airport (GBP)",
"Gandajika Airport (GDJ)",
"Gangaw Airport (GAW)",
"Ganges Seaplane Base (YGG)",
"Kangnung Ab (KAG)",
"Gannan Xiahe Airport (GXH)",
"Gansner Field (GNF)",
"Ganzhou Airport (KOW)",
"Gaoua Airport (XGA)",
"Gap-Tallard Airport (GAT)",
"Garachine Airport (GHE)",
"Garaina Airport (GAR)",
"Garanhuns Airport (QGP)",
"Garasa (GRL)",
"Garbahaareey Airport (GBM)",
"Garça Airport (0)",
"Gardabya Airport (SRX)",
"Garden County Airport (OKS)",
"Garden Point Airport (GPN)",
"Gardez Old Airfield (GRG)",
"Gardner Municipal Airport (GDM)",
"Qardho Airport (GSR)",
"Garfield County Regional Airport (RIL)",
"Garissa Airport (GAS)",
"Garner Field (UVA)",
"Garowe Airport (GGR)",
"Garze Gesar Airport (GZG)",
"Gascoyne Junction Airport (GSC)",
"Gasmata Island Airport (GMI)",
"Michel-Pouliot Gaspe Airport (YGP)",
"Gasuke (GBC)",
"Gatlinburg-Pigeon Forge Airport (GKT)",
"Gatokae Airport (GTA)",
"Gatot Subroto Airport (WYK)",
"Gaua Island Airport (ZGU)",
"Bhairahawa (BWA)",
"Las Gaviotas Airport (LGT)",
"Gavle-Sandviken Airport (GVX)",
"Gaylord Regional Airport (GLR)",
"Gayndah Airport (GAH)",
"Gaziemir (IGL)",
"Gbadolite Airport (BDT)",
"Gbangbatok Airport (GBK)",
"Gebe Airport (GEB)",
"Geelong Airport (GEX)",
"Dagali Geilo Airport (DLD)",
"Geladi Severnyy (GLC)",
"Gelephu Airport (GLU)",
"Gemena Airport (GMA)",
"Geneina Airport (EGN)",
"General Bartolome Salom International Airport (PBL)",
"Tartagal Aerodrome (TTG)",
"General Jose Francisco Bermudez Airport (CUP)",
"Curico (ZCQ)",
"General Leite de Castro Airport (RVD)",
"General Manuel Serrano Airport (MCH)",
"General Mariano Matamoros Airport (CVJ)",
"General Rivadeneira Airport (ESM)",
"General Ulpiano Paez Airport (SNC)",
"Isabela Island (IBB)",
"General Villegas Airport (VGS)",
"George Downer Airport (AIV)",
"George Airport (GRJ)",
"George R Carr Memorial Air Field (BXA)",
"Georgetown Airport (GTT)",
"George Town Aerodrome (GEE)",
"Geraldton Airport (GET)",
"Geraldton (Greenstone Regional) Airport (YGQ)",
"Buenaventura (BUN)",
"Captain German Olano Moreno Air Base (PAL)",
"Puerto Carreno / A. Guauquea (PCR)",
"Geva Airport (GEF)",
"Larantuka Airport (LKA)",
"Gewoia Airport (GEW)",
"Ghadames Airport (LTD)",
"Ghanzi Airport (GNZ)",
"Ghat Airport (GHT)",
"Ghazni Airport (GZI)",
"Ghinnir Airport (GNN)",
"Ghriss Airport (MUW)",
"Gibb River Airport (GBV)",
"Giebelstadt (GHF)",
"Gifu Ab (QGU)",
"Gilgal Airport (GGL)",
"Gilgit Airport (GIL)",
"Gillam Airport (YGX)",
"Gilze-Rijen Air Base (GLZ)",
"Gimli Industrial Park Airport (YGM)",
"Ginbata Airport (GBW)",
"Girdwood Airport (AQY)",
"Girua Airport (PNP)",
"Gisborne Airport (GIS)",
"Gisenyi Airport (GYI)",
"Gitega Airport (GID)",
"Giyani Airport (GIY)",
"Gjoa Haven Airport (YHK)",
"Gjoegur Airport (GJR)",
"Glacier Creek Airport (KGZ)",
"Gladstone Airport (GLT)",
"Gladwin Zettel Memorial Airport (GDW)",
"Glasgow Municipal Airport (GLW)",
"Glen Innes Airport (GLI)",
"Glendale Fokker Field Airport (GWV)",
"Glengyle Airport (GLG)",
"Glenormiston Airport (GLM)",
"Glentanner Airport (GTN)",
"Gobabis (GOG)",
"Gobernador Edgardo Castello Airport (VDM)",
"Gobernador Gregores Airport (GGS)",
"Gode Airport (GDE)",
"Godman Army Airfield (FTK)",
"Godofredo P. Ramos Airport (MPH)",
"Gods Lake Narrows Airport (YGO)",
"Gods River Airport (ZGI)",
"Gogebic-Iron County Airport (IWD)",
"Goekceada Airport (GKD)",
"Gol Airport (GLL)",
"Gold Beach Municipal Airport (GOL)",
"Golden Horn Lodge Seaplane Base (GDH)",
"Golfito Airport (GLF)",
"Golfo de Morrosquillo Airport (TLU)",
"Golmud Airport (GOQ)",
"Golog Maqin Airport (GMQ)",
"Golovin Airport (GLV)",
"Gombe Lawanti International Airport (GMO)",
"La Gomera Airport (GMZ)",
"Gomez Nino (API)",
"Gonaili Airport (GOE)",
"Gondar Airport (GDQ)",
"Gonzalo Mejia Airport (TRB)",
"Gooding Municipal Airport (GNG)",
"Goodnews Airport (GNU)",
"Goondiwindi Airport (GOO)",
"Goose Bay Airport (YYR)",
"Gora Airstrip (GOC)",
"Gorakhpur Airport (GOP)",
"Gordil Airport (GDI)",
"Gordon Downs Airport (GDD)",
"Gordon Municipal Airport (GRN)",
"Gordonsville Municipal Airport (GVE)",
"Gore Bay-Manitoulin Airport (YZE)",
"Gore Airport (GOR)",
"Forudgah-e Gorgan (GBT)",
"Gorge Harbour Seaplane Base (YGE)",
"Gorna Orechovista (GOZ)",
"Goroka Airport (GKA)",
"Gorom-Gorom Airport (XGG)",
"Gosford Hospital Helipad (GOS)",
"Goulburn Airport (GUL)",
"Goundam Airport (GUD)",
"Gove Airport (GOV)",
"Goya Airport (OYA)",
"Graciosa Airport (GRW)",
"Graeme Rowley Airport (CKW)",
"Grafton Airport (GFN)",
"Aerodromo Bernardo O'Higgins (YAI)",
"Grand Canyon Bar 10 Airport (GCT)",
"Grand Canyon National Park Airport (GCN)",
"Grand Canyon West Airport (GCW)",
"Grand Cess (GRC)",
"Grand Coulee Dam Airport (GCD)",
"Grand Forks Airport (ZGF)",
"Grand Geneva Resort Airport (XES)",
"Grand Lake Regional Airport (NRI)",
"Grand Marais-Cook County Airport (GRM)",
"Grand Rapids/Itasca County Airport-Gordon Newstrom Field (GPZ)",
"Grand-Santi Airport (GSI)",
"La Grande Riviere Airport (YGL)",
"La Grande/Union County Airport (LGD)",
"La Grande-4 Airport (YAH)",
"La Grande-3 Airport (YAR)",
"Granite Mountain Air Station (GMT)",
"The Granites Airport (GTS)",
"Grant County Airport (PGC)",
"Grant County Airport (SVC)",
"Grant County Regional Airport (JDA)",
"Grants-Milan Municipal Airport (GNT)",
"Grantsburg Municipal Airport (GTG)",
"Granville - Mont-Saint-Michel (GFR)",
"Gratiot Community Airport (AMN)",
"Gravatai (GCV)",
"Gray Army Air Field (GRF)",
"Grayling Airport (KGX)",
"Claris Aerodrome (GBZ)",
"Great Bear Lake Airport (DAS)",
"Great Bend Municipal Airport (GBD)",
"Great Harbour Cay Airport (GHC)",
"Great Keppel Isla (GKL)",
"Greater Cumberland Regional Airport (CBE)",
"Greater Kankakee Airport (IKK)",
"Greeley-Weld County Airport (GXY)",
"Green River Airport (GVI)",
"Green River Municipal Airport (RVR)",
"Greene County Airport (WAY)",
"Greeneville-Greene County Municipal Airport (GCY)",
"Greenlee County Airport (CFT)",
"Greenvale Airport (GVP)",
"Greenville Airport (GRE)",
"Greenville/Sinoe Airport (SNI)",
"Greenway Sound Seaplane Base (YGN)",
"Greenwood County Airport (GRD)",
"Greenwood-Leflore Airport (GWO)",
"Gregory Downs Airport (GGD)",
"Grenchen (ZHI)",
"Grenfell Airport (GFE)",
"Greymouth Airport (GMN)",
"Griffiss International Airport (RME)",
"Grise Fiord Airport (YGZ)",
"Grissom Air Force Base / Peru (GUS)",
"Kangilinnguit Heliport (JGR)",
"Groote Eylandt Airport (GTE)",
"Grootfontein Airport (GFY)",
"Grosseto Airport (GRS)",
"Grundarfjoerdur Airport (GUU)",
"Mohe Gulian Airport (OHE)",
"Guacamaya (GCA)",
"Guadalupe Airport (GDP)",
"Guaira Airport (GGJ)",
"La Guaira Airport (LAG)",
"Guajara-Mirim Airport (GJM)",
"Gualeguaychu Aerodrome (GHU)",
"Nicoya Airport (NCT)",
"Guanambi Airport (GNM)",
"Guanare Airport (GUQ)",
"Guanghan Airport (GHN)",
"Guangyuan Panlong Airport (GYS)",
"Guangzhou MR Air Base (LHK)",
"Guapiles Airport (GPL)",
"Guaporé Airport (0)",
"Guarapari Airport (GUZ)",
"Guaratingueta (GUJ)",
"Guardiamarina Zanartu Airport (WPU)",
"Guari (GUG)",
"Guasdualito Airport (GDO)",
"Guasopa (GAZ)",
"Goulimime Airport (GLN)",
"Guemar Airport (ELU)",
"Guerrero Negro Airport (GUB)",
"Guiglo Airport (GGO)",
"Guillermo Leon Valencia Airport (PPN)",
"Guiria (GUI)",
"Gujrat Airport (GRT)",
"Gulgubip (GLP)",
"Gulkana Airport (GKN)",
"Gulu Airport (ULU)",
"Guna Airport (GUX)",
"Gunnedah Airport (GUH)",
"Gunung Batin Airport (AKQ)",
"Gurayat Domestic Airport (URY)",
"Guriaso (Keraso) Airport (GUE)",
"Gurney Airport (GUR)",
"Vila Junqueiro (VJQ)",
"Gurupi Airport (GRP)",
"Gusap Airport (GAP)",
"Gusau (QUS)",
"Gustavo Artunduaga Paredes Airport (FLA)",
"Gustavo Rizo Airport (BCA)",
"Aeropuerto Gabriel Vargas Santos (TME)",
"Gustavus Airport (GST)",
"Gusti Syamsir Alam Airport (KBU)",
"Guthrie-Edmond Regional Airport (GOK)",
"Guymon Municipal Airport (GUY)",
"Gwa Airport (GWA)",
"Gwalior Airport (GWL)",
"Gwaunaru'u Airport (AKS)",
"Lawrenceville, Gwinnett County-Briscoe Field Airport (LZU)",
"Gympie Airport (GYP)",
"H. H. Coffield Regional Airport (RCK)",
"H.A.S. Hanandjoeddin Airport (TJQ)",
"Ha'il Regional Airport (HAS)",
"Habi Airport (HBD)",
"Hacaritama Airport (HAY)",
"Haelogo Airport (HEO)",
"Habei Qilian Airport (HBQ)",
"Haifa International Airport (HFA)",
"Haines Airport (HNS)",
"Haines Junction Airport (YHT)",
"Hivaro (HIT)",
"Yueksekova Airport (YKO)",
"Halali Airport (HAL)",
"Hale County Airport (PVW)",
"Shearwater Airport (YAW)",
"Halifax (Queen Elizabeth II Health Science Centre) Heliport (XDG)",
"Haliwen Airport (ABU)",
"Hall Beach Airport (YUX)",
"Halli Airport (KEV)",
"Halliburton Field (DUC)",
"Halls Creek Airport (HCQ)",
"Hamadan Airport (HDM)",
"Hambantota Sea Plane Base (HBT)",
"Hami Airport (HMI)",
"Hamilton Airport (HLT)",
"Hammerfest Airport (HFT)",
"Hampton Municipal Airport (HPT)",
"Hana Airport (HNM)",
"Fort Rucker, Hanchey Army Heliport (HEY)",
"Hancock County-Bar Harbor Airport (BHB)",
"Hanksville Airport (HVE)",
"Hanzhong Chenggu Airport (HZG)",
"Hao Airport (HOI)",
"Long Island Heliport (HAP)",
"Harar Meda Airport (QHR)",
"Hardy-Anders Field Natchez-Adams County Airport (HEZ)",
"Harrell Field (CDH)",
"Harris County Airport (PIM)",
"Harrisburg-Raleigh Airport (HSB)",
"Harrismith Airport (HRS)",
"Harry P Williams Memorial Airport (PTN)",
"Breckenridge-Wahpeton Interstate Airport (WAH)",
"Hartford-Brainard Airport (HFD)",
"Hartley Bay Water Aerodrome (YTB)",
"Hartness State Airport (Springfield) (VSF)",
"Hartsville Regional Airport (HVS)",
"Hassfurt Airfield (XXX)",
"Hassan I Airport (EUN)",
"Hassi R'Mel Airport (HRM)",
"Hastings Airport (HGS)",
"Hasvik Airport (HAA)",
"Hatchet Lake Airport (YDJ)",
"Hateruma Airport (HTR)",
"Hato Corozal Airport (HTZ)",
"Hattiesburg Municipal Airport (HBG)",
"Hattiesburg-Laurel Regional Airport (PIB)",
"Hatzfeldthaven (HAZ)",
"Haugesund Airport (HAU)",
"Havadarya Airport (HDR)",
"Haverfordwest Airport (HAW)",
"Havre City County Airport (HVR)",
"Le Havre - Octeville Airport (LEH)",
"Havre Saint-Pierre Airport (YGV)",
"Hawabango (HWA)",
"Hawarden Airport (CEG)",
"Hawkins Field (HKS)",
"Hawthorne Municipal Airport (HTH)",
"Hay Airport (HXX)",
"Hay River/Merlyn Carter Airport (YHY)",
"Haycock Airport (HAY)",
"Hayfields Airport (HYF)",
"Hayman Island Resort Seaplane Base (HIS)",
"Hays Municipal Airport (HYS)",
"Hazleton Regional Airport (HZL)",
"Turkistan International Airport (HSA)",
"Hazyview Airport (HZV)",
"Headingly Airport (HIP)",
"Healy Lake Airport (HKB)",
"Healy River Airport (HKB)",
"Hearst (Rene Fontaine) Municipal Airport (YHF)",
"Heathlands Airport (HAT)",
"Hechi Jinchengjiang Airport (HCJ)",
"Belmopan Hector Silva Airport (BCV)",
"Hefei Luogang International Airport (HFE)",
"Heglig Airport (HEG)",
"Heho Airport (HEH)",
"Heihe Airport (HEK)",
"Heiweni (HNI)",
"Helenvale Airport (HLV)",
"Helmuth Baumgartem Airport (LOI)",
"Hemavan Airport (HMV)",
"Henbury Airport (HRY)",
"Henderson Executive Airport (HSH)",
"Henderson Field Civil Airport (MDY)",
"Hendrik Van Eck Airport (PHW)",
"Hengyang Bajialing Airport (HNY)",
"Henry Post Army Airfield (FSI)",
"Henry Tift Myers Airport (TMA)",
"Herat International Airport (HEA)",
"Herendeen Bay Airport (HED)",
"Farfan Airport (ULQ)",
"Heritage Field (PTW)",
"Hermannsburg Airport (HMG)",
"Hermanos Ameijeiras Airport (VTU)",
"Hermiston Municipal Airport (HES)",
"Hernesaari Heliport (HEN)",
"Tandil Aerodrome (TDL)",
"La Herrera (HRR)",
"Hesa Airport (IFH)",
"Hesler-Noble Field (LUL)",
"Hickam Field (HIK)",
"High Level Airport (YOJ)",
"High Prairie Airport (ZHP)",
"Highbury Airport (HIG)",
"Higuerote Airport (HGE)",
"Hikueru Atoll Airport (HHZ)",
"Hill City Municipal Airport (HLC)",
"Hillenbrand Industries Airport (HLB)",
"Hillside Airport (HLL)",
"Hingurakgoda Minneriya Airport (HIM)",
"Hinthada Airport (HEB)",
"Hinton/Jasper-Hinton Airport (YJP)",
"Hiroshimanishi (HIW)",
"Hisar Airport (HSS)",
"Atuona Airport (AUQ)",
"Hjaltabakki Airport (BLO)",
"Hluhluwe Airport (HLW)",
"Hobart Regional Airport (HBR)",
"Obbia Airport (CMO)",
"Hodeidah Airport (HOD)",
"AFB Hoedspruit (HDS)",
"Hoedspruit Airport (HDS)",
"Hog River Airport (HGZ)",
"Hohenems-Dornbirn Airport (HOH)",
"Hokitika Aerodrome (HKK)",
"Hola Airport (HOA)",
"Holbrook Municipal Airport (HBK)",
"Huolinhe Airport (HUO)",
"Hollis Clark Bay Seaplane Base (HYL)",
"Hollister Municipal Airport (HLI)",
"Holmavik Airport (HVK)",
"Holt Airport (FLI)",
"Holy Cross Airport (HCR)",
"Homer Airport (HOM)",
"Homalin (HOX)",
"Hon Airport (HUQ)",
"Hongyuan Airport (AHJ)",
"Honinabi Airport (HNN)",
"Fort Hood, Hood AAF Ft Hood (HLR)",
"Hood Aerodrome (MRO)",
"Hooker County Airport (MHN)",
"Hooker Creek Airport (HOK)",
"Hoonah Airport (HNH)",
"Hooper Bay Airport (HPB)",
"Hope Bay Airport (UZM)",
"Hope Vale Airport (HPE)",
"Hope Aerodrome (YHE)",
"Hopedale Airport (YHO)",
"Hopetoun Airport (HTU)",
"Horn Island Airport (HID)",
"Hornafjoerdur Airport (HFN)",
"Hornepayne Municipal Airport (YHN)",
"Horsham Airport (HSM)",
"Horta Airport (HOR)",
"Hot Springs County Airport (THP)",
"Hotan Airport (HTN)",
"Una-Comandatuba Airport (UNA)",
"Houghton County Memorial Airport (CMX)",
"Houlton International Airport (HUL)",
"Houma-Terrebonne Airport (HUM)",
"Hpa-An Airport (PAA)",
"Hpapun Airport (PPU)",
"Hrodna Airport (GNA)",
"Hsinchu Air Base (HSZ)",
"Hua Hin Airport (HHQ)",
"Huacaraje Airport (BVK)",
"Huahine - Fare Airport (HUH)",
"Huallaga Airport (BLP)",
"Huatugou Airport (HTT)",
"Shihezi Huayuan Airport (SHF)",
"Hubli Airport (HBX)",
"Hudson Bay Airport (YHB)",
"Hudson's Hope Airport (YNH)",
"Huehuetenango (HUG)",
"Hughenden Airport (HGD)",
"Hughes Airport (HUS)",
"Huizhou Pingtan Airport (HUZ)",
"Hukuntsi Airport (HUK)",
"Hultsfred Airport (HLF)",
"Hulunbuir Hailar Airport (HLD)",
"Humacao Airport (HUC)",
"Francisco Correa da Cruz Airport (HUW)",
"Humbert River Airport (HUB)",
"Humboldt Airport (HUD)",
"Humera Airport (HUE)",
"Hunt Field (LND)",
"Hunter U. S. Army Airfield (SVN)",
"Huntingburg Airport (HNB)",
"Huntsville Municipal Airport (HTV)",
"Huron Regional Airport (HON)",
"Huslia Airport (HSL)",
"Husum-Schwesing Airport (QHU)",
"Hutchinson County Airport (BGD)",
"Hutchinson Municipal Airport (HUT)",
"Hwange National Park Airport (HWN)",
"Hwange Town Airport (WKI)",
"Hydaburg Seaplane Base (HYG)",
"Hyder Seaplane Base (WHD)",
"Hyvinkaa Airport (HYV)",
"Iaciara Airport (0)",
"Ialibu Airport (IAL)",
"Iamalele (IMA)",
"Iberia Airport (IBP)",
"Ibo (IBO)",
"Iboki Airport (IBI)",
"Icabaru Airport (ICA)",
"Iconi Airport (YVA)",
"Icy Bay Airport (ICY)",
"Ida Grove Municipal Airport (IDG)",
"Idaho County Airport (IDH)",
"Idiofa Airport (IDF)",
"Idre Airport (IDB)",
"Iejima Airport (IEJ)",
"Taif Regional Airport (TIF)",
"Iffley Airport (IFF)",
"Ifuru Airport (IFU)",
"Igaliku Heliport (QFX)",
"Igarka Airport (IAA)",
"Iginniarfik Heliport (QFI)",
"Igiugig Airport (IGG)",
"Igloolik Airport (YGT)",
"Ignace Municipal Airport (ZUC)",
"Igor I Sikorsky Memorial Airport (BDR)",
"Igrim Airport (IRM)",
"Iguatu (QIG)",
"Ihosy Airport (IHO)",
"Ihu Airport (IHU)",
"Ijui Airport (IJU)",
"Ikamiut Heliport (QJI)",
"Ikela Airport (IKL)",
"Ikerasak Heliport (IKE)",
"Ikerasaarsuk Heliport (QRY)",
"Iki Airport (IKI)",
"Ilam (IIL)",
"Ile Art - Waala Airport (BMY)",
"Ile d'Yeu (IDY)",
"Ile des Pins Airport (ILP)",
"Ilebo (PFR)",
"Iles-de-la-Madeleine Airport (YGR)",
"Ilford Airport (ILF)",
"Iliamna Airport (ILI)",
"Ilimanaq Heliport (XIQ)",
"Illaga Airport (ILA)",
"Illesheim Army Heliport (ILH)",
"Illinois Valley Regional Airport-Walter A Duncan Field (VYS)",
"Takhamalt Airport (VVZ)",
"Illorsuit Heliport (IOT)",
"Ilo Airport (ILQ)",
"Iloilo International Airport (ILO)",
"Ilopango International Airport (ILS)",
"Ilu Airport (IUL)",
"Imane (IMN)",
"Imbaimadai Airport (IMB)",
"Immokalee Regional Airport (IMM)",
"Imonda Airport (IMD)",
"Imperial County Airport (IPL)",
"Imperial Municipal Airport (IML)",
"Impfondo Airport (ION)",
"Imphal International Airport (IMF)",
"In Guezzam Airport (INF)",
"In Salah Airport (INZ)",
"Juara Airport (JUA)",
"Inagua Airport (IGA)",
"Inanwatan Airport (INX)",
"Incirlik Air Base (UAB)",
"Indagen Airport (IDN)",
"Independence Municipal Airport (IDP)",
"Independence Airport (INB)",
"De La Independencia Airport (QRC)",
"Indian Mountain LRRS Airport (UTO)",
"Indigo Bay Lodge Airport (IBL)",
"Indulkana Airport (IDK)",
"Industrial Airpark (HBB)",
"Ine Airport (IMI)",
"Ingalls Field (HSP)",
"THingeyri Airport (TEY)",
"Ingham Airport (IGH)",
"Ingjaldssandur Airport (HLO)",
"Ingolstadt Manching Airport (IGS)",
"Inhaca Airport (IHC)",
"Inhaminga (IMG)",
"Inisheer Aerodrome (INQ)",
"Inishmaan Aerodrome (IIA)",
"Inishmore Aerodrome (IOR)",
"Injune Airport (INJ)",
"Inkerman Airport (IKP)",
"Innamincka Airport, Innamincka (INM)",
"Innarsuit Heliport (IUI)",
"Innisfail Aerodrome (IFL)",
"Inongo (INO)",
"Inta Airport (INA)",
"Inukjuak Airport (YPH)",
"Inuvik Mike Zubko Airport (YEV)",
"Inverell Airport (IVR)",
"Inverway Airport (IVW)",
"Inyati Airfield (INY)",
"Inyokern Airport (IYK)",
"Iokea Airport (IOK)",
"Ioma Airport (IOP)",
"Iosco County Airport (ECA)",
"Iowa Falls Municipal Airport (IFA)",
"Iowa City Municipal Airport (IOW)",
"Ipiau Airport (IPU)",
"Ipil Airport (IPE)",
"Ipiranga Airport (IPG)",
"Ipota Airport (IPA)",
"Iqaluit Airport (YFB)",
"Iraan Municipal Airport (IRB)",
"Iranamadu (IRU)",
"Iranshahr Airport (IHR)",
"Irece Airport (IRE)",
"Iringa Airport (IRI)",
"Iriona (IRN)",
"Isafjoerdur Airport (IFJ)",
"Ishurdi Airport (IRD)",
"Isisford Airport (ISI)",
"Iskandar Airport (PKN)",
"Scusciuban Airport (CMS)",
"Isla Mujeres Airport (ISJ)",
"Isla de Tigre  Heliport (ELX)",
"Punta Islita Airport (PBP)",
"Isortoq Heliport (IOQ)",
"Issyk-Kul International Airport (IKU)",
"Itacoatiara Airport (ITA)",
"Itaituba Airport (ITB)",
"Itaperuna (ITP)",
"Itapetinga Airport (QIT)",
"Itaqui Airport (ITQ)",
"Itokama Airport (ITK)",
"Ittoqqortoormiit Heliport (OBY)",
"Itubera Airport (ITE)",
"Iturup Airport (ITU)",
"Hungriger Wolf Airport (IZE)",
"Ivanof Bay Seaplane Base (KIB)",
"Ivanovo South Airport (IWA)",
"Ivujivik Airport (YIK)",
"Iwo Jima Air Base (IWO)",
"Ixtepec Airport (IZT)",
"Ta'izz International Airport (TAI)",
"J. F. Mitchell Airport (BQU)",
"Duqm Jaaluni Airport (JNJ)",
"Jabalpur Airport (JLR)",
"Jabiru Airport (JAB)",
"Jabat Airfield (JAT)",
"Jaboticabal Airport (0)",
"Jacareacanga (JCR)",
"Jack Edwards Airport (GUF)",
"Jack Mc Namara Field (CEC)",
"Jackpot Airport/Hayden Field (KPT)",
"Jackson Municipal Airport (MJQ)",
"Jacksonville Executive at Craig Airport (CRG)",
"Jacksonville Municipal Airport (IJX)",
"Jacksonville, Naval Air Station (NIP)",
"Jacmel (JAK)",
"Jacobina Airport (JCM)",
"Jacqueline Cochran Regional Airport (TRM)",
"Jacquinot Bay Airport (JAQ)",
"Jaffrey Municipal-Silver Ranch Airport (AFN)",
"Jagdalpur Airport (JGB)",
"JAGS McCartney International Airport (GDT)",
"Garzon La Jagua Airport (GLJ)",
"Jahrom Airport (JAR)",
"Jaime Gonzalez Airport (CFG)",
"Jaisalmer Airport (JSA)",
"Jalalabad Airport (JAA)",
"Gorontalo / Jalaluddin (GTO)",
"Jales Airport (JLS)",
"Jaluit Airport (UIT)",
"Jam Airport (KNR)",
"Jamba Airport (JMB)",
"Jamestown Municipal Airport (JMS)",
"Jammu Airport (IXJ)",
"Jamnagar Airport (JGA)",
"Janakpur Airport (JKR)",
"Januaria Airport (JNA)",
"Rengat Japura Airport (RGT)",
"Jaque Airport (JQE)",
"Jask Airport (JSK)",
"Jasper, Jasper County-Bell Field Airport (JAS)",
"Jasper County Airport (RNZ)",
"Jasper airport (YJA)",
"Jatai Airport (JTI)",
"Jebel Ali Seaplane Base (DJH)",
"Jefferson County International Airport (TWD)",
"Jefferson Municipal Airport (EFW)",
"Jefferson City Memorial Airport (JEF)",
"Jeh Airport (JEJ)",
"Jeki Airport (JEK)",
"Jenpeg Airport (ZJG)",
"Jeon Ju Airport (CHN)",
"Jeongseok Airport (JDG)",
"Jequie Airport (JEQ)",
"Jeremie Airport (JEE)",
"Jerry Tyler Memorial Airport (NLE)",
"Jessore Airport (JSR)",
"Jeypore Airport (PYB)",
"Jharsuguda Airport (JRG)",
"Jiagedaqi Airport (JGD)",
"Jiamusi Airport (JMU)",
"Jiansanjiang Airport (JSJ)",
"Jiayuguan Airport (JGN)",
"Jicalapa (GUO)",
"Jilin Ertaizi Airport (JIL)",
"Jim Hamilton-L.B. Owens Airport (CUB)",
"Jim Kelly Field (LXN)",
"Jimma Airport (JIM)",
"Jinchuan Airport (JIC)",
"Jindabyne Airport (QJD)",
"Jinggangshan Airport (JGS)",
"Jinhae Airport (CHF)",
"Jinja Airport (JIN)",
"Jinzhou Bay Airport (JNZ)",
"Jipijapa Airport (JIP)",
"Jiri Airport (JIR)",
"Jiroft Airport (JYR)",
"Jiwani Airport (JIW)",
"Jixi Xingkaihu Airport (JXA)",
"Atsugi United States Naval Air Station (NJA)",
"Hachinohe Ab (HHE)",
"Minami Torishima Airport (MUS)",
"Omura Airport (OMJ)",
"Maraba Airport (MAB)",
"Joao Durval Carneiro Airport (FEC)",
"Pelotas (PET)",
"Jodhpur Airport (JDH)",
"Johan Pienaar Airport (KMH)",
"John F Kennedy Memorial Airport (ASX)",
"San Pedro Airport (SPR)",
"John H Batten Field (RAC)",
"Johnstown-Cambria County Airport (JST)",
"John A. Osborne Airport (MNI)",
"Johnson County Airport (BYG)",
"Johnson County Executive Airport (OJC)",
"Joliet Regional Airport (JOT)",
"Jolo Airport (JOL)",
"Jomsom Airport (JMO)",
"Jordan Airport (JDN)",
"Jorge Enrique Gonzalez Torres Airport (SJE)",
"Jorge Isaac Airport (MCJ)",
"Jorhat Airport (JRH)",
"Jos Orno Imsula Airport (JIO)",
"José Aponte de la Torre Airport (NRR)",
"Jose Celestino Mutis Airport (BSC)",
"Municipal Jose Figueiredo Airport (PSW)",
"J.M. Velasco Ibarra (MRR)",
"Jose De San Martin Airport (JSM)",
"Josefa Camejo International Airport (LSP)",
"Josephstaal Airport (JOP)",
"Juan de Ayolas Airport (AYO)",
"Juan Casiano Airport (GPI)",
"Juan H White Airport (CAQ)",
"Paipa (RON)",
"Juan Mendoza Airport (ORU)",
"Juan Pablo Perez Alfonso Airport (VIG)",
"Juan Simons Vela Airport (RIJ)",
"Juana Azurduy de Padilla International Airport (SRE)",
"Juancho E. Yrausquin Airport (SAB)",
"Juanjui Airport (JJI)",
"Jubail Airport (QJB)",
"Juina Airport (JIA)",
"Juist Airport (JUI)",
"Julia Creek Airport (JCK)",
"Júlio de Castilho Airport (0)",
"Jumandy Airport (TNW)",
"Jumla Airport (JUM)",
"Jundah Airport (JUN)",
"Jungwon Air Base (JWO)",
"Junin Airport (JNI)",
"Jurado Airport (JUO)",
"Jurien Bay Airport (JUR)",
"Juruena Airport (JRN)",
"Juscelino Kubitscheck Airport (TFL)",
"Jutigalpa airport (JUT)",
"Pato Branco Airport (PTO)",
"Jwaneng Airport (JWA)",
"Mthatha Airport (UTT)",
"Kaadedhdhoo Airport (KDM)",
"Kabala Airport (KBA)",
"Kabalo Airport (KBO)",
"Kaven Airfield (KBT)",
"Kabri Dar Airport (ABK)",
"Kabwum Airport (KBM)",
"Kadanwari Airport (KCF)",
"Kadapa Airport (CDP)",
"Kadena Air Base (DNA)",
"Kadhdhoo Airport (KDO)",
"Kadugli Airport (KDX)",
"Kaedi Airport (KED)",
"Kaele Airport (KLE)",
"Kaghau Island Airport (KGE)",
"Kagi Airstrip (KGW)",
"Kagua Airport (AGK)",
"Buzwagi Airport (KBH)",
"Kahramanmaras Airport (KCM)",
"Kaieteur International Airport (KAI)",
"Kaikohe Aerodrome (KKO)",
"Kaikoura Airport (KBZ)",
"Kailashahar Airport (IXH)",
"Kaili Huangping Airport (KJH)",
"Kaimana Airport (KNG)",
"Kaintiba Airport (KZF)",
"Kairuku (RKU)",
"Kaitaia Airport (KAT)",
"Kakamega Airport (GGM)",
"Kake Seaplane Base (KAE)",
"Kremenchuk Airport (KHU)",
"Kokoro Airport (KOR)",
"Kalabo Airport (KLB)",
"Kalakaket Creek Air Station (KKK)",
"Kalaleh Airport (KLM)",
"Kalaupapa Airport (LUP)",
"Kalay Airport (KMV)",
"Kalbarri Airport (KAX)",
"Kalemie Airport (FMI)",
"Kalimarau Airport (BEJ)",
"Kalkgurung Airport (KFG)",
"Kalokol Airport (KLK)",
"Kalpowar Airport (KPP)",
"Kalskag Airport (KLG)",
"Kaltag Airport (KAL)",
"Kalumburu Airport (UBU)",
"Kalymnos Island National Airport (JKL)",
"Kamalpur Airport (IXQ)",
"Kamaran Downs (KDS)",
"Kamaran Airport (KAM)",
"Kamarang Airport (KAR)",
"Kamarata Airport (KTV)",
"Kambalda Airport (KDB)",
"Kamberatoro Airport (KDQ)",
"Kambuaya Airport (KBX)",
"Kamembe Airport (KME)",
"Kamileroi Airport (KML)",
"Kamina Airport (KMF)",
"Kamina Air Base (KMN)",
"Kamphaeng Saen Airport (KDT)",
"Kampong Chhnang Airport (KZC)",
"Kamulai (KAQ)",
"Kamur Airport (KCD)",
"Kamusi Airport (KUY)",
"Kanab Municipal Airport (KNB)",
"Kanabea Airport (KEX)",
"Kanainj (KNE)",
"Kananga Airport (KGA)",
"Kanas Airport (KJI)",
"Kandep Airport (KDP)",
"Kandi Airport (KDC)",
"Kandla Airport (IXY)",
"Kandrian Airport (KDR)",
"Marine Corps Air Station Kaneohe Bay (NGF)",
"Kangaatsiaq Heliport (QPW)",
"Kangding Airport (KGT)",
"Kangersuatsiaq Heliport (KGQ)",
"Kangiqsualujjuaq Georges River Airport (XGR)",
"Kangiqsujuaq (Wakeham Bay) Airport (YWB)",
"Kangirsuk Airport (YKG)",
"Kangra Airport (DHM)",
"Kaniama Airport (KNM)",
"Kankan Airport (KNN)",
"Kankesanturai Airport (JAF)",
"Kannur International Airport (CNN)",
"Kanpur Civil Airport (KNU)",
"Kanpur Airport (KNU)",
"Kantchari Airport (XKA)",
"Kanti Airport (KHM)",
"Kao Kuabang Airport (KAZ)",
"Kaolack Airport (KLC)",
"Kaoma (KMZ)",
"Kapal Airport (KPL)",
"Kapanga Airport (KAP)",
"Kapit Airport (KPI)",
"Kapuskasing Airport (YYU)",
"Kar Airport (KAK)",
"Kar Kar Airport (KRX)",
"Karamay Airport (KRY)",
"Karanambo Airport (KRM)",
"Karasabai Airport (KRG)",
"Karasburg Airport (KAS)",
"Karato Airport (KAF)",
"Karawari Airstrip (KRJ)",
"Karel Sadsuitubun Airport (LUV)",
"Kariba Airport (KAB)",
"Karimui Airport (KMR)",
"Karl Stefan Memorial Airport (OFK)",
"Karlskoga Airport (KSK)",
"Karluk Airport (KYK)",
"Karluk Lake Seaplane Base (KKL)",
"Karonga Airport (KGJ)",
"Karratha Airport (KTA)",
"Karshi-Khanabad Air Base (KSQ)",
"Karshi Airport (KSQ)",
"Karubaga Airport (KBF)",
"Karumba Airport (KRB)",
"Kasaan Seaplane Base (KXA)",
"Kasaba Bay Airport (ZKB)",
"Kasabonika Airport (XKS)",
"Kasama Airport (KAA)",
"Kasba Lake Airport (YDU)",
"Kasenga Airport (KEC)",
"Kasese Airport (KSE)",
"Kashan Airport (KKS)",
"Kashechewan Airport (ZKE)",
"Kasigluk Airport (KUK)",
"Kasiguncu Airport (PSJ)",
"Kasompe Airport (CGJ)",
"Kasonombe Airport (KSB)",
"Kastellorizo Island Public Airport (KZS)",
"Kasungu Airport (KBQ)",
"Katanning Airport (KNI)",
"Katima Mulilo Airport (MPA)",
"Katiola Airport (KTC)",
"Katiu Airport (KXU)",
"Kato Airport (KTO)",
"Katrineholm (XXK)",
"Katsina Airport (DKA)",
"Katukurunda Airport (KTY)",
"Kauehi Airport (KHZ)",
"Kaukura Airport (KKR)",
"Kautokeino Airport (QKX)",
"Kavalerovo Airport (KVR)",
"Kavanayen Airport (KAV)",
"Kavieng Airport (KVG)",
"Kawama Airport (VRO)",
"Kawito Airport (KWO)",
"Kawthoung Airport (KAW)",
"Kaya Airport (XKY)",
"Kayenta Airport (MVM)",
"Kayes Airport (KYS)",
"Kazi Nazrul Islam Airport (RDP)",
"Kearney Regional Airport (EAR)",
"Kebar Airport (KEQ)",
"Kedougou (KGG)",
"Keekorok Airport (KEU)",
"Biloxi, Keesler Air Force Base, Navu (BIX)",
"Keetmanshoop Airport (KMP)",
"Keewaywin Airport (KEW)",
"Kegaska Airport (ZKG)",
"Kegelman Air Force Auxiliary Field (CKA)",
"Keglsugl Airport (KEG)",
"Keisah Airport (KEA)",
"Kelafo East Airport (LFO)",
"Kelani-Peliyagoda Sea Plane Base (KEZ)",
"Kelila Airport (LLN)",
"Kelle Airport (KEE)",
"Kelsey Airport (KES)",
"Keluang Airport (KLQ)",
"Kemi-Tornio Airport (KEM)",
"Kemmerer Municipal Airport (EMM)",
"Kempsey Airport (KPS)",
"Kenai Municipal Airport (ENA)",
"Kenema Airport (KEN)",
"Kengtung Airport (KET)",
"Kenieba Airport (KNZ)",
"Keningau Airport (KGU)",
"Kenitra Air Base (NNA)",
"Kenmore Air Harbor Seaplane Base (KEH)",
"Lake Union Seaplane Base (LKE)",
"Kennett Memorial Airport (KNT)",
"Kenora Airport (YQK)",
"Kentland Municipal Airport (KKT)",
"Keokuk Municipal Airport (EOK)",
"Keperveyem Airport (KPW)",
"Kepi Airport (KEI)",
"Kerang Airport (KRA)",
"Kerau (KRU)",
"Kerch Airport (KHC)",
"Kerema Airport (KMA)",
"Kericho Airport (KEY)",
"Kerikeri Airport (KKE)",
"Kerio Valley Airport (KRV)",
"Kerrville Municipal Airport (ERV)",
"Kerteh Airport (KTE)",
"Keshod Airport (IXK)",
"Ketapang / Rahadi Usmaman (KTG)",
"Ketchikan Harbor Seaplane Base (WFB)",
"Key Lake Airport (YKJ)",
"Key Field (MEI)",
"Khabarovsk Airport (KHV)",
"Khaneh Airport (KHA)",
"Kharg Island Airport (KHK)",
"Kharkhorin Airport (KHR)",
"Khassab (KHS)",
"Khashm El Girba Airport (GBU)",
"Khatanga Airport (HTG)",
"Khatgal Airport (HTM)",
"Khok Kathiam Airport (KKM)",
"Khoka Moya Airport (KHO)",
"Khorramabad International Airport (KHD)",
"Khost (KHT)",
"Khovd Airport (HVD)",
"Khowai Airport (IXN)",
"Khoy Airport (KHY)",
"Khujirt Airport (HJT)",
"Khuzdar Airport (KDD)",
"Khwahan Airport (KWH)",
"Khwai River Lodge Airport (KHW)",
"Kibuli Airstrip (KII)",
"Kichwa Tembo Airstrip (KTJ)",
"Kickapoo Downtown Airport (KIP)",
"Hasselt Airport (QHA)",
"Kiffa Airport (KFA)",
"Kigoma Airport (TKQ)",
"Kikori Airport (KRI)",
"Kikwit Airport (KKW)",
"Kilaguni Airport (ILU)",
"Kili Airfield (KIO)",
"Kilkenny Airport (KKY)",
"Kilwa Airport (KIL)",
"Kilwa Masoko Airport (KIY)",
"Kimaam Island Airport (KMM)",
"Kiman Airport (KMM)",
"Hoskins Airport (HKN)",
"Kimberley Downs Airport (KBD)",
"Kimble County Airport (JCT)",
"Kimmirut Airport (YLC)",
"Kincardine Airport (YKD)",
"Kindamba Airport (KNJ)",
"Kindersley Regional Airport (YKY)",
"Kindu Airport (KND)",
"King Abdulaziz Air Base (DHA)",
"King Cove Airport (KVC)",
"King Hussein Air Base (OMF)",
"King Khaled Military City Airport (KMC)",
"King Island Airport (KNS)",
"King Mswati III International Airport (SHO)",
"King Salman Abdulaziz Airport (DWD)",
"King Salmon Airport (AKN)",
"Kingaroy Airport (KGY)",
"Kingfisher Lake Airport (KIF)",
"Kings Canyon (KBJ)",
"Kings Creek Airport (KCS)",
"Kingscote Airport (KGC)",
"Kingston/Norman Rogers Airport (YGK)",
"Kingsville, Naval Air Station (NQI)",
"Kinkungwa Airport (KLY)",
"Kinston Regional Jetport (ISO)",
"Kipnuk Airport (KPN)",
"Kira Airport (KIQ)",
"Kirawira B Aerodrome (GTZ)",
"Kirensk Airport (KCK)",
"Kirk Field (PGR)",
"Kirkimbie Station Airport (KBB)",
"Kirkland Lake Airport (YKX)",
"Kirksville Regional Airport (IRK)",
"Kirkuk Air Base (KIK)",
"Kirsch Municipal Airport (IRS)",
"Kirundo Airport (KRE)",
"Kiryat Shmona Airport (KSW)",
"Ajmer Kishangarh Airport (KQH)",
"Kissidougou (KSI)",
"Kissimmee Gateway Airport (ISM)",
"Kitale Airport (KTL)",
"Kitava Airport (KVE)",
"Kitee Airport (KTQ)",
"Kithira Airport (KIT)",
"Kitkatla Seaplane Base (YKK)",
"Kitoi Bay Seaplane Base (KKB)",
"Kitsissuarsuit Heliport (QJE)",
"Kittilae Airport (KTT)",
"Kiunga Airport (KIU)",
"Kiunga Airport (UNG)",
"Kivalina Airport (KVL)",
"Kiwayu Airport (KWY)",
"Klaipeda Airport (KLJ)",
"Klawock Airport (KLW)",
"Kleinzee Airport (KLZ)",
"Klemtu Seaplane Base (YKT)",
"Tula Klokovo Air Base (TYA)",
"Knee Lake Airport (YKE)",
"Kneeland Airport (NLN)",
"Knox County Regional Airport (RKD)",
"Koblenz-Winningen Airport (ZNV)",
"Kobuk Airport (OBU)",
"Kodiak Municipal Airport (KDK)",
"Koggala Airport (KCT)",
"Kaoh Kong Airport (KKZ)",
"Kohat Airport (OHT)",
"Koinambe (KMB)",
"Koingnaas Airport (KIG)",
"Kokhanok Airport (KNK)",
"Kokoda Airport (KKD)",
"Kokomo Municipal Airport (OKK)",
"Kokonao / Timuka (KOX)",
"Kol Airport (KQL)",
"Kolda North Airport (KDA)",
"Kolhapur Airport (KLH)",
"Koliganek Airport (KGK)",
"Kolwezi Airport (KWZ)",
"Komaio Airport (KCJ)",
"Komako Airport (HOC)",
"Komatipoort (KOF)",
"Komo Airfield (KOM)",
"Komodo Airport (LBJ)",
"Kompiam Airport (KPM)",
"Kampong Cham (KZK)",
"Komsomolsk-on-Amur Airport (KXK)",
"Kun Airport (KCI)",
"Konarak Airport (ZBR)",
"Konawaruk (KKG)",
"Kondavattavan Tank Sea Plane Base (AFK)",
"Kondinskoye Airport (KXD)",
"Kondobol Airport (KPF)",
"Kunduz Airport (UND)",
"Kone Airport (KNQ)",
"Konge Airport (KGB)",
"Kongiganak Airport (KKH)",
"Kongolo Airport (KOO)",
"Kontum Airport (KON)",
"Kooddoo Atoll Airport (GKK)",
"Koolatah Airport (KOH)",
"Koolburra Airport (KKP)",
"Kopasker Airport (OPA)",
"Kopiago Airport (KPA)",
"Korhogo Airport (HGO)",
"Korla Airport (KRL)",
"Kornasoren Airfield (FOO)",
"Koro Island Airport (KXF)",
"Koroba Airport (KDE)",
"Kosciusko-Attala County Airport (OSX)",
"Kosipe Airport (KSP)",
"Kosrae International Airport (KSA)",
"Koszalin-Zegrze Pomorskie Airport (OSZ)",
"Kota Airport (KTU)",
"Kotabangun Airport (KOD)",
"Kotakoli Airport (KLI)",
"Flugplatz Koethen (KOQ)",
"Kotlas Airport (KSZ)",
"Kotlik Airport (KOT)",
"Koulamoutou Airport (KOU)",
"Koumac Airport (KOC)",
"Koutiala (KTX)",
"Kowanyama Airport (KWM)",
"Koyuk Airport (KKA)",
"Koyukuk Airport (KYU)",
"Baikonur Krayniy Airport (BXY)",
"Krakor Airport (KZD)",
"Kramatorsk Airport (KRQ)",
"Krasnoselkup (KKQ)",
"Kratie Airport (KTI)",
"Kribi Airport (KBI)",
"Krokstadarmelar Airport (HVM)",
"Kubin Airport (KUG)",
"Kudat Airport (KUD)",
"Kufra Airport (AKF)",
"Kugaaruk Airport (YBB)",
"Kugluktuk Airport (YCO)",
"Niuatoputapu Airport (NTT)",
"Kuito Airport (SVP)",
"Kukundu Airport (KUE)",
"Kulgera Airport (KGR)",
"Kulhudhuffushi Airport (HDK)",
"Kulik Lake Airport (LKK)",
"Kullorsuaq Heliport (KHQ)",
"Kullu Manali Airport (KUU)",
"Kulob Airport (TJU)",
"Kumejima Airport (UEO)",
"Kungim Airstrip (KGM)",
"Kunovice Airport (UHE)",
"Kununurra Airport (KNX)",
"Kupiano Airport (KUP)",
"Kuqa Qiuci Airport (KCA)",
"Kuri Airport (KUQ)",
"Kuria Airfield (KUC)",
"Kurnool Airport (KJB)",
"Kurundi (KRD)",
"Kuujjuaq Airport (YVP)",
"Kuujjuarapik Airport (YGW)",
"Kuummiit Heliport (KUZ)",
"Kuyol Airport (KUX)",
"Kwailabesi Airport (KWS)",
"Kwethluk Airport (KWT)",
"Kwigillingok Airport (KWK)",
"Kyaukpyu Airport (KYP)",
"Kyauktu Airport (KYT)",
"Murray-Calloway County Airport (CEY)",
"L'Aquila's Preturo Airport (QAQ)",
"Lyman Clayton Airport (OLF)",
"A L Mangham Jr. Regional Airport (OCH)",
"L O Simenstad Municipal Airport (OEO)",
"Lab Lab Airport (LAB)",
"Labasa Airport (LBS)",
"Labo Airport (OZC)",
"Labrea Airport (LBR)",
"Labuan Airport (LBU)",
"Lac La Biche Airport (YLB)",
"Lac Brochet Airport (XLB)",
"Kelly Field Annex (SKF)",
"Laconia Municipal Airport (LCI)",
"Ladd Army Airfield (FBK)",
"Laduani Airstrip (LDO)",
"Lady Elliot Island Airport (LYT)",
"Ladysmith Airport (LAY)",
"Lae Airfield (LML)",
"Laeso Airport (BYR)",
"Lages Airport (LAJ)",
"L'Mekrareg Airport (LOO)",
"Lague Airport (LCO)",
"Laguna Airport (0)",
"Guanaja (GJA)",
"Colonia del Sacramento International Airport (CYR)",
"Lagunillas Airport (LGY)",
"Lahad Datu Airport (LDU)",
"Lai Airport (LTC)",
"Lake Baringo Airport (LBN)",
"Lake Brooks Seaplane Base (BKF)",
"Lake County Airport (LKV)",
"Lake County Airport (LXV)",
"Lake Cumberland Regional Airport (SME)",
"Lake Evella Airport (LEL)",
"Lake Macquarie Airport (BEO)",
"Lake Manyara Airport (LKY)",
"Lake Murray Airport (LMY)",
"Lake Nash (LNH)",
"Lake Placid Airport (LKP)",
"Lake Rudolf Airport (LKU)",
"Island Lake Airport (YIV)",
"Lakeba Island Airport (LKB)",
"Lakefield Airport (LFP)",
"Lakeland Downs Airport (LKD)",
"Lakeland-Noble F Lee Memorial Field (ARV)",
"Lal Bahadur Shastri Airport (VNS)",
"Lalibela Airport (LLI)",
"Lalmonirhat Airport (LLJ)",
"Lamap Airport (LPM)",
"Lambarene Airport (LBQ)",
"Lamen Bay Airport (LNB)",
"Lamerd Airport (LFM)",
"Lamidanda Airport (LDN)",
"Lampang Airport (LPT)",
"Lampedusa Airport (LMP)",
"Lampson Field (CKE)",
"Lanai Airport (LNY)",
"Lancang Jingmai Airport (JMJ)",
"Landivisiau (LDV)",
"AFB Langebaanweg (SDB)",
"Langeoog (LGO)",
"Langimar (LNM)",
"Langkawi International Airport (LGK)",
"Langley Air Force Base (LFI)",
"Langley Regional Airport (YLY)",
"Lankaran International Airport (LLK)",
"Lansdowne House Airport (YLH)",
"Lansdowne Station (LDW)",
"Lanzhou Zhongchuan Airport (LHW)",
"Lar Airport (LRR)",
"Larissa Airport (LRA)",
"Larsen Bay Airport (KLN)",
"Las Breas Airport (TTC)",
"Las Brujas Airport (BWW)",
"Corozal Las Brujas Airport (CZU)",
"Las Cruces (SNT)",
"Las Flores Airport (DZI)",
"Aeropuerto El Banco (ELB)",
"Las Heras Airport (LHS)",
"Las Malvinas/Echarate Airport (VVN)",
"Las Vegas Airport (LCF)",
"Las Vegas Municipal Airport (LVS)",
"Lasham Airfield (QLA)",
"Lashio Airport (LSH)",
"Lastoursville Airport (LTL)",
"Latina Airport (QLT)",
"Latrobe Regional Airport (TGN)",
"Laucala Island Airport (LUC)",
"Laura Airport (LUU)",
"Laurie River Airport (LRQ)",
"Laurinburg-Maxton Airport (MXE)",
"Carriacou (CRU)",
"Passo Fundo (PFB)",
"Lausanne (QLS)",
"Lava Falls Heliport (HAE)",
"Laval-Entrammes (LVA)",
"Lavan Island Airport (LVP)",
"Laverton Airport (LVO)",
"Lawas Airport (LWY)",
"Lawn Hill Airport (LWH)",
"Lawrence Municipal Airport (LWC)",
"Lawrenceville-Vincennes International Airport (LWV)",
"Lawson Army Airfield (LSF)",
"Lawton-Fort Sill Regional Airport (LAW)",
"Layang-Layang Island Airport (LAC)",
"Lazaro Cardenas Airport (LZC)",
"LBJ Ranch Airport (JCY)",
"Hobbs Airport (HOB)",
"Leaf Rapids Airport (YLR)",
"Learmonth Airport (LEA)",
"Lwbak (LWA)",
"Lebakeng Airport (LEF)",
"Lebel-sur-Quevillon Airport (YLS)",
"Lecce Galatina Airport (LCC)",
"Lee Airport (ANP)",
"Lee C Fine Memorial Airport (AIZ)",
"Lee Gilmer Memorial Airport (GVL)",
"Guantanamo Bay Naval Base (NBW)",
"Gecitkale Air Base (GEC)",
"Legazpi International Airport (LGP)",
"Leigh Creek Airport (LGH)",
"Leinster Airport (LER)",
"Leirin Airport (VDB)",
"Leitre Airport (LTF)",
"Lekana Airport (LKC)",
"Lekhwair (LKW)",
"Leknes Airport (LKN)",
"Lekoni (LEO)",
"Lelystad Airport (LEY)",
"Leme Airport (0)",
"Lemhi County Airport (SMN)",
"Lemmon Municipal Airport (LEM)",
"Lemoore Naval Air Station (Reeves Field) (NLC)",
"Lencois Paulista Airport (QGC)",
"Lengbati Airport (LNC)",
"Lengpui Airport (AJL)",
"Leo Airport (XLU)",
"Leonora Airport (LNO)",
"Leopoldina Airport (LEP)",
"Lereh Airport (LHI)",
"Leribe Airport (LRB)",
"Leron Plains Airport (LPN)",
"Lerwick Tingwall Airport (LWK)",
"Les Bases Airport (GBJ)",
"Les Cayes Antoine-Simon Airport (CYA)",
"Les Eplatures (ZHV)",
"Les Sables-d'Olonne - Talmont Airport (LSO)",
"Lese Airport (LNG)",
"Leshukonskoye Airport (LDG)",
"Lesobeng Airport (LES)",
"Letfotar Airport (MOM)",
"Lethem Airport (LTM)",
"Letterkenny Airport (LTR)",
"Letung Airport (LMU)",
"Levelock Airport (KLL)",
"Levuka Airfield (LEV)",
"Lewis University Airport (LOT)",
"Lewistown Municipal Airport (LWT)",
"Lewoleba Airport (LWE)",
"Lhasa Gonggar Airport (LXA)",
"Lhok Sukon Airport (LSX)",
"Liangping Airport (LIA)",
"Lianshulu (LHU)",
"Libenge Airport (LIE)",
"Liberal Mid-America Regional Airport (LBL)",
"Libo Airport (LLB)",
"Liboi Airport (LBK)",
"Colima Airport (CLQ)",
"Lichinga Airport (VXC)",
"Lien Khuong Airport (DLI)",
"Lifou Airport (LIF)",
"Lifuka Island Airport (HPA)",
"Lightning Ridge Airport (LHG)",
"Likiep Airfield (LIK)",
"Likoma Island Airport (LIX)",
"Limbé Military Heliport (VCC)",
"Limbunya Airport (LIB)",
"Lime Acres Finsch Mine Airport (LMR)",
"Lime Village Airport (LVD)",
"Limeira Airport (QGB)",
"Limon Municipal Airport (LIC)",
"Tuli Lodge Airport (TLD)",
"Municipal de Linares Airport (ZLR)",
"Lindeman Island Airport (LDC)",
"Lindi Airport (LDI)",
"Yichun Lindu Airport (LDS)",
"Linga Linga Airport (LGN)",
"Lingling Airport (LLF)",
"Lins Airport (LIP)",
"Lincang Airport (LNJ)",
"Liping Airport (HZH)",
"Lisala Airport (LIQ)",
"Lissadell Airfield (LLL)",
"Farsund Airport, Lista (FAN)",
"Little Grand Rapids Airport (ZGR)",
"Liuzhou Bailian Airport (LZH)",
"Livengood Camp Airport (LIV)",
"Lleida-Alguaire Airport (ILD)",
"Lloydminster Airport (YLL)",
"Lobatse Airport (LOQ)",
"Lobito Airport (LLT)",
"Locarno Airport (ZJI)",
"Lock Airport (LOC)",
"Lockhart River Airport (IRG)",
"Lodja Airport (LJA)",
"Lodwar Airport (LOK)",
"Loei Airport (LOE)",
"Logan-Cache Airport (LGU)",
"Loikaw Airport (LIW)",
"Lokichoggio Airport (LKG)",
"Loncopue Airport (LCP)",
"Lihir Island/Kunaye Airport (LNV)",
"Londolozi Airport (LDZ)",
"London-Corbin Airport (LOZ)",
"Lonesome Pine Airport (LNP)",
"Long Akah Airport (LKH)",
"Long Apung Airport (LPU)",
"Long Banga Airport (LBP)",
"Long Bawan Airport (LBW)",
"Long Lama Airport (LLM)",
"Long Lellang Airport (LGL)",
"Long Island (LSJ)",
"Long Pasia Airport (GSA)",
"Long Semado Airport (LSM)",
"Long Seridan Airport (ODN)",
"Long Sukang Airport (LSU)",
"Longana Airport (LOD)",
"Longnan Chengzhou Airport (LNL)",
"Longreach Airport (LRE)",
"Longyan Guanzhishan Airport (LCX)",
"Lonorore Airport (LNE)",
"Lopez Island Airport (LPS)",
"Loralai Airport (LRG)",
"Lord Howe Island Airport (LDH)",
"Lordsburg Municipal Airport (LSB)",
"Lorenzo Airport (MXQ)",
"Lorient - Lann-Bihoue (LRT)",
"Loring International Airport (LIZ)",
"Loring Seaplane Base (WLR)",
"Lorraine Airport (LOA)",
"Los Banos Municipal Airport (LSN)",
"Los Cerrillos Military Heliport (ULC)",
"Los Chiles Airport (LSL)",
"Los Colonizadores Airport (RVE)",
"Los Garzones Airport (MTR)",
"Los Menucos Airport (LMD)",
"Los Perales Airport (BHA)",
"Los Roques Airport (LRV)",
"Losinj Airport (LSZ)",
"Losuia Airport (LSA)",
"Lotus Vale Airport (LTV)",
"Louis Trichardt Airport (LCD)",
"Louisa, Louisa County Airport/Freeman Field (LOW)",
"Louisville Winston County Airport (LMS)",
"Lourdes-de-Blanc-Sablon Airport (YBX)",
"Lowai Airport (LWI)",
"Fort Rucker, Lowe Army Heliport (LOR)",
"Loyengalani Airport (LOY)",
"Lt. Warren Eaton Airport (OIC)",
"LTS Pulau Redang Airport (RDN)",
"Luabo (LBM)",
"Luang Namtha Airport (LXG)",
"Luau Airport (UAL)",
"Lubang Airport (LBX)",
"Lubango Airport (SDD)",
"Lucapa Airport (LBZ)",
"Lucélia Airport (0)",
"Lucenec (LUE)",
"Luderitz Airport (LUD)",
"Ludhiana Airport (LUH)",
"Luena Airport (LUO)",
"Lugh Ganane Airport (LGX)",
"Luhansk International Airport (VSG)",
"Luiza Airport (LZA)",
"Tenzing-Hillary Airport (LUA)",
"Lukulu (LXU)",
"Lueliang Airport (LLV)",
"Lumbala Airport (GGC)",
"Lumbala Airport (GGC)",
"Lumbo (LFB)",
"Lumi Airport (LMI)",
"Lumid Pau Airport (LUB)",
"Lund Hospital Heliport (XGC)",
"Lunyuk Airport (LYK)",
"Luoyang Beijiao Airport (LYA)",
"Luozi Airport (LZI)",
"Lusambo Airport (LBO)",
"Lusanga Airport (LUS)",
"Lushoto Airport (LUY)",
"Lusikisiki Airport (LUJ)",
"Lusk Municipal Airport (LSK)",
"Lutselk'e Airport (YSG)",
"Lutsk Airport (UCK)",
"Luzamba Airport (LZM)",
"Luzhou Yunlong Airport (LZO)",
"Lyall Harbour Seaplane Base (YAJ)",
"Lycksele Airport (LYC)",
"Lyndhurst Airport (LTP)",
"Lynn Lake Airport (YYL)",
"Lyons-Rice County Municipal Airport (LYO)",
"M'Bigou Airport (MBC)",
"M'Boki Airport (MKI)",
"Gaddhoo Maavaarulaa Airport (RUL)",
"Mabaruma Airport (USI)",
"Mabuiag Island Airport (UBB)",
"Benedito Lacerda Airport (MEA)",
"Macagua Airport (SFX)",
"Macanal (NAD)",
"La Macarena Airport (LMC)",
"Macau Heliport at Outer Harbour Ferry Terminal (XZM)",
"La Macaza / Mont-Tremblant International Inc Airport (YTM)",
"MacDonald Downs Airport (MNW)",
"Macenta Airport (MCA)",
"Machado Airport (0)",
"Mackall Army Airfield (HFF)",
"Macmillan Pass Airport (XMP)",
"Macomb Municipal Airport (MQB)",
"Macon Downtown Airport (MAC)",
"Madang Airport (MAG)",
"Madison Municipal Airport (XMD)",
"Madras Municipal Airport (MDJ)",
"Madurai Airport (IXM)",
"Mae Hong Son Airport (HGN)",
"Pai (PYY)",
"Mae Sot Airport (MAQ)",
"Rondonopolis Airport (ROO)",
"Maewo Naone Airport (MWF)",
"Mafeteng Airport (MFC)",
"Mafia Island Airport (MFA)",
"Mafra Airport (QMF)",
"Magan Airport (GYG)",
"Maganja da Costa Airport (MJS)",
"Magdagachi Airport (GDG)",
"Magdalena Airport (MGD)",
"Magdeburg, Sachsen-Anhalt (ZMG)",
"Magnolia Municipal Airport (AGO)",
"Magway Airport (MWQ)",
"Touba Mahana Airport (TOZ)",
"Mahanoro (VVB)",
"Maharana Pratap Airport (UDR)",
"Mahdia Airport (MHA)",
"Mahendranagar Airport (XMG)",
"Mahenye Airport (MJW)",
"Mahshahr Airport (MRX)",
"Maiana Airport (MNK)",
"La Majayura Airport (MCJ)",
"Maikwak Airport (VEG)",
"Maimana Airport (MMZ)",
"Mainoru Airport (MIZ)",
"Maintirano Airport (MXT)",
"Maio Airport (MMO)",
"Teixeira de Freitas Airport (TXF)",
"Maitland Airport (MTL)",
"Prince Abdul Majeed bin Abdulaziz Domestic Airport (ULH)",
"Majkin Airport (MJE)",
"Barbacena (QAK)",
"Major Brigadeiro Trompowsky Airport (VAG)",
"Majors Airport (GVT)",
"Makabana Airport (KMK)",
"Makemo Airport (MKP)",
"Makin Airfield (MTK)",
"Makini (MPG)",
"Makkovik Airport (YMN)",
"Makokou (MKU)",
"Makoua Airport (MKJ)",
"Maku National Airport (IMQ)",
"Makurdi Airport (MDI)",
"Mal Airport (MMV)",
"Malabang Airport (MLP)",
"Malacca International Airport (MKZ)",
"Malad City Airport (MLD)",
"Malaimbandy (WML)",
"Malakal Airport (MAK)",
"Malalaua Airport (MLQ)",
"Malam Airport (MQO)",
"Malamala Airport (AAM)",
"Malanje Airport (MEG)",
"Malda Airport (LDA)",
"Malekolon Airport (MKN)",
"Morowali Airport (MOH)",
"Haskovo Malevo Airport (HKV)",
"Alor Island Airport (ARD)",
"Maliana airport (MPT)",
"Maiana Airport (MPT)",
"Malikus Saleh Airport (LSW)",
"Mallacoota Airport (XMC)",
"Maloelap Airfield (MAV)",
"Malolo Lailai Island Airport (PTF)",
"Malta Airport (MLK)",
"Mamai (MAP)",
"Mamburao Airport (MBO)",
"Mamfe Airport (MMF)",
"Mamitupo Airport (MPI)",
"Mampikony Airport (WMP)",
"Man Airport (MJC)",
"Mana Island Airport (MNF)",
"Manakara Airport (WVK)",
"Mananara Nord Airport (WMR)",
"Manang Airport (NGX)",
"Mananjary Airport (MNJ)",
"Manapouri Airport (TEU)",
"Manare (MRM)",
"Manassas Regional Airport (MNZ)",
"Manatee Airport (MZE)",
"Manaung Airport (MGU)",
"Manda Airport (LAU)",
"Mandabe Airport (WMD)",
"Mandalgovi Airport (MXW)",
"Mandera Airport (NDE)",
"Mandinga Airport (COG)",
"Mandora Airport (MQA)",
"Mandritsara Airport (WMA)",
"Manga (MGP)",
"Mangaia Auau Airstrip (MGS)",
"Mangla Airport (XJM)",
"Mangochi Airport (MAI)",
"Mangole Airport, Falabisahaya (MAL)",
"Manguna Airport (MFO)",
"Mangunjaya Airport (MJY)",
"Manicore (MNX)",
"Manihi Airport (XMH)",
"Manihiki Island Airport (MHX)",
"Manila Municipal Airport (MXA)",
"Maningrida Airport (MNG)",
"Manistee County-Blacker Airport (MBL)",
"Manitowaning/Manitoulin East Municipal Airport (YEM)",
"Manitouwadge Airport (YMG)",
"Maniwaki Airport (YMW)",
"Manja Airport (MJA)",
"Manley Hot Springs Airport (MLY)",
"Manners Creek Airport (MFP)",
"Manokotak Airport (KMO)",
"Manono Airport (MNO)",
"Le Mans-Arnage (LME)",
"Mansa Airport (MNS)",
"Mansehra Airport (HRA)",
"Mansons Landing Seaplane Base (YMU)",
"Manti-Ephraim Airport (NTJ)",
"Manumu Airport (UUU)",
"Manzhouli Airport (NZH)",
"Mao (AMO)",
"Maota Airport (MXS)",
"Maple Bay Seaplane Base (YAQ)",
"Mapoda Airport (MPF)",
"Mapua(Mabua) Airport (MPU)",
"Temuco Maquehue Airport (ZCO)",
"Maquinchao Airport (MQD)",
"Mar de Cortes International Airport (PPE)",
"Mara Lodges Airport (MRE)",
"Mara North Airport (HKR)",
"Mara Serena Airport (MRE)",
"Maradi Airport (MFQ)",
"Marakei Airport (MZK)",
"Maramag (XMA)",
"Maramuni Airport (MWI)",
"Maranggo Airport (TQQ)",
"Marathon Airport (YSP)",
"Marau Airport (RUS)",
"Marawaka (MWG)",
"Marble Bar Airport (MBB)",
"Marcelo Pires Halzhausen Airport (AIF)",
"March Air Reserve Base (RIV)",
"Marco Island Airport (MRK)",
"La Roche - Mare Island Airport (MEE)",
"Marib (MYN)",
"Mareeba Airport (MRG)",
"Marfa Municipal Airport (MRF)",
"Margaret River Airport (MQZ)",
"Margaret River Station Airport (MGV)",
"Margate Airport (MGH)",
"Iligan (IGN)",
"Maria Dolores Airport (LSQ)",
"Maria Montez International Airport (BRX)",
"Maria Reiche Neuman Airport (NZC)",
"Mariana Grajales Airport (GAO)",
"Marianske Lazne Airport (MKA)",
"Maribor Edvard Rusjan Airport (MBX)",
"Marigot Sea Plane Base (MSB)",
"Gnoss Field (NOT)",
"Marina Di Campo Airport (EBA)",
"Marina Municipal Airport (OAR)",
"Marinda Airport (RJM)",
"Marinduque Airport (MRQ)",
"Uberaba (UBA)",
"Sao Carlos Airport (QSC)",
"Marion County Airport (APT)",
"Marion County Regional Airport (FLP)",
"Marion County - Rankin Fite Airport (HAB)",
"Marion Downs Airport (MXD)",
"Marion Municipal Airport (MNN)",
"Marion Municipal Airport (MZZ)",
"Maripasoula Airport (MPY)",
"Mariposa-Yosemite Airport (RMY)",
"Mariquita Airport (MQU)",
"Cuenca / Mariscal Lamar (CUE)",
"Markovo Airport (KVM)",
"Marla Airport (MRP)",
"Marlboro County Jetport - HE Avent Field (BTN)",
"Marlboro Airport (MXG)",
"Marmul Airport (OMM)",
"Maroantsetra Airport (WMN)",
"Marqua Airport (MQE)",
"Marree Airport (RRE)",
"Marromeu (RRM)",
"Marsa Brega Airport (LMQ)",
"Mersa Matruh Airport (MUH)",
"Marsabit Airport (RBT)",
"Marshall Army Airfield (FRI)",
"Marshall Don Hunter Sr. Airport (MLL)",
"Marshall Memorial Municipal Airport (MHL)",
"Marshalltown Municipal Airport (MIW)",
"Martindale Army Heliport (MDA)",
"Martubah Airport (DNF)",
"Marudi Airport (MUR)",
"Mary Airport (MYP)",
"Mary River (YMV)",
"Mary's Harbour Airport (YMH)",
"Maryborough Airport (MBH)",
"Masa Airport (MBV)",
"Masalembo Airport (MSI)",
"Masasi Airport (XMI)",
"Masi Manimba Airport (MSM)",
"Masindi Airport (KCU)",
"Masirah (MSH)",
"Mason City Municipal Airport (MCW)",
"Massa Cinquale Airport (QMM)",
"Massawa International Airport (MSW)",
"Massena International Airport-Richards Field (MSS)",
"Masset Airport (ZMT)",
"Masvingo Airport (MVZ)",
"Mata'aho Airport (NFO)",
"Matagami Airport (YNM)",
"Matahora Airport (WNI)",
"Mataiva Airport (MVT)",
"Matamata Aerodrome (MTA)",
"Matane Airport (YME)",
"Matão Airport (0)",
"Matari Airport (IRP)",
"Pereira / Matecana (PEI)",
"Matei Airport (TVU)",
"Mati Airport (MXI)",
"Matsaile Airport (MSG)",
"Manzini - Matsapha Airport (MTS)",
"Matsu Beigan Airport (MFK)",
"Matsu Nangan Airport (LZN)",
"Mattala Rajapaksa International Airport (HRI)",
"Matthew Spain Airport (SQS)",
"Matthews Ridge (MWJ)",
"Jose Tadeo Monagas International Airport (MUN)",
"Maubeuge Elesmes Airport (XME)",
"Maues Airport (MBZ)",
"Mauke Airport (MUK)",
"Frans Seda Airport (MOF)",
"Maupiti Airport (MAU)",
"Maury County Airport (MRC)",
"Maverick County Memorial International Airport (EGP)",
"Dickwella Mawella Sea Plane Base (DIW)",
"Mawlamyine Airport (MNU)",
"Maximo Gomez Airport (AVI)",
"Maxson Airfield (AXB)",
"Maxwell Air Force Base / Montgomery (MXF)",
"May Creek Airport (MYK)",
"May River (MRH)",
"Mayaguana Airport (MYG)",
"Mayajigua Airport (MJG)",
"Mayo Airport (YMA)",
"Mayor Buenaventura Vivas Airport (STD)",
"Manuel Prado Airport (MZA)",
"Mayumba (MYB)",
"Mbala Airport (MMQ)",
"Mbandaka Airport (MDK)",
"Mbanza Congo Airport (SSY)",
"Mbarara Airport (MBQ)",
"Mbeya Airport (MBI)",
"Mbuji Mayi Airport (MJM)",
"McAlester Regional Airport (MLC)",
"McCarthy Airport (MXY)",
"Mc Connell Air Force Base (IAB)",
"McCook Ben Nelson Regional Airport (MCK)",
"McEntire Air National Guard Weather Facility Base (MMT)",
"Mc Pherson Airport (MPR)",
"McArthur River Mine Airport (MCV)",
"McCall Airport (MYL)",
"McChord Field Airport (Joint Base Lewis-McChord) (TCM)",
"Mc Comb/Pike County Airport/John E Lewis Field (MCB)",
"McGrath Airport (MCG)",
"Mchauru Airport (GIT)",
"Mc Kellar-Sipes Regional Airport (MKL)",
"McKinley National Park Airport (MCL)",
"McMinn County Airport (MMI)",
"Meadow Lake Airport (YLJ)",
"Mechanics Bay Heliport (MHB)",
"Mecheria Airport (MZW)",
"Medina Airport (MND)",
"Médouneu Airport (MDV)",
"Meekatharra Airport (MKR)",
"Megeve Altiport (MVV)",
"Meghauli Airport (MEY)",
"Mehamn Airport (MEH)",
"Meixian Airport (MXZ)",
"Mejit Airfield (MJB)",
"Mekambo (MKB)",
"Mekane Selam Airport (MKS)",
"Alula Aba Nega Airport (MQX)",
"Mekoryuk Airport (MYU)",
"West Kutai Melalan Airport (GHS)",
"Melangguane Airport (MNA)",
"Moorabbin Airport Aws (MBW)",
"Melchor de Mencos Airport (MCR)",
"Melfi Airport (MEF)",
"Melinda Airstrip (MDB)",
"Melio Viana Airport (QID)",
"Memorial Field (HOT)",
"Mena Intermountain Municipal Airport (UMZ)",
"Mende-Brenoux (MEN)",
"Mendez Airport (MZD)",
"Mendi Airport (MDU)",
"Mendi Airport (NDM)",
"Menominee-Marinette Twin County Airport (MNM)",
"Menongue Airport (SPP)",
"Menyamya Airport (MYX)",
"Mercedes Airport (MDX)",
"Mercer County Airport (BLF)",
"Merdei Airport (RDE)",
"Meribel (MFX)",
"Merimbula Airport (MIM)",
"Merle K. (Mudhole) Smith Airport (CDV)",
"Merluna (MLV)",
"Merowe Airport (MWE)",
"Merowe New Airport (MWE)",
"Merrill Municipal Airport (RRL)",
"Merritt Island Airport (COI)",
"Merritt Airport (YMB)",
"Mersing Airport (MEP)",
"Merty Merty Airport (RTY)",
"Merville-Calonne Airport (HZB)",
"Mesa Del Rey Airport (KIC)",
"Meselia Airport (MFZ)",
"Mesquite Airport (MFH)",
"Metema Airport (ETE)",
"Metlakatla Seaplane Base (MTM)",
"Metro Field Airport (MTX)",
"Metropolitan Airport (PMX)",
"Mettel Field Airport (CEV)",
"Meyers Chuck Seaplane Base (WMK)",
"Miami Municipal Airport (MIO)",
"Miami Seaplane Base (MPB)",
"Miandrivazo Airport (ZVA)",
"PAF Base M.M. Alam (MWD)",
"Michael Army Airfield (Dugway Proving Ground) (DPG)",
"Rowan County Airport (SRW)",
"Greenville International Airport (GLH)",
"Mid-State Airport (PSB)",
"Middle Caicos Airport (MDS)",
"Middle Georgia Regional Airport (MCN)",
"Middlemount Airport (MMM)",
"Middleton Island Airport (MDO)",
"Middletown Regional Airport (MWO)",
"Midgard Airport (MQG)",
"Tver Migalovo, Kalinin (KLD)",
"Mikkeli Airport (MIK)",
"Miles Airport (WLE)",
"Big Piney-Marbleton Airport (BPI)",
"Milford Municipal Airport (MLF)",
"Milford Sound Airport (MFN)",
"Mili Airfield (MIJ)",
"Milingimbi Airport (MGT)",
"Millard Airport (MIQ)",
"Miller Field (VTN)",
"Millicent Airport (MLR)",
"Kabwe (QKE)",
"Millinocket Municipal Airport (MLT)",
"Milos Airport (MLO)",
"Bickerton Island Airport (BCZ)",
"Minacu Airport (MQH)",
"Minchumina Airport (LMA)",
"Minden-Tahoe Airport (MEV)",
"Mindik Airport (MXK)",
"Mindiptana Airport (MDP)",
"Mineral Wells Airport (MWL)",
"Mingan Airport (YLP)",
"Minhas Airbase (ATG)",
"Minlaton Airport (XML)",
"Minna Airport (MXJ)",
"Minnipa Airport (MIN)",
"Minto Al Wright Airport (MNT)",
"Minvoul Airport (MVX)",
"Miquelon Airport (MQC)",
"Miraflores Airport (MFS)",
"Miramar Airport (MJR)",
"Miramichi Airport (YCH)",
"Miranda Downs Airport (MWY)",
"Mirassol Airport (0)",
"Mirny Airport (MJZ)",
"Mirpur Khas Air Base (MPD)",
"Misima Island Airport (MIS)",
"Mitchell Municipal Airport (MHE)",
"Mitchell Airport (MTQ)",
"Mitchell Plateau Airfield (MIH)",
"Mitiaro Island Airport (MOI)",
"Mitiga International Airport (MJI)",
"Mittiebah Airstrip (MIY)",
"Mitzic (MZC)",
"Mitzpe Ramon Airport (MIP)",
"Miyakejima Airport (MYE)",
"Miyanmin (MPX)",
"Miyazaki Airport (KMI)",
"Mizan Teferi Airport (MTF)",
"Mkambati Airport (MBM)",
"Mkuze (MZQ)",
"Mmabatho International Airport (MBD)",
"Moabi Airport (MGX)",
"Moala Airport (MFJ)",
"Moanamani Airport (ONI)",
"Moanda (MFF)",
"Moba Airport (BDV)",
"Mobile Downtown Airport (BFM)",
"Mobridge Municipal Airport (MBG)",
"Mocimboa da Praia Airport (MZB)",
"Mococa Airport (QOA)",
"Mocopulli Airport (MHC)",
"Moengo Airstrip (MOJ)",
"Moenjodaro Airport (MJD)",
"Moffett Federal Airfield (NUQ)",
"Mogi Mirim Airport (0)",
"Mogilev Airport (MVQ)",
"Moheli Bandar Es Eslam Airport (NWA)",
"Moises Benzaquen Rengifo Airport (YMS)",
"Moises R. Espinosa Airport (MBT)",
"Mojave Airport (MHV)",
"Mojica (CSC)",
"Mokhotlong Airport (MKH)",
"Moki (MJJ)",
"Mokuti Lodge Airport (OKU)",
"La Mole - Saint-Tropez Airport (LTT)",
"Molokai Airport (MKK)",
"Moma Airport (MQJ)",
"Mombaça Airport (0)",
"Momeik Airport (MOE)",
"Momote Airport (MAS)",
"Monaco Fontvieille Heliport (MCM)",
"Monbetsu Airport (MBE)",
"Duesseldorf Moenchengladbach Airport (MGL)",
"Venustiano Carranza International Airport (LOV)",
"Mondell Field (ECS)",
"Mondol Kiri Airport (MWV)",
"Monfort Airport (MFB)",
"Mong Hsat Airport (MOG)",
"Mong Tong Airport (MGK)",
"Mongo (MVO)",
"Mongu Airport (MNR)",
"Monkey Bay Airport (MYZ)",
"Monkey Mountain Airport (MYM)",
"Monkira Airport (ONR)",
"Mono Airport (MNY)",
"Monroe County Airport (BMG)",
"Mont-Dauphin - Saint-Crepin (SCP)",
"Mont Joli Airport (YYY)",
"Mont-de-Marsan (XMJ)",
"Montague Airport-Yreka Rohrer Field (ROF)",
"Montauk Airport (MTP)",
"Montbeliard - Courcelles Aerodrome (XMF)",
"Monte Alegre Airport (MTE)",
"Monte Carmelo Airport (0)",
"Monte Caseros Aerodrome (MCS)",
"Monte Dourado Airport (MEU)",
"Monte Real Air Base (QLR)",
"Monte Santo de Minas Airport (0)",
"Monteagudo Airport (MHW)",
"Montelibano Airport (MTB)",
"Montelimar-Ancone (XMK)",
"Montenegro Airport (QGF)",
"Montepuez Airport (MTU)",
"Monterrey Airfield (MOY)",
"Montevideo-Chippewa County Airport (MVE)",
"Monticello Airport (MXC)",
"Monticello Regional Airport (MXO)",
"Montlucon - Gueret Airport (MCU)",
"Monto Airport (MNQ)",
"Montreal-Mirabel International Airport (YMX)",
"Montreal/Saint-Hubert Airport (YHU)",
"The Monument Airport (PHQ)",
"Monument Valley Airport (GMV)",
"Monywa Airport (NYW)",
"Moolawatana Airport (MWT)",
"Moomba Airport (MOO)",
"Mooraberree Airport (OOR)",
"Moore County Airport (SOP)",
"Moore-Murrell Airport (MOR)",
"Moorea-Temae Airport (MOZ)",
"CFB Moose Jaw (YMJ)",
"Moosonee Airport (YMO)",
"Mora Airport (MXX)",
"Morafenobe Airport (TVA)",
"Moranbah Airport (MOV)",
"Morava Kraljevo Airport (KVO)",
"Morawa Airport (MWB)",
"Moree Airport (MRZ)",
"Morehead Airport (MHY)",
"Moreton Airport (MET)",
"Morichal (MHF)",
"Montauban Airport (XMW)",
"Morlaix - Ploujean Airport (MXN)",
"Morney Airport (OXY)",
"Morningside Farm Airport (MEZ)",
"Mornington Island Airport (ONG)",
"Moro Airport (MXH)",
"Moroak (MRT)",
"Morobe Airport (OBM)",
"Morombe Airport (MXM)",
"Moeroen Airport (MXV)",
"Moron Air Base (OZP)",
"Morondava Airport (MOQ)",
"Morris Municipal Airport - Charlie Schmidt Field (MOX)",
"Morristown Municipal Airport (MMU)",
"Morro do Chapéu Airport (0)",
"Moruya Airport (MYA)",
"Moser Bay Seaplane Base (KMY)",
"Moses Point Airport (MOS)",
"Moshi Airport (QSI)",
"Mosjoen Airport, Kjaerstad (MJF)",
"Mosquera (MQR)",
"Rygge (RYG)",
"Mossel Bay Cape Saint Blaize (MZY)",
"Mossendjo Airport (MSX)",
"Mostaganem Airport (MQV)",
"Mosteiros Airport (MTI)",
"Mostyn Airport (MZS)",
"Mosul International Airport (OSM)",
"Mota Lava Airport (MTV)",
"Motswari Airport (MWR)",
"Motueka Aerodrome (MZP)",
"Moucha Airport (MHI)",
"Mougulu Airport (GUV)",
"Mouila (MJL)",
"Moulins-Montbeugny (XMU)",
"Moultrie Municipal Airport (MGR)",
"Moundou (MQQ)",
"Mount Aue (UAE)",
"Mount Cavenagh (MKV)",
"Mount Cook Airport (MON)",
"Mount Etjo Airport (MJO)",
"Wando Vale (MFL)",
"Mount Gordon Airport (GPD)",
"Mount Gunson Airport (GSN)",
"Mount Hagen Airport (HGU)",
"Mount Hotham Airport (MHU)",
"Mount House Airport (MHO)",
"Mount Keith Airport (WME)",
"Mount Magnet Airport (MMG)",
"Mount Pleasant Airport (MPN)",
"Mount Pleasant Regional Airport (MPS)",
"Mount Pleasant Municipal Airport (MPZ)",
"Mount Sanford Station Airport (MTD)",
"Mount Swan (MSF)",
"Mount Valley (MNV)",
"Mountain Airport (MWP)",
"Mountain Village Airport (MOU)",
"Moussoro Airport (MXR)",
"Mouyondzi Airport (MUY)",
"Moyale Airport (MYS)",
"Moyale Airport (OYL)",
"Moyo Airport (OYG)",
"Moyobamba Airport (MBP)",
"Tembagapura Timika Airport (TIM)",
"Mpanda Airport (NPY)",
"Mount Vernon Airport (MVN)",
"Mtwara Airport (MYW)",
"Moanda (MNB)",
"Muara Bungo Airport (BUU)",
"Mtubatuba (DUK)",
"Muccan Station Airport (MUQ)",
"Mucuri Airport (MVS)",
"Mudgee Airport (DGE)",
"Mueda (MUD)",
"Mueo Airport (PDC)",
"Sultan Muhammad Salahudin Airport (BMU)",
"Muhammad Taufiq Keimas Airport (TFY)",
"Mui River Airport (MUJ)",
"Muir Army Airfield (MUI)",
"Mukah Airport (MKM)",
"Mukeiras Airport (UKR)",
"Mukhaizna Airport (UKH)",
"Muko Muko Airport (MPC)",
"Lake Gregory Airport (LGE)",
"Mulatupo Airport (MPP)",
"Mulege Airport (MUG)",
"Mulga Park Airport (MUP)",
"Mulia Airport (LII)",
"Mulika Lodge Airport (JJM)",
"Mulka Airport (MVK)",
"Mullewa Airport (MXU)",
"Multan International Airport (MUX)",
"Mulu Airport (MZV)",
"Mumias Airport (MUM)",
"Munbil (LNF)",
"Munda Airport (MUA)",
"Santa Elena Airport (FRS)",
"Mundo Novo Airport (0)",
"Munduku (MDM)",
"Mungeranie Airport (MNE)",
"Latur Airport (LTU)",
"Murray Field (EKA)",
"Murray Island Airport (MYI)",
"Murrin Murrin Airport (WUI)",
"Moruroa Airport (UOA)",
"Mus Airport (MSR)",
"Muscatine Municipal Airport (MUT)",
"Musgrave Airport (MVU)",
"PAF Base Mushaf (SGI)",
"Musina(Messina) Airport (MEZ)",
"Muskoka Airport (YQA)",
"Muskrat Dam Airport (MSA)",
"Musoma Airport (MUZ)",
"Rustaq Airport (MNH)",
"Mustique Airport (MQS)",
"Mutare Airport (UTA)",
"Mutiara Airport (PLW)",
"Muting Airport (MUF)",
"Muttaburra Airport (UTB)",
"Muzaffarabad Airport (MFG)",
"Muzaffarpur Airport (MZU)",
"Liwonde Mvuu Camp Airport (VUU)",
"Mwadui Airport (MWN)",
"Mwanza Airport (MWZ)",
"Mweka Airport (MEW)",
"Myeik Airport (MGZ)",
"Mykolaiv Airport (NLV)",
"Myrhorod Airport (MXR)",
"Mys Kamenny (YMK)",
"Mysore Airport (MYQ)",
"Mzuzu Airport (ZZU)",
"N'Dele (NDL)",
"Ngaoundere Airport (NGE)",
"Nkolo Fuma Airport (NKL)",
"N'zeto Airport (ARZ)",
"Na-San Airport (SQH)",
"Nabire Airport (NBX)",
"Nacala Airport (MNC)",
"Nachingwea Airport (NCH)",
"Nadunumu Airport (NDN)",
"Nadym Airport (NYM)",
"Nadzab Airport (LAE)",
"Nafoora (NFR)",
"Naga Airport (WNP)",
"Naha Airport (NAH)",
"Nain Airport (YDP)",
"Nairobi Wilson Airport (WIL)",
"Niau Airport (NIU)",
"Nakina Airport (YQN)",
"Naknek Airport (NNK)",
"Nakuru Airport (NUU)",
"Nal Airport (BKB)",
"Namangan Airport (NMA)",
"Namatanai Airport (ATN)",
"Nambaiyufa Airport (NBA)",
"Nambucca Heads Airport (NBH)",
"Namlea Airport (NAM)",
"Namdrik Airfield (NDK)",
"Nampula Airport (APL)",
"Namrole Airport (NRE)",
"Namsang Airport (NMS)",
"Namsos Lufthavn (OSY)",
"Namtu Airport (NMT)",
"Namu Seaplane Base (ZNU)",
"Namudi Airport (NDI)",
"Namutoni Airport (NNI)",
"Nana Airport (NAZ)",
"Nanaimo Harbour Water Airport (ZNA)",
"Nanchong Gaoping Airport (NAO)",
"Nancy-Essey Airport (ENC)",
"Nanded Airport (NDC)",
"Nanga Pinoh Airport (NPO)",
"Nangade Airport (NND)",
"Benjina-Nangasuri Airport (BJK)",
"Nankishirahama Airport (SHM)",
"Nankina (NKN)",
"Nanortalik Heliport (JNN)",
"Nanping Wuyishan Airport (WUS)",
"Nanuque Airport (NNU)",
"Nanwalek Airport (KEB)",
"Nanyuki Airport (NYK)",
"Naoro Airport (NOO)",
"Napakiak Airport (WNA)",
"Napaskiak Airport (PKA)",
"Nappa Merrie Airport (NMR)",
"Napperby (NPP)",
"Napuka Airport (NAU)",
"Nara Keibane Airport (NRM)",
"Naracoorte Airport (NAC)",
"Narathiwat Airport (NAW)",
"Narrabri Airport (NAA)",
"Narrandera Airport (NRA)",
"Narrogin (NRG)",
"Narromine Airport (QRM)",
"Narsaq Heliport (JNS)",
"Narsaq Kujalleq Heliport (QFN)",
"Nartron Field (RCT)",
"Naryan Mar Airport (NNM)",
"Naval Air Station Joint Reserve Base Fort Worth (FWH)",
"Ozar Airport (ISK)",
"Natashquan Airport (YNA)",
"Natitingou (NAE)",
"Natuashish Airport (YNP)",
"Naujaat Airport (YUT)",
"Naukiti Bay Seaplane Base (NKI)",
"Naval Air Station Key West (NQX)",
"Naval Outlying Field Barin (NHX)",
"Ream Field (NRS)",
"Naval Station Mayport (NRB)",
"Naypyidaw Airport (NYT)",
"Ndalatandos Airport (NDF)",
"Ndende Airport (KDN)",
"N'Dolo Airport (NLO)",
"Necochea Airport (NEC)",
"Necocli Airport (NCI)",
"Needles Airport (EED)",
"Neftekamsk Airport (NEF)",
"Nefteyugansk Airport (NFG)",
"Negage Airport (GXG)",
"Negarbo (GBF)",
"Neghelle Airport (EGL)",
"Negril Aerodrome (NEG)",
"Nejjo Airport (NEJ)",
"Nejran Airport (EAM)",
"Nekemte Airport (NEK)",
"Nelson Lagoon Airport (NLG)",
"Nelspruit Airport (NLP)",
"Nema Airport (EMN)",
"Nemiscau Airport (YNS)",
"Nenana Municipal Airport (ENN)",
"Neom Bay Airport (NUM)",
"Neosho Hugh Robinson Airport (EOS)",
"Nepalgunj Airport (KEP)",
"Nephi Municipal Airport (NPH)",
"Grand-Bereby Airport (BBV)",
"Nervino Airport (NVN)",
"Nesson Airport (HLU)",
"Neubrandenburg Airport (FNB)",
"Neuchatel (QNC)",
"Neumuenster (EUM)",
"Nevada Municipal Airport (NVD)",
"Nevatim Air Base (VTM)",
"Nevers-Fourchambault (NVS)",
"New Amsterdam Airport (QSX)",
"New Bight Airport (TBI)",
"Cameta (CMT)",
"New Century AirCenter (JCI)",
"New Coalinga Municipal Airport (CLG)",
"New Halfa Airport (NHF)",
"New Ishigaki Airport (ISG)",
"New Laura Airport (LUT)",
"New Moon Airport (NMP)",
"New Orleans, Naval Air Station - Alvin Callender Field (NBG)",
"New Plymouth Airport (NPL)",
"New Richmond Regional Airport (RNH)",
"New River Valley Airport (PSK)",
"New Stuyahok Airport (KNW)",
"New Ulm Municipal Airport (ULM)",
"New York Skyports Inc Seaplane Base (QNY)",
"Newcastle Airport (NCS)",
"Newman Airport (ZNE)",
"Newport State Airport (NPT)",
"Newry (NRY)",
"Newtok Airport (WWT)",
"Newton City-County Airport (EWK)",
"Newton Municipal Airport (TNU)",
"Neyveli Airport (NVY)",
"Ngala Airport (NGL)",
"Ngari Gunsa Airport (NGQ)",
"Ngau Airport (NGI)",
"Ngjiva Pereira Airport (VPE)",
"Ngloram Airport (CPF)",
"Ngoma (ZGM)",
"Kirakira Airport (IRA)",
"Ngot Nzoungou Airport (DIS)",
"Ngukurr Airport (RPM)",
"Nhon Co Airport (HOO)",
"St. Catharines/Niagara District Airport (YCM)",
"Niamtougou (LRL)",
"Niaqornaarsuk Heliport (QMK)",
"Niaqornat Heliport (NIQ)",
"Nicaro Airport (ICR)",
"Nicholson Airport (NLS)",
"Majoor Henry Fernandes Airport (ICK)",
"Nightmute Airport (NME)",
"Nikolai Airport (NIB)",
"Nikolski Air Station (IKO)",
"Nikunau Airfield (NIG)",
"Nimba (NIA)",
"Ninglang Luguhu Airport (NLH)",
"Nioki Airport (NIO)",
"Niokolo-Koba (NIK)",
"Nioro Airport (NIX)",
"Niort-Souche (NIT)",
"Nipa Airport (NPG)",
"Niquelandia Airport (NQL)",
"Nissan Island Airport (IIS)",
"Njombe (JOM)",
"Nkaus Airport (NKU)",
"Nkongsamba (NKS)",
"Noatak Airport (WTK)",
"Nogales International Airport (NOG)",
"Nogales International Airport (OLS)",
"Nogliki Airport (NGK)",
"Nomad River (NOM)",
"Nomane Airport (NMN)",
"Nome Airport (OME)",
"Nondalton Airport (NNL)",
"Nonoaí Airport (0)",
"Nonouti Airfield (NON)",
"Noonkanbah Airport (NKB)",
"Noosa Airport (NSV)",
"Yahukimo Nop Goliath Airport (DEX)",
"Nordfjoerdur Airport (NOR)",
"Norden-Norddeich (NOD)",
"Nordholz Naval Airbase (NDZ)",
"Naval Station Norfolk Chambers Field (NGU)",
"Norilsk-Alykel Airport (NSK)",
"Norman Wells Airport (YVQ)",
"Norman's Cay Airport (NMC)",
"Normanton Airport (NTN)",
"Norseman Airport (NSM)",
"Norsup Airport (NUS)",
"Del Norte International Airport (NTR)",
"North Battleford Airport (YQW)",
"North Bay/Jack Garland Airport (YYB)",
"North Caicos Airport (NCA)",
"North Central State Airport (SFZ)",
"North Central West Virginia Airport (CKB)",
"North Fork Valley Airport (WPO)",
"Lilabari Airport (IXI)",
"North Las Vegas Airport (VGT)",
"Naval Air Station North Island (NZY)",
"North Perry Airport (HWO)",
"North Platte Regional Airport Lee Bird Field (LBF)",
"North Ronaldsay Airport (NRL)",
"Obre Lake/North of Sixty Airport (YDW)",
"North Spirit Lake Airport (YNO)",
"North Texas Regional Airport (PNX)",
"North West Santo Airport (OLJ)",
"Northeast Alabama Regional Airport (GAD)",
"Charles City Municipal Airport (CCY)",
"Newport State Airport (EFK)",
"Northeastern Regional Airport (EDE)",
"Northern Aroostook Regional Airport (WFK)",
"Northern Peninsula Airport (ABM)",
"Northwest Alabama Regional Airport (MSL)",
"Northwest Regional Airport Terrace-Kitimat (YXT)",
"Noervenich Air Base (QOE)",
"Norway House Airport (YNE)",
"Nosara Airport (NOB)",
"Noshahr Airport (NSH)",
"Jember Noto Hadinegoro Airport (JBB)",
"Nouakchott-Oumtounsy International Airport (NKC)",
"Ghardaia (GHA)",
"Nouna Airport (XNU)",
"Albano Machado Airport (NOV)",
"Nova Prata Airport (0)",
"Rio Turbio Airport (RYO)",
"Novo Aripuana Airport (NVP)",
"Novo Campo Airport (BCR)",
"Novo Hamburgo Airport (QHV)",
"Novo Horizonte Airport (0)",
"Novo Progresso Airport (NPR)",
"Novy Urengoy Airport (NUX)",
"Nowata Airport (NWT)",
"Naval Air Station Nowra (NOA)",
"Noyabrsk Airport (NOJ)",
"La Nubia Airport (MZL)",
"Nueva Guinea Airport (NVG)",
"Aeropuerto Nueva Hesperides (STY)",
"Nueva Loja Airport (LGQ)",
"Nuevo Casas Grandes Municipal Airport (NCG)",
"Nuguria (NUG)",
"Nuiqsut Airport (NUI)",
"Nuku Hiva Airport (NHV)",
"Nuku Airport (UKU)",
"Nukus Airport (NCU)",
"Nukutavake Airport (NUK)",
"Nukutepipi Airport (NKP)",
"Nulato Airport (NUL)",
"Numbulwar Airport (NUB)",
"Nunam Iqua Airport (SXP)",
"Nunapitchuk Airport (NUP)",
"Nunukan Airport (NNX)",
"Nusatupe Airport (GZO)",
"Cijulang Nusawiru Airport (CJN)",
"Nushki Airport (NHS)",
"Nutwood Downs (UTD)",
"Nuugaatsiaq Heliport (JUU)",
"Nuussuaq Heliport (NSQ)",
"Nyac Airport (ZNC)",
"Nyagan Airport (NYA)",
"Nyala Airport (UYL)",
"Nyeri Airport (NYE)",
"Nyingchi Mainling Airport (LZY)",
"Nyngan Airport (NYN)",
"Nyurba Airport (NYR)",
"Nzagi Airport (NZA)",
"Nzerekore Airport (NZE)",
"O'Neal Airport (OEA)",
"The O'Neill Municipal Airport (ONL)",
"Oakdale Airport (ODC)",
"Oakey Aerodrome (OKY)",
"Oamaru Airport (OAM)",
"Oban Airport (OBN)",
"Obando Airport (PDA)",
"Obano Airport (OBD)",
"Obidos Airport (OBI)",
"Obo Airport (OBX)",
"Obock Airport (OBC)",
"Robert J. Miller Air Park (MJX)",
"Ocean Falls Seaplane Base (ZOF)",
"Ocean Reef Club Airport (OCA)",
"Virginia Beach, Oceana, Naval Air Station (NTU)",
"Oconee County Regional Airport (CEU)",
"Odienne Airport (KEO)",
"Oeiras Airport (0)",
"Oenpelli Airport (OPI)",
"Oesman Sadik Airport (LAH)",
"Ofu Airport (OFU)",
"Ogden Municipal Airport (OGD)",
"Ogeranang (OGE)",
"Ogoki Airport (YOG)",
"Ogobsucum Airport (OGM)",
"Ohkay Owingeh Airport (ESO)",
"Oiapoque (OYK)",
"Okaba Airport (OKQ)",
"Okao Airport (OKV)",
"Okaukuejo (OKF)",
"Okeechobee County Airport (OBE)",
"Okha Airport (OHH)",
"Okhotsk Airport (OHO)",
"Oki Airport (OKI)",
"Okondja Airport (OKN)",
"Okoyo Airport (OKG)",
"Oksapmin Airport (OKP)",
"Oksibil Airport (OKL)",
"Gdynia-Babie Doly Airport (QYD)",
"Oktyabrskiy Airport (OKT)",
"Okushiri Airport (OIR)",
"Ol Seki Airfield (OSJ)",
"Olafsfjoerdur Airport (OFJ)",
"Olare Airfield (OLG)",
"Olavarria Airport (OVR)",
"Old Crow Airport (YOC)",
"Old Harbor Airport (OLH)",
"Olenyok Airport (ONK)",
"Olga Bay Seaplane Base (KOY)",
"OElgii Airport (ULG)",
"Agua Boa Airport (GGB)",
"Olive Branch Airport (OLV)",
"Olkiombo Airport (OLX)",
"Olney-Noble Airport (OLY)",
"Olney Municipal Airport (ONY)",
"Olomouc Neredin Airport (OLO)",
"Olsobip Airport (OLQ)",
"Olympia Regional Airport (OLM)",
"Olympic Dam Airport (OLP)",
"Olyokminsk Airport (OLZ)",
"Omak Airport (OMK)",
"Omar N Bradley Airport (MBY)",
"Omboue Hopital Airport (OMB)",
"Omega Airport (OMG)",
"Omidiyeh (OMI)",
"Omora (OSE)",
"Ondangwa Airport (OND)",
"Underkhaan (UNR)",
"Ono-I-Lau Airport (ONU)",
"Ononge Airport (ONB)",
"Onotoa Airport (OOT)",
"Onslow Airport (ONS)",
"Ontario Municipal Airport (ONO)",
"Oodnadatta Airport (ODD)",
"Forrest River Airport (FVR)",
"Opapimiskan Lake Airport (YBS)",
"Open Bay (OPB)",
"Opinaca (YOI)",
"Opuwo (OPW)",
"Oram (RAX)",
"Oran Airport (ORA)",
"Orang (RGO)",
"Orange-Caritat (XOG)",
"Orange County Airport (MGJ)",
"Orange Airport (OAG)",
"Orange Walk Airport (ORZ)",
"Orangeburg Municipal Airport (OGB)",
"Oranjemund Airport (OMD)",
"Orapa Airport (ORP)",
"Orbost Airport (RBS)",
"Orcas Island Airport (ESD)",
"Orchid Beach Airport (OKB)",
"Ord River Airport (ODR)",
"Ordos Ejin Horo Airport (DSN)",
"Orestes Acosta Airport (MOA)",
"Oria (OTY)",
"Orientos Airport (OXO)",
"Orinduik Airport (ORJ)",
"Oristano-Fenosu Airport (FNU)",
"Oriximina Airport (ORX)",
"Orland Airport (OLA)",
"Matupa Airport (MBK)",
"Ormara Airport (ORW)",
"Ormoc Airport (OMC)",
"OErnskoeldsvik Airport (OER)",
"Oro Negro Airport (CBS)",
"Orocue Airport (ORC)",
"Orsta-Volda Airport, Hovden (HOV)",
"Uruzgan (URZ)",
"Oryol Yuzhny Airport (OEL)",
"Osan Air Base (OSN)",
"Osborne Mine Airport (OSO)",
"Oschersleben Airfield (FRF)",
"Oscoda-Wurtsmith Airport (OSC)",
"Oshakati Airport (OHI)",
"Oshima Airport (OIM)",
"Oskaloosa Municipal Airport (OOA)",
"Oskarshamn Airport (OSK)",
"Osmanabad Airport (OMN)",
"Osmani International Airport (ZYL)",
"Ossima Airport (OSG)",
"Ostafyevo International Airport (OSF)",
"Acarigua (AGV)",
"Otjiwarongo (OTJ)",
"Gatineau-Ottawa Executive Airport (YND)",
"Ottawa / Rockcliffe Seaplane Base (TR7)",
"Ottawa / Rockcliffe Airport (YRO)",
"Ottumwa Industrial Airport (OTM)",
"Ouadda Airport (ODA)",
"Ouahigouya Airport (OUG)",
"Ouanda Djalle Airport (ODJ)",
"Ouango Fitini Airport (OFI)",
"Oudomsay Airport (ODY)",
"Oudtshoorn Airport (OUH)",
"Ouesso (OUE)",
"Oum el Bouaghi Airport (QMH)",
"Oum Hadjer Airport (OUM)",
"Ourilandia (OIA)",
"Ourinhos Airport (OUS)",
"Ouro Fino Airport (0)",
"Ouro Sogui Airport (MAX)",
"Outer Skerries Airport (OUK)",
"Ouvea Ouloup Airport (UVE)",
"Ouyen Airport (OYN)",
"Ouzinkie Airport (KOZ)",
"Ovda Airport (VDA)",
"AFB Overberg (OVG)",
"Owando Airport (FTX)",
"Owatonna Degner Regional Airport (OWA)",
"Owen Sound / Billy Bishop Regional Airport (YOS)",
"Oxford House Airport (YOH)",
"Oyem Airport (OYE)",
"Oyo Ollombo Airport (OLL)",
"Ozark Regional Airport (WMH)",
"Ozona Municipal Airport (OZA)",
"Klerksdorp Airport (KXE)",
"A P Hill Army Airfield (Fort A P Hill) (APH)",
"Pablo L. Sidar Airport (AZG)",
"Pacific City State Airport (PFC)",
"Pagadian Airport (PAG)",
"Page Field (FMY)",
"Paiela Airport (PLE)",
"Pajala Airport (PJA)",
"Pakhokku Airport (PKK)",
"Pakse International Airport (PKZ)",
"Pakuba Airport (PAF)",
"Pakyong Airport (PYG)",
"Pala (PLF)",
"Palacios (PCH)",
"Palacios Municipal Airport (PSX)",
"Palapye Airport (QPH)",
"Palenque International Airport (PQM)",
"Palestine Municipal Airport (PSN)",
"Palm Beach County Glades Airport (PHK)",
"Palm Beach County Park Airport (LNA)",
"Palm Beach Seaplane Base (LBH)",
"Palm Island Airport (PMK)",
"Palma Airport (LMZ)",
"Palmar Sur Airport (PMZ)",
"Palmarito Airport (PTM)",
"Palmdale United States Air Force Plant 42 Airport (PMD)",
"Palmerston North Airport (PMR)",
"Palmyra (PMS)",
"Palo Verde Airport (SRL)",
"Gorkha Palungtar Airport (GKH)",
"Pama Airport (XPA)",
"Pamol Airport (PAY)",
"Pampa Guanaco Airport (DPB)",
"Howard Air Force Base (historical) (BLB)",
"Pancevo Airport (QBG)",
"Pandie Pandie Airport (PDE)",
"Pajuostis Air Base (PNV)",
"Pangia Airport (PGN)",
"Pangnirtung Airport (YXP)",
"Pangoa Airport (PGB)",
"Pangsuma Airport (PSU)",
"Panguilemo Airport (TLX)",
"Panguitch Municipal Airport (PNU)",
"Panjgur Airport (PJG)",
"Pantnagar Airport (PGH)",
"Papa Stour Airport (PSV)",
"Paraburdoo Airport (PBO)",
"Parachinar Airport (PAJ)",
"Santa Carolina Airport (NTC)",
"Paraguaçu Airport (0)",
"Parakou Airport (PKO)",
"Param (PPX)",
"Paramakotoi (PMT)",
"San Cristobal Paramillo Airport (SCI)",
"Paranaiba (PBB)",
"Paranavai Airport (PVI)",
"Paraparaumu Airport (PPQ)",
"Parasi Airport (PRS)",
"Paratebueno Airfield (EUO)",
"Pardoo Airport (PRD)",
"Parintins Airport (PIN)",
"Paris Issy-les-Moulineaux Heliport (JDP)",
"Park Falls Municipal Airport (PKF)",
"Park Rapids Municipal-Konshok Field Airport (PKD)",
"Parker County Airport (WEA)",
"Parkes Airport (PKE)",
"Parndana (PDN)",
"Paernu Airport (EPU)",
"Parry Sound Area Municipal Airport (YPD)",
"Parry Sound/Frying Pan Island-Sans Souci Water Aerodrome (YSI)",
"Parsabad-Moghan Airport (PFQ)",
"Paruma Airport (PRR)",
"The Pas Airport (YQD)",
"Pasighat Airport (IXT)",
"Pasir Pangaraan Airport (PPR)",
"Pasni Airfield (PSI)",
"Paso Caballos Airport (PCG)",
"Paso de los Libres Airport (AOL)",
"Passikudah Helipad (PQD)",
"Pastos Bons Airport (0)",
"Pathankot Airport (IXP)",
"Pathein Airport (BSX)",
"Patos de Minas Airport (POJ)",
"Patreksfjordur Airport (PFJ)",
"Patrocínio Airport (0)",
"Pattani (PAN)",
"Naval Air Station Patuxent River (NHK)",
"Pauk Airport (PAU)",
"Paulatuk (Nora Aliqatchialuk Ruben) Airport (YPC)",
"Paulo Afonso Airport (PAV)",
"Paya Lebar Airport (QPG)",
"Payam International Airport (PYK)",
"Payson Airport (PJB)",
"Paz de Ariporo (PZA)",
"Peace River Airport (YPE)",
"Peawanuck Airport (YPO)",
"Pebane (PEB)",
"Pechora Airport (PEX)",
"Pecos Municipal Airport (PEQ)",
"Pedernales Airport (PDZ)",
"La Pedrera Airport (LPD)",
"Pedro Bay Airport (PDB)",
"Pedro Teixeira Castelo Regional Airport (JTA)",
"Peenemuende (PEF)",
"Pelaneng Airport (PEL)",
"Pelican Seaplane Base (PEC)",
"Pemba / Karume Airport (PMA)",
"Pemba Airport (POL)",
"Pembina Municipal Airport (PMB)",
"Pender Harbour Waterdrome (YPT)",
"Pendopo Airport (PDO)",
"Penneshaw Airport (PEA)",
"Penong Airport (PEY)",
"Naval Air Station Pensacola (NPA)",
"Pentagon Army Heliport (JPN)",
"Peppimenarti Airport (PEP)",
"Perai Tepuy Airport (PPH)",
"Perales Airport, El Salado (IBE)",
"Perigueux - Bassillac Airport (PGX)",
"Perito Moreno Airport (PMQ)",
"Peronne-St Quentin Airport (XSJ)",
"Perry-Foley Airport (FPY)",
"Perry Lefors Field (PPA)",
"Perry Municipal Airport (PRO)",
"Perry Island Seaplane Base (PYL)",
"Perry Stokes Airport (TAD)",
"Perryville Airport (KPV)",
"Persian Gulf International Airport (PGU)",
"Petrozavodsk (PES)",
"Petawawa Airport (YWA)",
"Peterborough Airport (YPQ)",
"Petit Jean Park Airport (MPJ)",
"Pevek Airport (PWE)",
"Phan Rang Air Base (PHA)",
"Phan Thiet Airport (PHH)",
"Phanom Sarakham Airport (PMM)",
"Phaplu Airport (PPL)",
"Phetchabun Airport (PHY)",
"Phifer Airfield (EAN)",
"Philip Billard Municipal Airport (TOP)",
"Philip Municipal Airport (PHP)",
"Phillips Army Airfield (APG)",
"Phoenix Deer Valley Airport (DVT)",
"Phrae Airport (PRH)",
"Phu Cat Airport (UIH)",
"Phu Quoc International Airport (PQC)",
"Piatã Airport (0)",
"Pickens County Airport (LQK)",
"Pickle Lake Airport (YPL)",
"Pico Airport (PIX)",
"Picos Airport (PCS)",
"Picton Aerodrome (PCN)",
"Pietermaritzburg Airport (PZB)",
"Pikangikum Airport (YPM)",
"Pike County Regional Airport - Hatcher Field (PVL)",
"Pikwitonei Airport (PIW)",
"Pilanesberg International Airport (NTY)",
"Pilot Point Airport (PIP)",
"Pilot Station Airport (PQS)",
"Alta Floresta Aeroporto (AFL)",
"Pimaga Airport (PMP)",
"Pimenta Bueno Airport (PBQ)",
"Pinal Airpark (MZJ)",
"Pinang Kampai Airport (DUM)",
"Pincher Creek Airport (WPC)",
"Pindiu Airport (PDI)",
"Pine Cay Airport (PIC)",
"Pine Island Airport (DUF)",
"Pine Ridge Airport (XPR)",
"Pinehouse Lake Airport (ZPO)",
"Pingtung North Air Force Base (PIF)",
"Pingtung South Air Force Base (PIF)",
"Pinheiro Airport (PHI)",
"Piracicaba Airport (QHB)",
"Pirapora (PIV)",
"Pirelli Sumaré Heliport (RWS)",
"Andorra - La Seu d'Urgell Airport (LEU)",
"Pitts Town Airport (PWN)",
"Butler-Graham Airport (BTP)",
"Pittsfield Municipal Airport (PSF)",
"Pitu Airport (OTI)",
"Placencia Airport (PLJ)",
"Placerville Airport (PVF)",
"Plastun Airport (TLY)",
"Platinum Airport (PTU)",
"Plato Airport (PLT)",
"Playa Baracoa Airport (UPB)",
"Playa del Carmen Airport (PCM)",
"Playa Grande Airport (PKJ)",
"Playa Samara Airport (PLD)",
"Playon Chico Airport (PYC)",
"Pleiku Airport (PXU)",
"Plettenberg Bay Airport (PBZ)",
"Tres Lagoas Airport (TJL)",
"Plovdiv Airport (PDV)",
"Plymouth Municipal Airport (PLY)",
"Po Airport (PUP)",
"Pobedilovo Airport (KVX)",
"Pocahontas Municipal Airport (POH)",
"Pocheon G 217 Airport (QJP)",
"Podkamennaya Tunguska Airport (TGP)",
"Podor (POD)",
"Pohang Airport (KPO)",
"Point Baker Seaplane Base (KPB)",
"Point Hope Airport (PHO)",
"Point Lay LRRS Airport (PIZ)",
"Lonely Air Station (LNI)",
"Point Mugu, Naval Air Warfare Center (NTD)",
"Pointe Vele Airport (FUT)",
"Points North Landing Airport (YNL)",
"Polacca Airport (PXL)",
"Polgolla Reservoir Sea Plane Base (KDZ)",
"Fort Polk, Polk AAF Ft Polk (POE)",
"Pomala Airport (PUM)",
"Pompano Beach Airpark (PPM)",
"Pompeu Airport (0)",
"Ponca City Regional Airport (PNC)",
"Pond Inlet Airport (YIO)",
"Pondicherry Airport (PNY)",
"Pondok Cabe Air Base (PCB)",
"Pongtiku Airport (TTR)",
"Ponta Do Ouro (PDD)",
"Ponta Grossa Airport (PGZ)",
"Ponta Pelada Airport (PLL)",
"Pontes e Lacerda Airport (LCB)",
"Pontoise - Cormeilles Aerodrom (POX)",
"Pope Field Airport (GFD)",
"Poplar Bluff Municipal Airport (POF)",
"Poplar Hill Airport (YHP)",
"Poplar River Airport (XPP)",
"Poplarville-Pearl River County Airport (PCU)",
"Poptun Airport (PON)",
"Porbandar Airport (PBD)",
"Pore Airport (PRE)",
"Porepunkah Airport (MBF)",
"Pormpuraaw Airport (EDR)",
"Port Alexander Seaplane Base (PTD)",
"Port Alfred Airport (AFD)",
"Port Alice Seaplane Base (PTC)",
"Port Allen Airport (PAK)",
"Port Alsworth Airport (PTA)",
"Port Augusta Airport (PUG)",
"Port Bailey Seaplane Base (KPY)",
"Port Berge Airport (WPB)",
"Port Clarence Coast Guard Station (KPC)",
"Port Graham Airport (PGM)",
"Port Harcourt City Airport (PHG)",
"Port Hardy Airport (YZT)",
"Allan J. MacEachen Port Hawkesbury Airport (YPS)",
"Port Heiden Airport (PTH)",
"Port Hope Simpson Airport (YHA)",
"Port Keats Airport (PKT)",
"Port Lincoln Airport (PLO)",
"Port Lions Airport (ORI)",
"Port McNeill Airport (YMP)",
"Port Meadville Airport (MEJ)",
"Port-Menier Airport (YPN)",
"Port Moller Airport (PML)",
"Port de Paix Airport (PAX)",
"Port Pirie Airport (PPI)",
"Port of Poulsbo Marina Moorage Seaplane Base (PUL)",
"Port Protection Seaplane Base (PPV)",
"Port Said Airport (PSD)",
"Port St Johns (JOH)",
"Port Stanley Airport (PSY)",
"Port Sudan Airport (PZU)",
"Port Walter Seaplane Base (PWR)",
"Port Williams Seaplane Base (KPR)",
"Portage Creek Airport (PCA)",
"Porterville Municipal Airport (PTV)",
"Portimao Airport (PRM)",
"Portland Airport (PTJ)",
"Portland-Troutdale Airport (TTD)",
"Porto Amboim Airport (PBN)",
"Portos Dos Gauchos (PBV)",
"Porto de Moz Airport (PTQ)",
"Porto Nacional Airport (PNB)",
"Portoroz Airport (POW)",
"Postville Airport (YSO)",
"Potchefstroom Aerodrome (PCF)",
"Poum Airport (PUV)",
"Pouso Alegre Airport (PPY)",
"Powell Lake Seaplane Base (WPL)",
"Powell Municipal Airport (POY)",
"Powell River Airport (YPW)",
"Prado Airport (PDF)",
"Prairie du Chien Municipal Airport (PCD)",
"Sa Pran Nak Airport (KKM)",
"Praslin Island Airport (PRI)",
"Pratt Municipal Airport (PTT)",
"Thbeng Meanchey Airport (OMY)",
"Preguica Airport (SNE)",
"Prentice Airport (PRW)",
"Preobrazheniye Airport (RZH)",
"Prerov Air Base (PRV)",
"Ernest A. Love Field (PRC)",
"President Obiang Nguema International Airport (GEM)",
"Rivera International Airport (RVY)",
"Presidente Dutra (PDR)",
"Presidente Venceslau Airport (0)",
"Presov Air Base (POV)",
"Northern Maine Regional Airport at Presque Isle (PQI)",
"Preston Airport (PST)",
"Pretoria Central Heliport (HPR)",
"Prieska Airport (PRK)",
"La Primavera Airport (LPE)",
"La Primavera Airport (LPE)",
"Prince Albert (Glass Field) Airport (YPA)",
"Prince Mangosuthu Buthelezi Airport (ULD)",
"Prince Rupert/Seal Cove Seaplane Base (ZSW)",
"Prince Sultan Air Base (AKH)",
"Princeton Airport (PCT)",
"Princeton Municipal Airport (PNN)",
"Princeville Airport (HPV)",
"Principe Airport (PCP)",
"Prineville Airport (PRZ)",
"Proma Heliport (PRM)",
"Prominent Hill Airport (PXH)",
"Propria (0)",
"Propriano (PRP)",
"Proserpine Airport (PPP)",
"Prospect Creek Airport (PPC)",
"Provideniya Bay Airport (PVS)",
"Pryor Field (DCU)",
"Pu'er Simao Airport (SYM)",
"Puas Airport (PUA)",
"Pucon Airport (ZPC)",
"Pueblo Memorial Airport (PUB)",
"Puerto Berrio Airport (PBE)",
"Puerto Cabezas Airport (PUZ)",
"Puerto Deseado Airport (PUD)",
"Puerto Jimenez Airport (PJM)",
"Puerto Leda Airport (PBT)",
"Puerto Lempira Airport (PEU)",
"Puerto Nare Airport (NAR)",
"Puerto Obaldia Airport (PUE)",
"Puerto Paez Airport (PPZ)",
"Puerto Rico Airport (PCC)",
"Puerto Rico Airport (PUR)",
"Puerto La Victoria Airport (PCJ)",
"Puka Puka Airport (PKP)",
"Pukaki Airport (TWZ)",
"Pukapuka Island (PZK)",
"Pukarua Airport (PUK)",
"Pukatawagan Airport (XPK)",
"Pangkor Airport (PKG)",
"Pulau Panjang Airport (PPJ)",
"Tioman Airport (TOD)",
"Pullman-Moscow Regional Airport (PUW)",
"Pumani (PMN)",
"Punia Airport (PUN)",
"Punta Abreojos Airport (AJS)",
"Punta Alegre Airport (UPA)",
"Punta Chivato Airport (PCV)",
"Punta Colorada Airstrip (PCO)",
"Punta Gorda Airport (PND)",
"Punta de Maisi Airport (UMA)",
"Pupelde (ZUD)",
"General Sudirman Airport (PWL)",
"Putao Airport (PBU)",
"Putumayo Airport (PYO)",
"Puvirnituq Airport (YPX)",
"Le Puy - Loudes Airport (LPY)",
"Pweto Airport (PWO)",
"Pyay Airport (PRU)",
"Qacha's Nek Airport (UNE)",
"Qala i Naw Airport (LQN)",
"Qamdo Bangda Airport (BPX)",
"Kamishli (KAC)",
"Qarn Alam Airport (RNM)",
"Qasigiannguit Heliport (JCH)",
"Qassimiut Heliport (QJH)",
"Qayyarah West (RQW)",
"Qazvin Airport (GZW)",
"Qeqertaq Heliport (PQT)",
"Qeqertarsuaq Heliport (JGO)",
"Dayrestan Airport (GSM)",
"Qianjiang Wulingshan Airport (JIQ)",
"Qiemo Airport (IQM)",
"Qikiqtarjuaq Airport (YVM)",
"Qingyang Airport (IQN)",
"Qinhuangdao Beidaihe Airport (BPE)",
"Qinhuangdao Shanhaiguan Airport (SHP)",
"Qionghai Bo'ao Airport (BAR)",
"Qiqihar Sanjiazi Airport (NDG)",
"Qishn Airport (IHN)",
"Quakertown Airport (UKT)",
"Qualicum Beach Airport (XQU)",
"Quantico Marine Corps Airfield / Turner Field (NYG)",
"Quaqtaq Airport (YQC)",
"Quartz Creek Airport (JLA)",
"Queenstown Airport (UEE)",
"Queenstown Airport (UTW)",
"Quelimane Airport (UEL)",
"Quepos La Managua Airport (XQP)",
"Quesnel Airport (YQZ)",
"Quetta International Airport (UET)",
"Quezaltenango (AAZ)",
"Quiche Airport (AQB)",
"Quillayute Airport (UIL)",
"Quilpie Airport (ULP)",
"Quince Air Base (UMI)",
"Quinhagak Airport (KWN)",
"Quirindi Airport (UIR)",
"Quonset State Airport (NCO)",
"Qurghonteppa International Airport (KQT)",
"Quthing Airport (UTG)",
"RAAF Base Curtin (DCN)",
"RAAF Base Richmond (XRH)",
"Rabaraba (RBP)",
"Rabi Airport (RBI)",
"Rach Gia Airport (VKG)",
"Gameti/Rae Lakes Airport (YRA)",
"Akrotiri Airfield (AKT)",
"RAF Ascension Island (ASI)",
"RAF Benson (BEX)",
"RAF Brize Norton (BZZ)",
"Coningsby Royal Air Force Base (QCY)",
"Fairford Royal Air Force Base (FFD)",
"Honington Royal Air Force Base (BEQ)",
"Kinloss (FSS)",
"Leuchars (ADX)",
"RAF Linton-On-Ouse (HRT)",
"Lossiemouth (LMO)",
"RAF Marham (KNF)",
"RAF Odiham (ODH)",
"RAF Scampton (SQZ)",
"RAF Sculthorpe (FKH)",
"RAF Waddington (WTN)",
"Wyton Royal Air Force Base (QUY)",
"Rafael Cabrera Airport (GER)",
"Rafaela Airport (RAF)",
"Rafai Airport (RFA)",
"Rafha Domestic Airport (RAH)",
"Rafsanjan Airport (RJN)",
"Raglan Airfield (RAG)",
"Raiatea Airport (RFP)",
"Rainbow Lake Airport (YOP)",
"Raivavae Airport (RVV)",
"Raja Bhoj International Airport (BHO)",
"Rajahmundry Airport (RJA)",
"Rajbiraj Airport (RJB)",
"Rajkot Airport (RAJ)",
"Rajouri Airport (RJI)",
"Rakanda (RAA)",
"Raleigh County Memorial Airport (BKW)",
"Ralph M Calhoun Memorial Airport (TAL)",
"Ralph Wenz Field (PWY)",
"Ramata Airport (RBV)",
"Ramechhap Airport (RHP)",
"Ramingining Airport (RAM)",
"Eilat/Ramon International Airport (ETM)",
"Mitspe-Ramon Airport (MIP)",
"Rampart Airport (RMP)",
"Ramsar Airport (RZR)",
"Ranai Airport (NTX)",
"Ranau Airport (RNU)",
"Rand Airport (QRA)",
"Randolph Air Force Base (RND)",
"Range Regional Airport (HIB)",
"Ranger Municipal Airport (RGR)",
"Rangiroa Airport (RGI)",
"Rankin Inlet Airport (YRT)",
"Ranong Airport (UNN)",
"Rar Gwamar Airport (DOB)",
"Raroia Airport (RRR)",
"Al Hamra Seaplane Base (RHR)",
"Ratanakiri Airport (RBE)",
"Ratnagiri Airport (RTC)",
"Raton Municipal Airport/Crews Field (RTN)",
"Raufarhoefn Airport (RFN)",
"Ravensthorpe (RVT)",
"Rawalakot Airport (RAZ)",
"Rawlins Municipal Airport (RWL)",
"Reales Tamarindos Airport (PVO)",
"Reao Airport (REA)",
"Rebun Airport (RBJ)",
"Rechlin-Laerz Airfield (REB)",
"Reconquista Airport (RCQ)",
"Red Bluff Municipal Airport (RBL)",
"Red Devil Airport (RDV)",
"Red Dog Airport (RDB)",
"Red Lake Airport (YRL)",
"Red Sucker Lake Airport (YRS)",
"Redcliffe Airport (RCL)",
"Redencao Airport (RDC)",
"Redhill Aerodrome (KRH)",
"Redstone Army Airfield (HUA)",
"Redwood Falls Municipal Airport (RWF)",
"Reese Airpark (REE)",
"Refuge Cove Seaplane Base (YRC)",
"Regina Airport (REI)",
"Las Limas Reginaldo Hammer Airport (LLH)",
"Region de Murcia International Airport (RMU)",
"Reivilo (RVO)",
"Reko Diq Airport (REQ)",
"Takengon Rembele Airport (TXE)",
"Rene Mouawad Air Base (KYE)",
"Renmark Airport (RMK)",
"Rennell/Tingoa Airport (RNL)",
"Goodland Municipal Airport (GLD)",
"Rentschler Heliport (EHT)",
"Resende Airport (REZ)",
"Resolute Bay Airport (YRB)",
"Retalhuleu Airport (RER)",
"Revelstoke Airport (YRV)",
"Rewa Chorhata Airport (REW)",
"Rexburg Airport (RXE)",
"Reyes Murillo Airport (NQU)",
"Reyes Airport (REY)",
"Reykholar Airport (RHA)",
"Reykjahlid Myvatn Airport (MVA)",
"Rhinelander-Oneida County Airport (RHI)",
"Riachão Airport (0)",
"El Salvador (ESR)",
"Rice Lake, Rice Lake Regional-Carl's Field Airport (RIE)",
"Rice Lake Airport (RIE)",
"Richard B Russell Airport (RMG)",
"Richard I. Bong Airport (SUW)",
"Richard Lloyd Jones Jr Airport (RVS)",
"Richard-Toll (RDT)",
"Richards Bay Airport (RCB)",
"Richfield Municipal Airport (RIF)",
"Richmond Airport (RCM)",
"Richmond Municipal Airport (RID)",
"Riesa Goehlis Airport (IES)",
"Rieti Airport (QRT)",
"Olafsvik (OLI)",
"Rigolet Airport (YRG)",
"Rimatara Airport (RMT)",
"Rimouski Airport (YXK)",
"Rincon De Los Sauces Airport (RDS)",
"Ringgi Cove Airport (RIN)",
"Rinkenberger RLA Airport (BDF)",
"Rio Amazonas Airport (PTZ)",
"Rio Branco (RBR)",
"Rio Claro Airport (QIQ)",
"Del Rio International Airport (DRT)",
"Rio Frio / Progreso Airport (RFR)",
"Rio Grande Regional Airport (RIG)",
"Rio Mayo Airport (ROY)",
"Rio Paranaíba Airport (0)",
"River Cess (RVC)",
"Rivers Inlet Waterdrome (YRN)",
"Malelane Airport (LLE)",
"Riviere-du-Loup Airport (YRI)",
"Riyadh Air Base (XXN)",
"Riyan International Airport (RIY)",
"Riyuetan (Sun Moon Lake) Heliport (SMT)",
"Butterworth Airport (BWH)",
"RNAS Yeovilton (YEO)",
"Ohakea Aerodrome (OHA)",
"Roanne - Renaison Airport (RNE)",
"Robe Airport (GOB)",
"Roben-Hood Airport (WBR)",
"Robert (Bob) Curtis Memorial Airport (ORV)",
"Robert F. Swinnie Airport (ADR)",
"Robert S Kerr Airport (RKR)",
"Roberts Army Heliport (SYL)",
"Robertson Airfield (ROD)",
"Roberval Airport (YRJ)",
"Robinhood Airport (ROH)",
"Robinson River (RRV)",
"Robinvale Airport (RBC)",
"Robore Airport (RBO)",
"La Roche-sur-Yon - Les Ajoncs (EDM)",
"Rochefort - Saint-Agnant (RCO)",
"Rock Hill, Rock Hill - York County Airport (RKH)",
"Rock Sound International Airport (RSD)",
"Rockhampton Downs Airport (RDA)",
"Rockwood Municipal Airport (RKW)",
"Rocky Mountain House Airport (YRM)",
"Rocky Mountain Metropolitan Airport (BJC)",
"Rodelillo Airport (VAP)",
"Roebourne Airport (RBU)",
"Rogers Municipal Airport (ROG)",
"Roi Et Airport (ROI)",
"Rokeby (RKY)",
"Rokot Airport (RKI)",
"Rokot Airport (RKO)",
"Rolla Downtown Airport (RLA)",
"Rolla National Airport (VIH)",
"Rolpa Airport (RPA)",
"Roma Airport (RMA)",
"La Romaine Airport (ZGS)",
"Rome State Airport (REO)",
"Araxa Airport (AAX)",
"La Ronge (Barber Field) Airport (YVC)",
"Rongelap Airport (RNP)",
"Refugio Rooke Field Airport (RFG)",
"Roosevelt Municipal Airport (ROL)",
"Roper Bar Airport (RPB)",
"Roper Valley (RPV)",
"Roros Airport (RRS)",
"Rorvik Airport, Ryum (RVK)",
"Rosario Seaplane Base (RSJ)",
"Roscoe Turner Airport (CRX)",
"Roscommon County-Blodgett Memorial Airport (HTL)",
"Rose Bay Seaplane Base (RSE)",
"Roseau Municipal Airport/Rudy Billberg Field (ROX)",
"Roseberth Airport (RSB)",
"Roseburg Regional Airport (RBG)",
"Rosecrans Memorial Airport (STJ)",
"Rosella Plains (RLP)",
"Rosita Airport (RFS)",
"Ross River Airport (XRR)",
"Rost Flyplass (RET)",
"Roswell Industrial Air Center Airport (ROW)",
"Oecusse Airport (OEC)",
"Naval Station Rota (ROZ)",
"Rota International Airport (ROP)",
"Roti Airport (RTI)",
"Rottnest Island Airport (RTS)",
"Rotuma (RTA)",
"Rouen Airport (URO)",
"Round Lake (Weagamow Lake) Airport (ZRJ)",
"Roundup Airport (RPX)",
"Rourkela Airport (RRK)",
"Rouses Point Seaplane Base (RSX)",
"Rouyn-Noranda Airport (YUY)",
"Roxas (RXS)",
"Roy Hill Station Airport (RHL)",
"Roy Hurd Memorial Airport (MIF)",
"Roy Otten Memorial Airfield (VRS)",
"Lower Zambezi Royal Airport (RYL)",
"Royan - Medis Aerodrome (RYN)",
"Rubelsanto Airport (RUV)",
"Ruben Cantu Airport (SYP)",
"Ruby Airport (RBY)",
"Rugao Air Base (RUG)",
"Rughenda Airport (RUE)",
"Ruhengeri Airport (RHG)",
"Chaurjahari Airport (RUK)",
"Port Nelson Airport (RCY)",
"Rumbek Airport (RBX)",
"Rumginae Airport (RMN)",
"Rumjatar Airport (RUM)",
"Rundu Airport (NDU)",
"Ruoqiang Loulan Airport (RQA)",
"Rupsi India Airport (RUP)",
"Rurrenabaque (RBQ)",
"Rurutu Airport (RUR)",
"Russell Municipal Airport (RSL)",
"Russian Mission Airport (RSH)",
"Ruston Regional Airport (RSN)",
"Ruti (RUU)",
"Rutland Plains Airport (RTP)",
"Rutland - Southern Vermont Regional Airport (RUT)",
"Ryans Creek Aerodrome (SZS)",
"Rzeszów Airport (RZE)",
"Saarloq Heliport (QOQ)",
"Saattut Heliport (SAE)",
"Sabadell Airport (QSA)",
"Sabah (SBV)",
"Sebha (SEB)",
"Sabi Sabi Airport (GSS)",
"Sable Island Airport (YSA)",
"Tardamu Airport (SAU)",
"Sabzevar Airport (AFZ)",
"Sacheon Airport (HIN)",
"Sachigo Lake Airport (ZPB)",
"Sachs Harbour (David Nasogaluak Jr. Saaryuaq) Airport (YSY)",
"Sacramento Airport (0)",
"Sadah Airport (SYE)",
"Sado Airport (SDS)",
"Daqing Sartu Airport (DQA)",
"Safia (SFU)",
"Saginaw Seaplane Base (SGW)",
"Sahabat [Sahabat 16] Airport (SXS)",
"Sahand Airport (ACP)",
"Sahiwal Airport (SWN)",
"Saibai Island Airport (SBR)",
"Saidor Airport (SDI)",
"Saidpur Airport (SPD)",
"Saidu Sharif Airport (SDT)",
"St. Catherine International Airport (SKV)",
"Saint-Georges-de-l'Oyapock Airport (OYP)",
"Saint John Airport (YSJ)",
"Saint-Laurent-du-Maroni Airport (LDX)",
"Saint-Louis (XLS)",
"Saint-Yan Airport (SYT)",
"Phetchabun Airport (PHY)",
"Sakkyryr Airport (SUK)",
"Sakon Nakhon Airport (SNO)",
"Salamanca Airport (SLM)",
"Salamo Airport (SAM)",
"Saldanha /Vredenburg Airport (SDB)",
"Salem-Leckrone Airport (SLO)",
"Salem Airport (SXV)",
"FOB Salerno (OLR)",
"Harriet Alexander Field Airport (SLT)",
"Salima (LMB)",
"Salina Cruz Airport (SCX)",
"Salina-Gunnison Airport (SBO)",
"Salina Municipal Airport (SLN)",
"Sallanches - Mont-Blanc (XSN)",
"Salluit Airport (YZG)",
"Salt Cay Airport (SLX)",
"Salton Sea Airport (SAS)",
"Sam Mbakwe International Airport (QOW)",
"Sam Neua Airport (NEU)",
"Samana El Portillo Airport (EPS)",
"Tanandava (TDV)",
"Sambailo Airport (SBI)",
"Sambava Airport (SVB)",
"Samberigi Airport (SWE)",
"Sambu Airport (SAX)",
"Samjiyon Airport (YJS)",
"Sampit H. Asan Airport (SMQ)",
"Sampson County Airport (CTZ)",
"Samuels Field (BRY)",
"San Andros Airport (SAQ)",
"Comitan Copalar Airport (CJT)",
"Juan Vicente Gomez International Airport (SVZ)",
"San Bernardo Airport (MMP)",
"San Blas Airport (NBL)",
"San Carlos (NCR)",
"San Cristobal Airport (SCY)",
"San Domino Island Heliport (TQR)",
"San Felipe International Airport (SFH)",
"Las Flecheras Airport (SFD)",
"San Ignacio de Moxos Airport (SNM)",
"San Ignacio Airfield (SGM)",
"San Ignacio Town Airstrip (CYD)",
"San Isidro de El General Airport (IPZ)",
"Murcia-San Javier Airport (MJV)",
"San Javier Airport (SJV)",
"San Joaquin Airport (SJB)",
"San Jose De Chiquitos (SJS)",
"San Jose Airport (GSJ)",
"San Jose Island Airport (SIC)",
"San Jose Airport (SJI)",
"San Juan Aposento Airport (APE)",
"San Juan Del Cesa (SJH)",
"San Juan De La Maguana Airport (SJM)",
"San Juan Airport (SJA)",
"San Juan Uganik Seaplane Base (UGI)",
"San Juan D Ur (SJR)",
"San Julian Air Base (SNJ)",
"San Luis Airport (IPI)",
"San Luis de Palenque Airport (SQE)",
"San Luis Rio Colorado Airport (UAC)",
"San Marcos (SRS)",
"San Matias Airport (MQK)",
"San Miguel (NMG)",
"San Nicolas de Bari Airport (QSN)",
"San Nicolas Airport (RIM)",
"San Pedro Airport (BZA)",
"San Pedro de Uraba Airport (NPU)",
"San Pedro Airport (SPY)",
"San Quintin Military Airstrip (SNQ)",
"San Rafael Airport (LOB)",
"San Ramon Airport (SRD)",
"San Salvador de Paul Airport (SVV)",
"El Tigre San Tome Airport (SOM)",
"San Vicente Airport (SWL)",
"San Vito De Java Airport (TOO)",
"Sanandaj Airport (SDG)",
"Sancti Spiritus Airport (USS)",
"Sandcreek (SDC)",
"Sand Point Airport (SDP)",
"Sandakan Airport (SDK)",
"Sandane / Anda (SDN)",
"Sanday Airport (NDY)",
"Sanderson Field (SHN)",
"Sandnessjoen Airport, Stokka (SSJ)",
"Sandringham Airfield (SRM)",
"Sandspit Airport (YZP)",
"Sandstone Airport (NDS)",
"Sandy Lake Airport (ZSJ)",
"Sanfebagar Airport (FEB)",
"Sanford Seacoast Regional Airport (SFM)",
"Sanga Sanga Airport (TWT)",
"Sanggata/Sangkimah Airport (SGQ)",
"Sangir Airport (SAE)",
"Danilo Atienza Air Base (SGL)",
"Sanikiluaq Airport (YSK)",
"Şanlıurfa Airport (SFQ)",
"Santa Ana Airport (CRC)",
"Santa Ana Airport (NNB)",
"Santa Ana Airport (SQB)",
"Santa Ana (SBL)",
"Santa Barbara de Barinas Airport (SBB)",
"Iscuande (ISD)",
"Santa Barbara Zulia Airport (STB)",
"Durazno (DZO)",
"Santa Cruz Air Force Base (SNZ)",
"Santa Cruz do Sul Airport (CSU)",
"Santa Cruz/Graciosa Bay/Luova Airport (SCZ)",
"Santa Cruz Airport (RZA)",
"Santa Cruz Airport (STU)",
"Santa Cruz Island Airport (SZN)",
"Santa Elena de Uairen Airport (SNV)",
"Santa Isabel do Morro Airport (IDO)",
"Serra Talhada/Santa Magalhaes Airport (SET)",
"Santa Maria da Vitória Airport (0)",
"Santa Maria Airport (SMG)",
"Santa Monica Municipal Airport (SMO)",
"Santa Paula Airport (0)",
"Santa Rosa Regional Airport (ETR)",
"Santa Rosa Airport (SRA)",
"Santa Rosa De Yacuma Airport (SRB)",
"Santa Rosalia Airport (SSL)",
"Santa Teresita (SST)",
"Santa Terezinha Municipal Airport (JCB)",
"Santa Terezinha Airport (STZ)",
"Santa Vitoria do Palmar Airport (CTQ)",
"Santana do Araguaia Airport (CMP)",
"Santana Ramos (SRO)",
"Aeropuerto Santiago Perez Quiroz (AUC)",
"Santiago Vila Airport (GIR)",
"Santo Angelo Airport (GEL)",
"Sao Borja Joao Manoel Airport (JBS)",
"Sao Felix do Araguaia Airport (SXO)",
"Sao Felix do Xingu Airport (SXX)",
"Sao Filipe Airport (SFL)",
"São Gabriel Airport (0)",
"Sao Gabriel Da Cachoeira Airport (SJL)",
"São Joaquim Airport (0)",
"São Joaquim da Barra Airport (0)",
"Sao Jorge Airport (SJZ)",
"São José Airport (0)",
"Sao Lourenco do Sul Airport (SQY)",
"Sao Lourenco Airport (SSO)",
"Sao Mateus Airport (SBJ)",
"Sao Miguel do Araguaia Airport (SQM)",
"Sao Miguel do Oeste Airport (SQX)",
"São Sepé Airport (0)",
"Sapmanga (SMH)",
"Saposoa Airport (SQU)",
"Saqqaq Heliport (QUP)",
"Sara Airport (SSR)",
"Sarakhs (CKT)",
"Sarandi Airport (0)",
"Saravane (VNA)",
"Sardeh Band Airport (SBF)",
"Sarh Airport (SRH)",
"Sarmi Orai Airport (ZRM)",
"La Sarre Airport (SSQ)",
"Sarteneja Airport (SJX)",
"Sary Su Airport (HRC)",
"Sarzana / Luni (QLP)",
"Sasereme Airport (TDS)",
"Saskatoon John G. Diefenbaker International Airport (YXE)",
"Saskylakh Airport (SYS)",
"Sassandra Airport (ZSS)",
"Sasstown Airport (SAZ)",
"Satenas (FUP)",
"Satna Airport (TNI)",
"Satwag Airport (SWG)",
"Saudarkrokur Airport (SAK)",
"Saufley Field Naval Outlying Field (NUN)",
"Sauel Airport (XAU)",
"Sault Ste Marie Municipal Airport (SSM)",
"Olilit-Saumlaki Airport (SXK)",
"Sauren Airport (SXW)",
"Saurimo Airport (VHC)",
"Kihihi Airport (KHX)",
"Savannakhet Airport (ZVK)",
"Save Airport (SVF)",
"Savissivik Heliport (SVR)",
"Savoonga Airport (SVA)",
"Savusavu Airport (SVU)",
"Savuti Airport (SVT)",
"Sawan Airport (RZS)",
"Sawan Airport (RZS)",
"Sawyer County Airport (HYR)",
"Sayaboury Airport (ZBY)",
"Scammon Bay Airport (SCM)",
"Scandinavian Mountains Airport (SCR)",
"Scarlett Martinez International Airport (RIH)",
"Scatsta Airport (SCS)",
"Schefferville Airport (YKL)",
"Schenck Field Airport (ICL)",
"Schleswig Air Base (WBG)",
"Schoolcraft County Airport (ISQ)",
"Schwerin-Parchim International Airport (SZW)",
"Scone Airport (NSO)",
"MidAmerica St. Louis Airport (BLV)",
"Scottsdale Airport (SCF)",
"Scribner State Airport (SCB)",
"Searcy Municipal Airport (SRC)",
"Searle Airport (OGA)",
"Sebba Airport (XSE)",
"Sechelt-Gibsons Airport (YHS)",
"Secunda Airport (ZEC)",
"Sedalia Memorial Airport (DMO)",
"Sedona Airport (SDX)",
"Sege Airport (EGM)",
"Seguela Airport (SEO)",
"Sehonghong Airport (SHK)",
"Sehulea Airport (SXH)",
"Sehwan Sharif Airport (SYW)",
"Sei Bati Airport (TJB)",
"Seiyun Airport (GXF)",
"Sekakes Airport (SKQ)",
"Selawik Airport (WLK)",
"Selayar Haji Aroeppala Airport (KSR)",
"Selbang (SBC)",
"Seldovia Airport (SOV)",
"Selebi Phikwe (PKW)",
"Selfs Airport (MMS)",
"Selibaby Airport (SEY)",
"Sematan Airport (BSE)",
"Semera Airport (SZE)",
"Semnan Municipal Airport (SNX)",
"Semonkong Airport (SOK)",
"Semporna Airport (SMM)",
"Sena Madureira Airport (ZMD)",
"Petrolina (PNZ)",
"Sao Paulo de Olivenca Airport (OLC)",
"Senanga (SXG)",
"Sangapi Airport (SGK)",
"Senggeh Airport (SEH)",
"Senggo Airport (ZEG)",
"Senipah Heliport (SZH)",
"Seosan Airport (HMY)",
"Seoul Air Base (SSN)",
"Sepik Plains (SPV)",
"Sept-Iles Airport (YZV)",
"Sepulot Airport (SPE)",
"Sequim Valley Airport (SQV)",
"Chapeco Airport (XAP)",
"Sermiligaaq Heliport (SGG)",
"Seronera Airport (SEU)",
"Sertanópolis Airport (0)",
"Serui (ZRI)",
"Sesheke Airport (SJQ)",
"Seshutes Airport (SHZ)",
"Sesriem Airport (SZM)",
"Sette Cama Airport (ZKM)",
"Seunagan Airport (MEQ)",
"Severo-Evensk Airport (SWV)",
"Seward Airport (SWD)",
"Shafter / Kern County Airport (MIT)",
"Shageluk Airport (SHX)",
"Shah Makhdum Airport (RJH)",
"PAF Base Shahbaz (JAG)",
"Nawabshah Airport (WNS)",
"Shahid Ashrafi Esfahani Airport (KSH)",
"Yazd Airport (AZD)",
"Shahrekord Airport (CQD)",
"Shahrud Airport (RUD)",
"Shaikh Zayed International Airport (RYK)",
"Shakawe Airport (SWX)",
"Shakhtyorsk Airport (EKS)",
"Shakiso Airport (SKR)",
"Shaktoolik Airport (SKK)",
"Shalz Field (CBK)",
"Shamattawa Airport (ZTM)",
"Shamshernagar Airport (ZHM)",
"Zhengzhou Shangjie Airport (HSJ)",
"Shangrao Sanqingshan Airport (SQD)",
"Forward Operating Base Shank (OAA)",
"Shanshan Airport (SXJ)",
"Shaoguan Guitou Airport (HSC)",
"Shaoyang Wugang Airport (WGN)",
"Sharana (OAS)",
"Shark Bay Airport (MJK)",
"Sharp County Regional Airport (CKK)",
"Sharpe Field (TGE)",
"Sharurah Airport (SHW)",
"Shaw River Airport (SWB)",
"Sanming Shaxian Airport (SQJ)",
"Shay Gap Airport (SGP)",
"Sheboygan County Memorial Airport (SBM)",
"Sheep Mountain Airport (SMU)",
"Sheghnan Airport (SGA)",
"Shelby Airport (SBX)",
"Illawarra Regional Airport (WOL)",
"Shennongjia Hongping Airport (HPG)",
"Shepparton Airport (SHT)",
"Sher-Wood Airport (PWD)",
"Sherbro International Airport (BTE)",
"Sherbrooke Airport (YSC)",
"Sherman Army Airfield (FLV)",
"Shigatse Peace Airport (RKZ)",
"Shilavo Airport (HIL)",
"Shillong Airport (SHL)",
"Shimla Airport (SLV)",
"Shindand Air Base (OAH)",
"Shinyanga Airport (SHY)",
"Shirdi Airport (SAG)",
"Shire Inda Selassie Airport (SHC)",
"Shiringayoc Airport (SYC)",
"Shishmaref Airport (SHH)",
"Shively Field (SAA)",
"Shiyan Wudangshan Airport (WDS)",
"Show Low Municipal Airport (SOW)",
"Shumba Airport (JAE)",
"Shun Tak Heliport (HHP)",
"Shungnak Airport (SHG)",
"Salmon Arm Airport (YSN)",
"Shute Harbour Airport (JHQ)",
"Sialkot Airport (SKT)",
"Sialum Airport (SXA)",
"Sayak Airport (IAO)",
"Siassi Airport (SSS)",
"Siauliai International Airport (SQQ)",
"Sibi Airport (SBQ)",
"Parapat Sibisa Airport (SIW)",
"Sibiti Airport (SIB)",
"Sibu Airport (SBW)",
"Dumaguete-Sibulan Airport (DGT)",
"Sicogon Island (ICO)",
"Bizerte-Sidi Ahmed Air Base (QIZ)",
"Sidi Barrani (SQK)",
"Sidi Bel Abbes Airport (BFW)",
"Sidney Municipal Airport (SNY)",
"Sidney-Richland Municipal Airport (SDY)",
"Sidney Municipal Airport (SXY)",
"Siegerland Airport (SGE)",
"Ampugnano Airport (SAY)",
"Sierra Blanca Regional Airport (RUI)",
"Sierra Grande Airport (SGV)",
"Sierra Maestra Airport (MZO)",
"Sierra Vista Municipal Libby Army Air Field (FHU)",
"Sievierodonetsk Airport (SEV)",
"Sigiriya Airport (GIU)",
"Siglufjordur Airport (SIJ)",
"Sigonella Naval Air Station (NSY)",
"Siguanea Airport (SZJ)",
"Siguiri Airport (GII)",
"Sihanoukville International Airport (KOS)",
"Sikasso Airport (KSS)",
"Sikorsky Heliport (JSD)",
"Sila Airport (SIL)",
"Silampari Airport (LLJ)",
"Silangit Airport (DTB)",
"Silchar Airport (IXS)",
"Silgadi Doti Airport (SIH)",
"Silur Airport (SWR)",
"Silva Bay Seaplane Base (SYF)",
"Silver Creek Airport (SVK)",
"Silver Plains Airport (SSP)",
"Sim (SMJ)",
"Simara Airport (SIF)",
"Simbai Airport (SIM)",
"Simberi Island (NIS)",
"Simenti Airport (SMY)",
"Simikot Airport (IMK)",
"Simmons Army Airfield (FBG)",
"Simon Mwansa Kapwepwe International Airport (NLA)",
"Sinak (NKD)",
"Sindhri Airport (MPD)",
"Singita Safari Lodge Airport (SSX)",
"Singleton Airport (SIX)",
"Sintang (SQG)",
"Siocon Airport (XSO)",
"Siorapaluk Heliport (SRK)",
"Sioux Lookout Airport (YXL)",
"Sipitang Airport (SPT)",
"Sir Abubakar Tafawa Balewa International Airport (BCU)",
"Sir Bani Yas Island Airport (XSB)",
"Belize City Municipal Airport (TZA)",
"Sir Gaetan Duval Airport (RRG)",
"Sir Seretse Khama International Airport (GBE)",
"Sirjan Airport (SYJ)",
"Sirri Island Airport (SXI)",
"Sishen Airport (SIS)",
"Siskiyou County Airport (SIY)",
"Sissano (SIZ)",
"Sitiawan Airport (SWY)",
"Akyab (AKY)",
"Siuna (SIU)",
"Siwa Oasis North Airport (SEW)",
"Siwea Airport (SWE)",
"Siwo Airport (EAE)",
"Skagit Regional Airport (MVW)",
"Skagway Airport (SGY)",
"Skardu Airport (KDU)",
"Skeldon Airport (SKM)",
"Skien Airport (SKE)",
"Skiros Airport (SKU)",
"Skive Airport (SQW)",
"Rosh Pinah Airport (RHN)",
"Skoevde Airport (KVB)",
"Vojens Airport (SKS)",
"Skukuza Airport (SZK)",
"Skwentna Airport (SKW)",
"Skylark Field Airport (ILE)",
"Sky Park Airport (BTF)",
"Slayton Municipal Airport (NSL)",
"Sleetmute Airport (SLQ)",
"Smara Airport (SMW)",
"Smith Point Airport (SHU)",
"Smith Field (SLG)",
"Smithers Airport (YYD)",
"Smithton Airport (SIO)",
"Smolensk North Airport (LNX)",
"Snake Bay Airport (SNB)",
"Snap Lake Airport (YNX)",
"Soalala Airport (DWB)",
"Sobral Airport (QBX)",
"Sóc Trăng Airport (SOA)",
"Socorro Municipal Airport (ONM)",
"Socotra (SCT)",
"Socrates Mariani Bittencourt Airport (BMS)",
"Canavieiras Airport (CNV)",
"Sodankylae Airfield (SOT)",
"Soddu Airport (SXU)",
"Soderhamn Airport (SOO)",
"Soewondo Air Force Base (MES)",
"Sogndal Airport (SOG)",
"Sohar Airport (OHS)",
"Bondoukou (BDK)",
"Sola Airport (SLH)",
"Solano Airport (SQF)",
"Sholapur (SSE)",
"Soldotna Airport (SXQ)",
"Soledade Airport (0)",
"Solenzara Air Base (SOZ)",
"Solita Airport (SOH)",
"Solomon Airport (SLJ)",
"Solomon State Field (SOL)",
"Solovki Airport (CSH)",
"Solwezi (SLI)",
"Jamshedpur (IXW)",
"Sondok Airport (DSO)",
"Songea Airport (SGX)",
"Sanam Bin Songkhla (SGZ)",
"Songwe Airport (MBI)",
"ChaGanHu Airport (YSQ)",
"Sopu Airport (SPH)",
"Sorkjosen Airport (SOJ)",
"Soroako Airport (SQR)",
"Sorocaba Airport (SOD)",
"Soroti Airport (SRT)",
"Souanke Airport (SOE)",
"Soure Airport (SFK)",
"Goodwin Field (ELD)",
"South Big Horn County Airport (GEY)",
"South Cariboo Regional Airport (ZMH)",
"South Galway Airport (ZGL)",
"South Goulburn Island Airport (GBL)",
"South Indian Lake Airport (XSI)",
"South Jersey Regional Airport (LLY)",
"South Molle Island Heliport (SOI)",
"South Naknek Airport (WSN)",
"Southdowns Airport (KIW)",
"Lamar Municipal Airport (LAA)",
"Burlington Municipal Airport (BRL)",
"Southern Cross Airport (SQC)",
"Southport Airport (SHQ)",
"South West Bay Airport (SWJ)",
"Marshall, Southwest Minnesota Regional Airport - Marshall/Ryan Field (MML)",
"Southwest Washington Regional Airport (KLS)",
"Rock Springs-Sweetwater County Airport (RKS)",
"May-gatka Airport (GVN)",
"Sovetskiy Airport (OVS)",
"Soyo Airport (SZA)",
"Sparrevohn LRRS Airport (SVW)",
"Sparta Community Airport-Hunter Field (SAR)",
"Sparta-Fort McCoy Airport (CMY)",
"Spartanburg Downtown Memorial Airport (SPA)",
"Sparti Airport (SPJ)",
"Spence Airport (MUL)",
"Spencer Municipal Airport (SPW)",
"Spichenkovo Airport (NOZ)",
"Spirit Lake Municipal Airport (RTL)",
"Spriggs Payne Airport (MLW)",
"Spring Creek Airport (SCG)",
"Spring Point Airport (AXP)",
"Springbok Airport (SBU)",
"Springvale Airport (KSV)",
"Springvale Airport (ZVG)",
"Squamish Airport (YSE)",
"Srednekolymsk Airport (SEK)",
"Sri Sathya Sai Airport (PUT)",
"Enniskillen Airport (ENK)",
"Saint Anthony Airport (YAY)",
"Dieppe - Saint-Aubin (DPE)",
"Saint-Augustin Airport (YIF)",
"St Clair County Airport (PLR)",
"Saint-Francois (SFC)",
"St George Airport (SGO)",
"Saint George Municipal Airport (SGU)",
"St. George Airport (STG)",
"St Helens Airport (HLS)",
"Saint-Jean Airport (YJN)",
"Saint Johns Industrial Air Park (SJN)",
"St Leonard Airport (YSL)",
"St. Lewis (Fox Harbour) Airport (YFX)",
"Saint Louis Regional Airport (ALN)",
"Saint Lucie County International Airport (FPR)",
"St. Mary's Airport (KSM)",
"St Marys Municipal Airport (STQ)",
"St. Michael Airport (SMK)",
"St Moritz-Bad Heliport (ZKH)",
"St. Paul Downtown Airport (STP)",
"Saint Paul Island Airport (SNP)",
"St. Paul Airport (ZSP)",
"Sankt Peter-Ording Airport (PSH)",
"Saint-Pierre Airport (FSP)",
"Malcolm McKinnon Airport (SSI)",
"St. Theresa Point Airport (YST)",
"St. Thomas Municipal Airport (YQS)",
"Stafsberg Airport (HMR)",
"Stan Stamper Municipal Airport (HUJ)",
"Staniel Cay Airport (TYM)",
"Stanthorpe Airport (SNH)",
"Stanton Airfield (SYN)",
"Kolyu Ganchevo Airport (SZR)",
"Staroselye Airport (RYB)",
"Statesboro-Bulloch County Airport (TBR)",
"Statesville Regional Airport (SVH)",
"Stauning Vestjylland Airport (STA)",
"Stawell Airport (SWC)",
"Steamboat Bay Seaplane Base (WSB)",
"Steamboat Springs-Bob Adams Field Stolport (SBS)",
"Stebbins Airport (WBB)",
"Stenkol Airport (NTI)",
"Stephens County Airport (BKD)",
"Stephens Island Airport (STF)",
"Stephenville Clark Regional Airport (SEP)",
"Stephenville Airport (YJT)",
"Sterling Municipal Airport (STK)",
"Stevens Field (PGO)",
"Stevens Point Municipal Airport (STE)",
"Stevens Village Airport (SVS)",
"Stewart Airport (ZST)",
"Stillwater Regional Airport (SWO)",
"Stockholm Airport (SMP)",
"Stoelmans Eiland Airstrip (SMZ)",
"Stokmarknes Skagen Airport (SKN)",
"Stony Rapids Airport (YSF)",
"Stony River 2 Airport (SRV)",
"Stord Airport (SRP)",
"Storm Lake Municipal Airport (SLB)",
"Storuman Airport (SQO)",
"Strahan Airport (SRN)",
"Strathmore Airport (STH)",
"Straubing Wallmuehle Airport (RBM)",
"Streaky Bay Airport (KBY)",
"Strezhevoy Airport (SWT)",
"Stronsay Airport (SOY)",
"Strother Field Airport (WLD)",
"Stroud Municipal Airport (SUD)",
"Stroudsburg-Pocono Airport (ESP)",
"Stuart Island Airstrip (YRR)",
"Stuart Island Airpark (SSW)",
"Stung Treng Airport (TNX)",
"Sturt Creek Airport (SSK)",
"Stuttgart Municipal Airport (SGT)",
"Stykkisholmur Airport (SYK)",
"Sua Pan Airport (SXN)",
"Suabi Airport (SBE)",
"Suai Airport (UAI)",
"Suavanao Airport (VAO)",
"San Felipe Nestor Arias Airport (SNF)",
"Subic Bay International Airport (SFS)",
"Sucua Airport (SUQ)",
"CFB Suffield (YSD)",
"Sugimanuru Airport (RAQ)",
"Zarafshan Airport (AFS)",
"Sui Airport (SUL)",
"Sukhothai Airport (THS)",
"Sukhumi Dranda Airport (SUI)",
"Suki Airport (SKC)",
"Sukkur Airport (SKZ)",
"Sulaco Airport (SCD)",
"Sulayel Airport (SLF)",
"Sule Airport (ULE)",
"Sullivan Bay Seaplane Base (YTG)",
"Sullivan County International Airport (MSV)",
"Sullivan County Airport (SIV)",
"Sultan Abdul Aziz Shah Airport (SZB)",
"Sultan Bantilan Airport (TLI)",
"Sultan Thaha Airport (DJB)",
"Sultan Muhammad Kaharuddin III Airport (SWQ)",
"Sumbawanga Airport (SUT)",
"Sumbe Airport (NDD)",
"Summer Beaver Airport (SUR)",
"Summerside Airport (YSU)",
"Summit Airport (UMM)",
"Sumter Airport (SUM)",
"Sunchales Aeroclub Airport (NCJ)",
"Sungai Pakning Bengkalis Airport (SEQ)",
"Sungai Tekai Airport (GTK)",
"Sungai Tiang Airport (SXT)",
"Suntar Airport (SUY)",
"Sunyani Airport (NYI)",
"Surat Airport (STV)",
"Surat Thani Airport (URT)",
"Surigao Airport (SUG)",
"Surin (PXR)",
"Surkhet Airport (SKH)",
"Susanville Municipal Airport (SVE)",
"Sussex County Airport (GED)",
"Suwalki Airfield (ZWK)",
"Suwon Air Base (SWU)",
"Guangfu Airport (SZV)",
"Sveg Airport (EVG)",
"Svetlaya Airport (ETL)",
"Swakopmund (SWP)",
"Swami Vivekananda Airport (RPR)",
"Swan Hill Airport (SWH)",
"Swan River Airport (ZJN)",
"Swift Current Airport (YYN)",
"Syangboche Airport (SYH)",
"Bankstown Airport Aws (BWU)",
"Sylvester Airport (SYV)",
"Sywell Aerodrome (ORM)",
"Tabal Airstrip (TBV)",
"Tabas Airport (TCX)",
"La Tabatiere Airport (ZLT)",
"Tabing Airport (PDG)",
"Tabiteuea North Airfield (TBF)",
"Tabiteuea South Airport (TSU)",
"Tableland Homestead Airport (TBL)",
"Tabora Airport (TBO)",
"Tabou (TXU)",
"Tabouaeran Airstrip (TNV)",
"Tabubil Airport (TBG)",
"Tabuk Regional Airport (TUU)",
"Tacheng Airport (TCG)",
"Tachileik Airport (THL)",
"Tacoma Narrows Airport (TIW)",
"Tacuarembo (TAW)",
"Tadji Airport (TAJ)",
"Tadjoura Airport (TDJ)",
"Tadoule Lake Airport (XTL)",
"Taean Airport (QDY)",
"Oran/Tafaraoui (TAF)",
"Taftan Airport (TFT)",
"Taguatinga Airport (QHN)",
"Tagula Airport (TGL)",
"Taharoa Aerodrome (THH)",
"Tahoua Airport (THZ)",
"Tahsis Seaplane Base (ZTS)",
"Taiping (Tekah) Airport (TPG)",
"Taisha Airport (TSC)",
"Tak (TKT)",
"Takaka Airport (KTF)",
"Takapoto Airport (TKP)",
"Takaroa Airport (TKX)",
"Sanambin Takhli (TKH)",
"Takoradi Airport (TKD)",
"Takotna Airport (TCT)",
"Taku Lodge Seaplane Base (TKL)",
"Takume Airport (TJN)",
"Talakan Airport (TLK)",
"Talasea Airport (TLW)",
"Talhar Airport (BDN)",
"Taliabu Island Airport (TAX)",
"Talkeetna Airport (TKA)",
"Talladega Municipal Airport (ASN)",
"Tallinn Linnahall Heliport (CHE)",
"Taloqan Airport (TQN)",
"Taloyoak Airport (YYH)",
"Taltheilei Narrows Airport (GSL)",
"Tamana Airfield (TMN)",
"Tamarindo Airport (TNO)",
"Tambacounda (TUD)",
"Tambao Airport (TMQ)",
"Tambillos (COW)",
"Tambonorano Airport (WTA)",
"Tambolaka Airport (TMC)",
"Tambor Airport (TMU)",
"Tamchakett Airport (THT)",
"Tampa Executive Airport (VDF)",
"Tampa North Aero Park (KYO)",
"Tampa Padang Airport (MJU)",
"Tamuin Airport (TSL)",
"Tan Tan Airport (TTA)",
"Tanacross Airport (TSG)",
"Tanah Grogot Airport (TNB)",
"Tanahmerah Airport (TMH)",
"Tanbar Airport (TXR)",
"Tancredo Thomaz Faria Airport (GPB)",
"Tanda Tula Airport (TDT)",
"Tandag Airport (TDG)",
"Tanga Airport (TGT)",
"Tangalooma Airport (TAN)",
"Tangara (TGQ)",
"Tanjung Harapan Airport (TJS)",
"Tanjung Manis Airport (TGC)",
"Tanjung Santan Airport (TSX)",
"Tanna Airport (TAH)",
"Tanque Novo Airport (0)",
"Taos Regional Airport (TSM)",
"Tappita Airfield (TPT)",
"Tapini Airport (TPI)",
"Taplejung Airport (TPJ)",
"Tapuruquara Airport (IRZ)",
"Tara Airport (XTR)",
"Tarabo Airport (TBQ)",
"Tarabits Airport (TRJ)",
"Tarama Airport (TRA)",
"Taranto-Grottaglie Airport (TAR)",
"Tarapaca Airport (TCD)",
"Tarapoa Airport (TPC)",
"Tarauaca Airport (TRQ)",
"Taraz Airport (DMB)",
"Tarbela Dam Airport (TLB)",
"Tarcoola Airport (TAQ)",
"Taree Airport (TRO)",
"Tarempa Airport (MWK)",
"Tari Airport (TIZ)",
"Tarin Kowt Airport (TII)",
"Tarko-Sale Airport (TQL)",
"Taroom Airport (XTO)",
"Tasiilaq Heliport (AGM)",
"Tasiujaq Airport (YTQ)",
"Tasiusaq Heliport (XEQ)",
"Tasiusaq Heliport (TQA)",
"Taszar Air Base (TZR)",
"Tata Airport (LEK)",
"Tatakoto Airport (TKV)",
"Tatalina LRRS Airport (TLJ)",
"Tatitlek Airport (TEK)",
"Taupo Airport (TUO)",
"Tauramena (TAU)",
"Paama Airport (PBJ)",
"Tawa (TWY)",
"Taylor County Airport (MDF)",
"Taylor Airport (TWE)",
"Taylor Airport (TYZ)",
"Tazadit Airport (OUZ)",
"Tchibanga Airport (TCH)",
"Tchien Airport (THC)",
"Iguela (IGE)",
"Tehachapi Municipal Airport (TSP)",
"Tehuacan Airport (TCN)",
"Tekadu (TKB)",
"Tekamah Municipal Airport (TQE)",
"Tekin (TKW)",
"Tela Airport (TEA)",
"Telefomin Airport (TFM)",
"Telegraph Harbour Seaplane Base (YBQ)",
"Telemaco Borba Airport (TEC)",
"Telfair-Wheeler Airport (MQW)",
"Telfer Airport (TEF)",
"Telida Airport (TLF)",
"Teller Airport (TLA)",
"Telupid Airport (TEL)",
"Teminabuan Airport (TXM)",
"Temora Airport (TEM)",
"Tenakee Seaplane Base (TKE)",
"Franca Airport (FRC)",
"Tengah Air Base (TGA)",
"Tengchong Tuofeng Airport (TCZ)",
"Teniente Amin Ayub Gonzalez Airport (ENO)",
"Teniente Jorge Henrich Arauz Airport (TDD)",
"Concepcion Airport (CIO)",
"Teniente Coronel Luis A Mantilla International Airport (TUA)",
"Villamontes (VLM)",
"Chimbote (CHM)",
"Lieutenant General Gerardo Perez Pinedo Airport (AYX)",
"Teniente R. Marsh Airport (TNM)",
"Teniente Vidal Airfield (GXQ)",
"Tenkodogo Airport (TEG)",
"Tennant Creek Airport (TCA)",
"Teptep (TEP)",
"Tepoe Airstrip (KCB)",
"Terapo (TEO)",
"Presidencia Roque Saenz Pena Airport (PRQ)",
"Las Termas Airport (RHD)",
"Terney Airport (NEI)",
"Ternopil International Airport (TNL)",
"Terrace Bay (TCY)",
"Les Saintes - Terre-de-Haut (LSS)",
"Terre Haute, Terre Haute International Airport - Hulman Field (HUF)",
"Terrell Municipal Airport (TRL)",
"Itabuna Airport (ITN)",
"Teruel Airport (TEV)",
"Teslin Airport (YZW)",
"Teseney Airport (TES)",
"Tete-a-la-Baleine Airport (ZTB)",
"Tetabedi (TDB)",
"Tetiaroa Airport (TTI)",
"Teuku Cut Ali Airport (TPK)",
"Texada Gillies Bay Airport (YGB)",
"Tezpur Airport (TEZ)",
"Tezu Airport (TEI)",
"Thaba Nchu Tar Airport (TCU)",
"Thaba-Tseka Airport (THB)",
"Thakhek (THK)",
"Thakurgaon Airport (TKR)",
"Thames Aerodrome (TMZ)",
"Thandwe Airport (SNW)",
"Thangool Airport (THG)",
"Tanjore Air Force Base (TJV)",
"Thargomindah Airport (XTG)",
"Theda Station (TDN)",
"Theodore Airport (TDR)",
"Thicket Portage Airport (YTD)",
"Thief River Falls Regional Airport (TVF)",
"Thimarafushi Airport (TMF)",
"Thisted Airport (TED)",
"Tho Xuan Airport (THD)",
"Thohoyandou Airport (THY)",
"Thomasville Regional Airport (TVI)",
"Thompson Falls Airport (THM)",
"Thompson-Robbins Airport (HEE)",
"Thompson Airport (YTH)",
"Thorne Bay Seaplane Base (KTB)",
"Gweru-Thornhill Air Base (GWE)",
"Thorshofn Airport (THO)",
"RAFO Thumrait Airbase (TTH)",
"Thylungra Airport (TYG)",
"Tianshui Maijishan Airport (THQ)",
"Tibooburra Airport (TYB)",
"Tibu (TIB)",
"Ticantiki Airport (TJC)",
"Tichitt Airport (THI)",
"Tidjikja Airport (TIY)",
"Tifalmin Airport (TFB)",
"Tiga Airport (TGJ)",
"Tikapur Airport (TPU)",
"Tikchik Lodge Seaplane Base (KTH)",
"Tikehau Airport (TIH)",
"Tiko Airport (TKC)",
"Tiksi Airport (IKS)",
"Tilin Airport (TIO)",
"Tillamook Airport (OTK)",
"Timaru Airport (TIU)",
"Timbedra Airport (TMD)",
"Timber Creek Airport (TBK)",
"Timbunke Airport (TBE)",
"Timimoun Airport (TMX)",
"Timmins/Victor M. Power Airport (YTS)",
"Tin City LRRS Airport (TNC)",
"Tinak airfield (TIC)",
"Tinboli Airport (TCK)",
"Tindouf Airport (TIN)",
"Tingo Maria Airport (TGI)",
"Tinian International Airport (TIQ)",
"Tiniteqilaaq Heliport (TQI)",
"Tioga Municipal Airport (VEX)",
"Tiom Airport (TMY)",
"Tippi Airport (TIE)",
"Tiputini (TPN)",
"Tiree Airport (TRE)",
"Tirupati Airport (TIR)",
"Tisdale Airport (YTT)",
"Tissa Tank Seaplane Base (TTW)",
"Tjilik Riwut Airport (PKY)",
"Tlokoeng Airport (TKO)",
"Toamasina Airport (TMM)",
"Tobermorey Airport (TYP)",
"Tobias Bolanos International Airport (SYQ)",
"Tobolsk Airport (TOX)",
"Toccoa Airport (TOC)",
"Tocoa Airport (TCF)",
"Tocumwal Airport (TCW)",
"Tofino Harbour Water Aerodrome (YTP)",
"Tofino/Long Beach Airport (YAZ)",
"Togiak Airport (TOG)",
"Tok Junction Airport (TKJ)",
"Tokeen Seaplane Base (TKI)",
"Tokoroa Airfield (TKZ)",
"Toksook Bay Airport (OOK)",
"Tokua Airport (RAB)",
"Tol Airport (TLO)",
"Tolanaro Airport (FTU)",
"Toliara Airport (TLE)",
"Tom Price Airport (TPR)",
"Tomanggong Airport (TMG)",
"Colville Lake/Tommy Kochon Aerodrome (YCK)",
"Penrhyn Tongareva Airport (PYE)",
"Tonghua Sanyuanpu Airport (TNH)",
"Tongliao Airport (TGO)",
"Tongoa Airport (TGH)",
"Tongren Fenghuang Airport (TEN)",
"Tonopah Test Range Airport (XSD)",
"Tonopah Airport (TPH)",
"Tonu Airport (TON)",
"Torembi Airport (TCJ)",
"Torokina Airport (TOK)",
"Tororo Airport (TRY)",
"Madrid-Torrejon Airport (TOJ)",
"Torres Airport (TOH)",
"Torres (TSQ)",
"Torrington Municipal Airport (TOR)",
"Tortola West End Sea Plane Base (TOV)",
"Tortoli Airport (TTB)",
"Torwood Airport (TWP)",
"Tosontsengel (TNZ)",
"Totegegie Airport (GMR)",
"Totness Airstrip (TOT)",
"Tougan Airport (TUQ)",
"Touggourt (TGR)",
"Touho Airport (TOU)",
"Le Touquet - Cote d'Opale Airport (LTQ)",
"Toussus-le-Noble Airport (TNF)",
"Toyama Airport (TOY)",
"Tradewind Airport (TDW)",
"Trail Airport (YZZ)",
"Trang Airport (TST)",
"Trat Airport (TDX)",
"Treinta Y Tres (TYT)",
"Trent Lott International Airport (PGL)",
"Trenton Municipal Airport (TRX)",
"Trepell Airport (TQP)",
"Tres Arroyos Airport (OYO)",
"Captain Ernesto Esguerra Cubides Air Base (TQS)",
"Tres de Mayo Airport (PUU)",
"Três Passos Airport (0)",
"Tresco Airport (TSO)",
"Tri-County Regional Airport (LNR)",
"Tri-City Airport (PPF)",
"Tri-State Steuben County Airport (ANQ)",
"Louisburg, Franklin County Airport (LFN)",
"Trier-Foehren Airport (ZQF)",
"Trincomalee Harbour Seaplane Base (THW)",
"Trinidad Airport (TDA)",
"Tripoli International Airport (TIP)",
"Trois-Rivieres Airport (YRQ)",
"Troll Airfield (QET)",
"Trombetas Airport (TMT)",
"Trompeteros Airport (TDP)",
"Trona Airport (TRH)",
"Troy Municipal Airport (TOI)",
"Truckee Tahoe Airport (TKF)",
"Trujillo Airport (TJI)",
"Trunojoyo Airport (SUP)",
"Truscott-Mungalalu Airport (TTX)",
"Truth or Consequences Municipal Airport (TCS)",
"Tsaratanana Airfield (TTS)",
"Tsetserleg (TSZ)",
"Tsewi (TSW)",
"Tshabong Airport (TBY)",
"Tshikapa Airport (TSH)",
"Matadi (MAT)",
"Tshipise Airport (TSD)",
"Tsili Tsili (TSI)",
"Ankisatra Tsimiro Airport (TZO)",
"Tsiroanomandidy Airport (WTS)",
"Djelfa (QDJ)",
"TSTC Waco Airport (CNW)",
"Tsumeb Airport (TSB)",
"Tte. Julio Gallardo Airport (PNT)",
"Tuba City Airport (TBC)",
"Tubuai - Mataura Airport (TUB)",
"Tucumcari Municipal Airport (TCC)",
"Tucupita Airport (TUV)",
"Tucurui Airport (TUR)",
"Tufi Airport (TFI)",
"Tugdan Airport (TBH)",
"Tuguegarao Airport (TUG)",
"Tuktoyaktuk James Gruben Airport (YUB)",
"Tulaghi Island Heliport (TLG)",
"Tulita Airport (ZFN)",
"Tullahoma Regional Airport (THA)",
"Tarigaun Airport (DNP)",
"Tuluksak Airport (TLT)",
"Tulum Naval Air Station (TUY)",
"Tum (TUJ)",
"Tumbang Samba Airport (TBM)",
"Tumbler Ridge Airport (TUX)",
"Tumeremo Airport (TMO)",
"Tumling Tar Airport (TMI)",
"Tumolbil (TLP)",
"Tumut Airport (TUM)",
"Tumxuk Tangwangcheng Airport (TWC)",
"Tunggul Wulung Airport (CXP)",
"Tungsten (Cantung) Airport (TNS)",
"Tunta Airport (KBN)",
"Tuntutuliak Airport (WTL)",
"Tununak Airport (TNK)",
"Tupai (TPX)",
"Tupile Airport (TUE)",
"La Tuque Airport (YLQ)",
"Turaif Domestic Airport (TUI)",
"Turbat International Airport (TUK)",
"Tureia Airport (ZTA)",
"Turkey Creek (TKY)",
"Turkmenabat Airport (CRZ)",
"Turlatovo Airport (RZN)",
"Turpan Jiaohe Airport (TLQ)",
"Turtle Island Seaplane Base (TTL)",
"Turukhansk Airport (THX)",
"Tuscaloosa Regional Airport (TCL)",
"Caro, Tuscola Area Airport (TZC)",
"Fort Hunter Liggett Heliport (HGT)",
"Tuticorin Airport (TCR)",
"Twentynine Palms Airport (TNP)",
"Twin Hills Airport (TWA)",
"Two Harbors Amphibious Terminal Seaplane Base (TWH)",
"Paysandu Airport (PDU)",
"Tynda Airport (TYD)",
"Tyonek Airport (TYE)",
"Teply Klyuch Airport (KDY)",
"Tzaneen Airport (LTA)",
"Ua Huka Airport (UAH)",
"Ua Pou Airport (UAP)",
"Uaxactun Airport (UAX)",
"Ubari Airport (QUB)",
"Ubatuba Airport (UBT)",
"Ubrub Airport (UBR)",
"Uchiza Airport (UCZ)",
"Udine-Campoformido Airport (UDN)",
"Udorn Air Base (BAO)",
"Ugashik Bay Airport (UGB)",
"Ugashik Airport (UGS)",
"Ugnu-Kuparuk Airport (UUK)",
"Uige Airport (UGO)",
"Ujae Airfield (UJE)",
"Ukiah Municipal Airport (UKI)",
"Ukkusissat Heliport (JUK)",
"Ukunda Airstrip (UKA)",
"Ulaanbaatar International Airport (UBN)",
"Ulaangom Airport (ULO)",
"Ulanhot Airport (HLH)",
"Ulanqab Airport (UCB)",
"Ulawa Airport (RNA)",
"Ulei Airport (ULB)",
"Ulithi Airport (ULI)",
"Uljin Airport (UJN)",
"Ulsan Airport (USN)",
"Ulukhaktok/Holman Island Airport (YHI)",
"Ulusaba Airport (ULX)",
"Ulyanovsk Vostochny Airport (ULY)",
"Umba Airport (UMC)",
"Armacao dos Buzios Airport (BZC)",
"Umbu Mehang Kunda Airport (WGP)",
"Umiat Airport (UMT)",
"Umiujaq Airport (YUD)",
"Umuarama Airport (UMU)",
"Unalakleet Airport (UNK)",
"Unalaska Airport (DUT)",
"Undara Airport (UDA)",
"Unguia (UNC)",
"Uniao da Vitoria Airport (UVI)",
"Unifly Heliport (ZFU)",
"Union Glacier Blue-Ice Runway (UGL)",
"Union Island Airport (UNI)",
"University-Oxford Airport (UOX)",
"Unst Baltasound (UNT)",
"Uonquen Airport (WOK)",
"Upala Airport (UPL)",
"RAF Upavon (UPV)",
"Upernavik Kujalleq Heliport (UPK)",
"Upiara Airport (UPR)",
"Upolu Airport (UPP)",
"Urad Middle Banner Airport (WZQ)",
"Uranium City Airport (YBE)",
"Uray Airport (URJ)",
"Urengoy (UEN)",
"Uribe (URI)",
"Uriman Airport (URM)",
"Uroubi (URU)",
"Urrao Airport (URR)",
"Uru Harbour Airport (ATD)",
"Urzhar Airport (UZR)",
"United States Air Force Academy Airstrip (AFF)",
"Useless Loop Airport (USL)",
"Usharal Airport (USJ)",
"Usiminas Airport (IPN)",
"Usina Catanduva Airport (0)",
"Usina Mandu Airport (0)",
"Usinsk Airport (USK)",
"Ust-Ilimsk Airport (UIK)",
"Ust-Kut Airport (UKX)",
"Ust-Kuyga Airport (UKG)",
"Ust-Maya Airport (UMS)",
"Ust-Nera Airport (USR)",
"Ust-Tsilma Airport (UTS)",
"Ustupo Airport (UTU)",
"Utila Airport (UII)",
"Utrik Airfield (UTK)",
"Uttaradit Airport (UTR)",
"Utti Airport (UTI)",
"Uummannaq Heliport (UMD)",
"Uvol Airport (UVO)",
"Uyuni Airport (UYU)",
"Uzhhorod International Airport (UDJ)",
"Vadodara Airport (BDQ)",
"Vadso Airport (VDS)",
"Vaeroy Heliport (VRY)",
"Firing Center Army Air Field (FCT)",
"Vahitahi Airport (VHZ)",
"Valan Airport (HVG)",
"Valcheta Airport (VCF)",
"Valdez Airport (VDZ)",
"Valenca Airport (VAL)",
"Valenciennes-Denain Airport (XVS)",
"Valesdir Airport (VLS)",
"Valle del Conlara Airport (RLO)",
"Valle de la Pascua (VDP)",
"Valle Airport (VLE)",
"Vallenar (VLR)",
"Van Don International Airport (VDO)",
"Vandalia Municipal Airport (VLA)",
"Vandenberg Air Force Base (VBG)",
"Vangaindrano Airport (VND)",
"Vannes-Meucon Airport (VNE)",
"Vanrook Station Airport (VNR)",
"Vanua Balavu Airport (VBV)",
"Varandey Airport (VRI)",
"Vares-Venegono Airport (QVA)",
"Varkaus Airport (VRK)",
"Vaestervik flygplats (VVK)",
"Vatomandry Airport (VAT)",
"Vatukoula Airport (VAU)",
"Vatulele Airport (VTF)",
"Vaexjoe Smaland Airport (VXO)",
"Vejrø (-)",
"Velasquez Airport (PYA)",
"Velikiy Ustyug Airport (VUS)",
"Velikie Luki (VLU)",
"Venango Regional Airport (FKL)",
"Venetie Airport (VEE)",
"Ventspils International Airport (VNT)",
"Veranópolis Airport (0)",
"Verdun-Sommedieue (XVN)",
"Verkhnevilyuysk Airport (VHV)",
"Vermilion Regional Airport (DNV)",
"Vermilion Airport (YVG)",
"Vernal Regional Airport (VEL)",
"Vernon Regional Airport (YVE)",
"Vesoul-Frotey (XVO)",
"Vestmannaeyjar Airport (VEY)",
"Vichadero Airport (VCH)",
"Vichy - Charmeil Airport (VHY)",
"Vicksburg Municipal Airport (VKS)",
"Victor Lafon Airport (SSD)",
"Victoria Inner Harbour Seaplane Base (YWH)",
"Victoria Reservoir Sea Plane Base (KDW)",
"Victoria River Downs Airport (VCD)",
"Victoria Regional Airport (VCT)",
"Victoria Airport (ZIC)",
"Vidalia Regional Airport (VDI)",
"Videira Airport (VIA)",
"Vidin Smurdan Airfield (VID)",
"Vieste Heliport (VIF)",
"Vidyanagar Airport (VDY)",
"Vijayawada Airport (VGA)",
"Mato Grosso (MTG)",
"Vila Real Airport (VRL)",
"Vila Rica Airport (VLP)",
"Vilanculos Airport (VNX)",
"Vilhelmina Airport (VHM)",
"Villa Dolores Aerodrome (VDR)",
"Villa Garzon Airport (VGZ)",
"Villa Gesell Airport (VLG)",
"Villa International Airport (VAM)",
"Velizy - Villacoublay Air Base (VIY)",
"Ndjole Ville Airport (KDJ)",
"Kamina Airport (KMN)",
"Vilyuysk Airport (VYI)",
"Vina del mar Airport (KNA)",
"Vincent Fayks Airport (OEM)",
"Vinh Long Airfield (XVL)",
"Vinh Airport (VII)",
"Vipingo Airport (VPG)",
"Viqueque Airport (VIQ)",
"Veer Savarkar International Airport (IXZ)",
"Virac Airport (VRC)",
"Virgil I Grissom Municipal Airport (BFR)",
"Virgin Gorda Airport (VIJ)",
"Virginia Airport (VIR)",
"Viru Harbour Airstrip (VIU)",
"Vitebsk Vostochny Airport (VTB)",
"Viveros Island Airport (IVI)",
"Vivigani Airport (VIV)",
"Vohemar Airport (VOH)",
"Voinjama Airport (VOI)",
"Volk Field (VOK)",
"Volta Grande Airport (0)",
"Vopnafjoerdur Airport (VPN)",
"Votuporanga Airport (VOT)",
"Vredendal (VRE)",
"Vryburg Airport (VRU)",
"Vryheid Airport (VYD)",
"Vung Tau Airport (VTG)",
"Vunisea Airport (KDV)",
"W. H. 'Bud' Barron Airport (DBN)",
"Wa Airport (WZA)",
"Wabo Airport (WAO)",
"Wabush Airport (YWK)",
"Waca Airport (WAC)",
"Waku Kungo Airport (CEO)",
"Wad Madani Airport (DNI)",
"Wadi al-Dawasir Domestic Airport (WAE)",
"Wadi Halfa Airport (WHF)",
"Wageningen Airport Airport (AGI)",
"Wagethe Airport (WET)",
"Wahai Maluku (WBA)",
"Waiheke Reeve Airport (WIK)",
"Waikoloa Heliport (WKL)",
"Waimea-Kohala Airport (MUE)",
"Wainwright Airport (AIN)",
"Wainwright Airport (AIN)",
"Wairoa Aerodrome (WIR)",
"Wajir Airport (WJR)",
"Wakaya Island Airport (KAY)",
"Wake Island Airfield (AWK)",
"Wakunai Inus Airport (WKN)",
"Walaha Airport (WLH)",
"Walcha Airport (WLC)",
"Waldronaire Airport (WDN)",
"Wales Airport (WAA)",
"Walfrido Salmito de Almeida Airport (JSB)",
"Walgett Airport (WGE)",
"Wallal Airport (WLA)",
"Wallops Flight Facility (WAL)",
"Walnut Ridge Regional Airport (ARG)",
"Horizontina Airport (HRZ)",
"Wamena Airport (WMX)",
"Wana Airfield (WAF)",
"Wanaka Airport (WKA)",
"Wanganui Airport (WAG)",
"Wangaratta Airport (WGT)",
"Wanigela Airport (AGL)",
"Wantoat Airport (WTT)",
"Wanuma Airport (WNU)",
"Wanzhou Wuqiao Airport (WXN)",
"Wapekeka Airport (YAX)",
"Wapenamanda Airport (WBM)",
"Wapolu (WBC)",
"Warangal Airport (WGC)",
"Warder Airport (WRA)",
"Ware Airport (UWA)",
"Waris Airport (WAR)",
"Warraber Island Airport (SYU)",
"Warracknabeal Airport (WKB)",
"Warrawagine Airport (WRW)",
"Palmer Municipal Airport (PAQ)",
"Warren County Memorial Airport (RNC)",
"Warren Field Airport (OCW)",
"Warren Airport (QRR)",
"Warri Airport (QRW)",
"Warrnambool Airport (WMB)",
"Warroad International-Swede Carlson Field (RRT)",
"Warton Airport (WRT)",
"Warukin Airport (TJG)",
"Warwick Airport (WAZ)",
"Washabo Airport (WSO)",
"Washington Island Airstrip (TNQ)",
"Washington County Airport (WSG)",
"Wasilla Airport (WWA)",
"Wasior Airport (WSR)",
"Waskaganish Airport (YKQ)",
"Waspam Airport (WSP)",
"Wasu (WSU)",
"Wasua Airport (WSA)",
"Wasum Airport (WUM)",
"Waterbury-Oxford Airport (OXC)",
"Waterfall Seaplane Base (KWF)",
"Waterloo Municipal Airport (ALO)",
"Waterloo Airport (WLO)",
"Watertown International Airport (ART)",
"Watertown Municipal Airport (ATY)",
"Waterville Robert Lafleur Airport (WVL)",
"Watson Lake Airport (YQH)",
"Watsonville Municipal Airport (WVI)",
"Wau Airport (WUG)",
"Wau Airport (WUU)",
"Wauchope Landing Ground (WAU)",
"Waukon Municipal Airport (UKN)",
"Wausau Municipal Airport (AUW)",
"Wave Hill Airport (WAV)",
"Waverney Airport (WAN)",
"Wawa Airport (YXZ)",
"Wawoi Falls (WAJ)",
"Waycross-Ware County Airport (AYS)",
"Waynesville-St. Robert Regional Airport (TBN)",
"Weam Airport (WEP)",
"Webequie Airport (YWP)",
"Webster City Municipal Airport (EBS)",
"Wedau (WED)",
"Wee Waa (WEW)",
"Weedon Field (EUF)",
"Weerawila International Airport (WRZ)",
"Weifang Airport (WEF)",
"Weipa Airport (WEI)",
"Wekweeti Airport (YFJ)",
"Welkom Airport (WEL)",
"Wells Municipal Airport/Harriet Field (LWL)",
"Wellsville Municipal Airport-Tarantine Field (ELZ)",
"Welshpool Aerodrome (WHL)",
"Namibe Airport (MSZ)",
"Wemindji Airport (YNC)",
"Wendover Airport (ENV)",
"Wenshan Puzhehei Airport (WNH)",
"Werur Airport (WRR)",
"West Angelas Airport (WLP)",
"West Bend Municipal Airport (ETB)",
"West Branch Community Airport (WBK)",
"West End Airport (WTD)",
"West Memphis Municipal Airport (AWM)",
"West Point Village Seaplane Base (KWP)",
"West Sale Airport (SXE)",
"West 30th Street Heliport (JRA)",
"West Woodward Airport (WWR)",
"West Wyalong Airport (WWY)",
"Westerly State Airport (WST)",
"Western Nebraska Regional Airport (BFF)",
"Westport Airport (WSZ)",
"Westray Airport (WRY)",
"Westsound Seaplane Base (WSX)",
"MDP Wethersfield (WXF)",
"Wewak Airport (WWK)",
"Wexford County Airport (CAD)",
"Whakatane Airport (WHK)",
"Whale Cove Airport (YXN)",
"North Whale Seaplane Base (WWP)",
"Whalsay Airport (WHS)",
"Wharton Regional Airport (WHT)",
"Whati Airport (YLE)",
"Wheeler Army Airfield (HHI)",
"Naval Air Station Whidbey Island (NUW)",
"Whistler Green Lake Water Aerodrome (YWS)",
"White Mountain Airport (WMO)",
"White River Seaplane Base (YWR)",
"Whitecourt Airport (YZU)",
"Erik Nielsen Whitehorse International Airport (YXY)",
"Whitehouse Naval Outlying Field (NEN)",
"Whiteriver Airport (WTR)",
"Whiteside County Airport (SQI)",
"Whitianga Airport (WTZ)",
"Milton, Whiting Field Naval Air Station North (NSE)",
"Whyalla Airport (WYA)",
"Wick Airport (WIC)",
"Wiesbaden (WIE)",
"Wilbarger County Airport (WIB)",
"Wilcannia Airport (WIO)",
"Wiley Post Airport (PWA)",
"Wiley Post-Will Rogers Memorial Airport (BRW)",
"Wilhelmshaven-Mariensiel Airfield (WVN)",
"Wilkes-Barre Wyoming Valley Airport (WBW)",
"Wilkes County Airport (IKB)",
"Will Rogers World Airport (OKC)",
"William R Fairchild International Airport (CLM)",
"William T. Piper Memorial Airport (LHV)",
"Williams Harbour Airport (YWM)",
"Williams Lake Airport (YWL)",
"Williamson County Regional Airport (MWA)",
"Williston Basin International Airport (XWA)",
"Willmar Municipal Airport (ILL)",
"Willow Airport (WOW)",
"Willows-Glenn County Airport (WLW)",
"Wilpena Pound Airport (HWK)",
"Wiluna Airport (WUN)",
"Jijiga Wilwal International Airport (JIJ)",
"Winchester Regional Airport (WGO)",
"Windarling Airport (WRN)",
"Windarra Airport (WND)",
"Windom Municipal Airport (MWM)",
"Windorah Airport (WNR)",
"Wings Field (BBX)",
"Winkler County Airport (INK)",
"Winnemucca Municipal Airport (WMC)",
"Winslow Municipal Airport (INW)",
"Winston Field (SNK)",
"Winter Haven's Gilbert Airport (GIF)",
"Winton Airport (WIN)",
"Wipim Airport (WPM)",
"Wiscasset Airport (ISS)",
"Wiseman Airport (WSM)",
"Witham Field (SUA)",
"Wittenoom Airport (WIT)",
"Witu Airport (WIU)",
"Wodgina Airport (GYB)",
"Woensdrecht Air Base (WOE)",
"Woitape Airport (WTP)",
"Woja Airport (WJA)",
"Glasgow International Airport (GGW)",
"Wollaston Lake Airport (ZWL)",
"Wollogorang (WLL)",
"Wondai Airport (WDI)",
"Wondoola Airport (WON)",
"Wonenara Airport (WOA)",
"Wonju Airport (WJU)",
"Wonken Airport (WOK)",
"Kalma Airport (WOS)",
"Woodbourne Airport (BHE)",
"Woodie Woodie Airport (WWI)",
"Woodward Field (CDN)",
"RAAF Woomera Airfield (UMR)",
"Worland Municipal Airport (WRL)",
"Worthington Municipal Airport (OTG)",
"Wotho Airfield (WTO)",
"Wotje Airfield (WTE)",
"Wrangell Airport (WRG)",
"Ft. Stewart (LIY)",
"Wrigley Airport (YWY)",
"Wrotham Park Airport (WPK)",
"Wudalianchi Dedu Airport (DTU)",
"Wudinna Airport (WUD)",
"Wuhai Airport (WUA)",
"Wuhu Air Base (WHU)",
"Wunnummin Lake Airport (WNN)",
"Wuvulu Island Airport (WUV)",
"Wuzhou Xijiang Airport (WUZ)",
"Wycombe Air Park (HYC)",
"Wyk auf Foehr (OHR)",
"Wyndham Airport (WYN)",
"Wynyard Airport (BWT)",
"Xai-Xai Airport (VJB)",
"Xangongo Airport (XGN)",
"Xanxere (AXE)",
"Xavantina Airport (NOK)",
"Xewkija Heliport (GZM)",
"Xi'an Xiguan Airport (SIA)",
"Xieng Khouang Airport (XKH)",
"Xienglom Airport (XIE)",
"Xilinhot Airport (XIL)",
"Hulunbuir Xinbarag Youqi Baogede Airport (XRQ)",
"Xingcheng (XEN)",
"Xingning Airport (XIN)",
"Xingtai Dalian Airport (XNT)",
"Xinguara Municipal Airport (XIG)",
"Xingyi Airport (ACX)",
"Xining Caojiabao Airport (XNN)",
"Xinyang Minggang Airport (XAI)",
"Nalati Airport (NLT)",
"Xinzhou Wutaishan Airport (WUT)",
"Yacuiba Airport (BYC)",
"Yagoua Airport (GXX)",
"Yaguara Airport (AYG)",
"Yakataga Airport (CYT)",
"Yakubu Gowon Airport (JOS)",
"Yakushima Airport (KUM)",
"Yakutat Airport (YAK)",
"Yalata Mission Airport (KYI)",
"Yalgoo Airport (YLG)",
"Yalinga (AIG)",
"Yalumet Airport (KYX)",
"Yam Island Airport (XMY)",
"Yamoussoukro Airport (ASK)",
"Yampa Valley Airport (HDN)",
"Yan'an Nanniwan Airport (ENY)",
"Yanbu Airport (YNB)",
"Yandina Airport (XYA)",
"Yangambi Airport (YAN)",
"Yaounde Airport (YAO)",
"Yapsiei (KPE)",
"Yari Airport (AYI)",
"Yarmouth Airport (YQI)",
"Yarra Bank Heliport (KAH)",
"Yas Island Seaplane Base (AYM)",
"Yasawa Island Airport (YAS)",
"Yasuj Airport (YES)",
"Yasuru (KSX)",
"Yaviza Airport (PYV)",
"Ye Airport (XYE)",
"Yechon Ab (YEC)",
"Yedinka Airport (EDN)",
"Yeelirrie Airport (KYF)",
"Shache Yeerqiang Airport (QSZ)",
"Yegepa (PGE)",
"Yelimane Airport (EYL)",
"Yengema Airport (WYE)",
"Yeniseysk Airport (EIE)",
"Yenkis (YEQ)",
"Yerbogachen Airport (ERG)",
"Yerington Municipal Airport (EYR)",
"Yes Bay Lodge Seaplane Base (WYB)",
"Yeva (YVD)",
"Yevlakh Airport (YLV)",
"Yeysk Airport (EIK)",
"Yibin Caiba Airport (YBP)",
"Yibin Wuliangye Airport (YBP)",
"Yichun Mingyueshan Airport (YIC)",
"Yinchuan Helanshan Airport (YEH)",
"Yingkou Lanqi Airport (YKH)",
"Yining Airport (YIN)",
"Yiwu Airport (YIW)",
"Ylivieska Airport (YLI)",
"Yogyakarta International Airport (YIA)",
"Nkayi-Yokangassi Airport (NKY)",
"Yokota Air Base (OKO)",
"Yola Airport (YOL)",
"Yongai Airport (KGH)",
"Yongphulla Airport (YON)",
"York Landing Airport (ZAC)",
"York Airport (THV)",
"Yorke Island Airport (OKR)",
"Yorketown Airport (ORR)",
"Yorkton Municipal Airport (YQV)",
"Yoro (ORO)",
"Yoron Airport (RNJ)",
"Yotvata Airfield (YOT)",
"Young Airport (NGA)",
"Yuanmou Airport (YUA)",
"Yuba County Airport (MYV)",
"Yucca Airstrip (UCC)",
"Yuendumu Airport (YUE)",
"Yueyang Sanhe Airport (YYA)",
"Yuruf Airport (RUF)",
"Yushu Batang Airport (YUS)",
"Yuzhno-Kurilsk Mendeleyevo Airport (DEE)",
"Yverdon Airfield (ZLJ)",
"Z M Jack Stell Field (CRT)",
"Zabol Airport (ACZ)",
"Zabrat Airport (ZXT)",
"Zabre Airport (XZA)",
"Zabreh Ostrava Airport (ZBE)",
"Zachar Bay Seaplane Base (KZB)",
"Zagora Airport (OZG)",
"Zakouma Airport (AKM)",
"Zalengei Airport (ZLX)",
"Zambezi Airport (BBZ)",
"Zamora Regional Airport (ZMM)",
"Zamzama Heliport (ZIZ)",
"Zanaga Airport (ANJ)",
"Zanjan Airport (JWN)",
"Zapala Airport (APZ)",
"Zapatoca Airport (AZT)",
"Zaqatala International Airport (ZTU)",
"Zaranj Airport (ZAJ)",
"Zaraza Airport (ZRZ)",
"Zaria Airport (ZAR)",
"Zaysan Airport (SZI)",
"Zemio Airport (IMO)",
"Zenag Airport (ZEN)",
"Zhangjiakou Ningyuan Airport (ZQZ)",
"Zhangye Ganzhou Airport (YZY)",
"Zhanjiang Airport (ZHA)",
"Zhaotong Airport (ZAT)",
"Zhigansk Airport (ZIX)",
"Zhijiang Airport (HJJ)",
"Zhob Airport (PZH)",
"Zhongwei Airport (ZHY)",
"Zhoushan Airport (HSN)",
"Zigong Fengming General Airport (ZKL)",
"Zilfi Airport (ZUL)",
"Zero Airport (ZER)",
"Oostmalle Air Base (OBL)",
"Zonalnoye Airport (ZZO)",
"Zorg en Hoop Airport (ORG)",
"Zulu Inyala Airport (PZL)",
"Zunyi Maotai Airport (WMT)",
"Zunyi Xinzhou Airport (ZYI)",
"Zuwarah Airport (WAX)",
"Zweibruecken Airport (ZQW)",
"Zyryanka Airport (ZKP)",
"Langtang Heliport (LTG)",
"Aachen-Merzbrueck Airport (AAH)",
"Aappilattoq Kujalleq Heliport (QUV)",
"Aappilattoq Upernavik Heliport (AOQ)",
"Walker's Cay Airport (WKR)",
"Abadan Airport (ABD)",
"Abaiang Atoll Airport (ABF)",
"Abbaye Airport (BGH)",
"Abbeville Airport (XAB)",
"Abbottabad Airport (AAW)",
"Abbs Airport (EAB)",
"Abdul Rachman Saleh Airport (MLG)",
"Abeche (AEH)",
"Abemama Airport (AEA)",
"Abengourou Airport (OGO)",
"Aberdeen Regional Airport (ABR)",
"Abha Regional Airport (AHB)",
"Abilene Regional Airport (ABI)",
"Abingdon Airport (ABG)",
"Aboisso Airport (ABO)",
"Abou-Deia Airport (AOD)",
"Abresso Airport (RSK)",
"Abu Musa Airport (AEU)",
"Abu Rudeis Airfield (AUE)",
"Abu Simbel Airport (ABS)",
"Acadiana Regional Airport (ARA)",
"Acaricuara Airport (ARF)",
"ACFC (WAP)",
"Achutupo Airport (ACU)",
"Dhala Airport (DHL)",
"Ada Municipal Airport (ADT)",
"Adado Airport (AAD)",
"Adam Airport (AOM)",
"Adampur Airport (AIP)",
"Adareil Airport (AEE)",
"Addington Field (EKX)",
"Sorriso Airport (SMT)",
"Aek Godang Airport (AEG)",
"Anapolis AFB (APS)",
"Santos Aeroporto (SSZ)",
"Aeroclube Airport (0)",
"Bento Goncalves Airport (BGV)",
"Aeroclube Airport (QNV)",
"Chaves Airport (CHV)",
"Coimbra Airport (CBP)",
"Viseu Airport (VSE)",
"University of Oklahoma Westheimer Airport (OUN)",
"Weide Army Heliport (EDG)",
"Camp Edwards Heliport (FMH)",
"Mankato Regional Airport (MKT)",
"Holmansville Heliport (NEL)",
"Ridgecrest Community Hospital Heliport (NID)",
"Urgun Heliport (URN)",
"Bhurban Heliport (BHC)",
"Paloich Airport, Heliport (HGI)",
"Sphinx International Airport (SPX)",
"Roche Harbor Seaplane Base (RCE)",
"Seal Bay Seaplane Base (SYB)",
"Doris Lake Seaplane Base (JOJ)",
"Southern Seaplane Airport (BCS)",
"Wuhu Xuanzhou (WHA)"

];



const originInput = document.querySelector('#origin');
const destinationInput = document.querySelector('#destination');
const originSuggestions = document.querySelector('.originsuggestions ul');
const destinationSuggestions = document.querySelector('.destsuggestions ul');

const maxVisibleSuggestions = 10; // Number of visible suggestions

function search(str, dataList) {
    let results = [];
    const val = str.toLowerCase();

    for (i = 0; i < dataList.length; i++) {
        if (dataList[i].toLowerCase().indexOf(val) > -1) {
            results.push(dataList[i]);
        }
    }
    return results;
}

function searchHandler(e, dataList, suggestionsContainer, input) {
    const inputVal = input.value; // Use input's value, not e.currentTarget.value
    let results = [];
    if (inputVal.length > 0) {
        results = search(inputVal, dataList);
    }
    showSuggestions(results, inputVal, suggestionsContainer);
}

function showSuggestions(results, inputVal, suggestionsContainer) {
    suggestionsContainer.innerHTML = '';

    const startIndex = 0; // Start index of suggestions
    const endIndex = Math.min(startIndex + maxVisibleSuggestions, results.length);

    if (endIndex > startIndex) {
        for (let i = startIndex; i < endIndex; i++) {
            let item = results[i];
            const match = item.match(new RegExp(inputVal, 'i'));
            item = item.replace(match[0], `<strong>${match[0]}</strong>`);
            suggestionsContainer.innerHTML += `<li>${item}</li>`;
        }
        suggestionsContainer.classList.add('has-suggestions');
    } else {
        suggestionsContainer.classList.remove('has-suggestions');
    }
}

function useSuggestion(e, input, suggestionsContainer) {
    input.value = e.target.innerText;
    input.focus();
    suggestionsContainer.innerHTML = '';
    suggestionsContainer.classList.remove('has-suggestions');
}

originInput.addEventListener('input', () => {
    searchHandler(null, airports, originSuggestions, originInput);
});

destinationInput.addEventListener('input', () => {
    searchHandler(null, airports, destinationSuggestions, destinationInput);
});

originSuggestions.addEventListener('click', e => {
    useSuggestion(e, originInput, originSuggestions);
});

destinationSuggestions.addEventListener('click', e => {
    useSuggestion(e, destinationInput, destinationSuggestions);
});



const searchTypeDropdown = document.querySelector('#searchTypeDropdown');
const searchTypeOptions = document.querySelectorAll('.search-option');

// Function to toggle the search type dropdown
function toggleSearchTypeDropdown() {
    searchTypeDropdown.classList.toggle('active');
}

// Function to set the selected search type
function setSelectedSearchType(searchType) {
    searchTypeOptions.forEach(option => {
        if (option.dataset.value === searchType) {
            option.classList.add('selected');
            searchTypeSelect.value = searchType;
        } else {
            option.classList.remove('selected');
        }
    });
}

// Function to handle search type option selection
searchTypeOptions.forEach(option => {
    option.addEventListener('click', () => {
        const selectedSearchType = option.dataset.value;
        setSelectedSearchType(selectedSearchType);
        toggleSearchTypeDropdown();
    });
});

// Clicking outside the dropdown closes it
window.addEventListener('click', (e) => {
    if (!searchTypeDropdown.contains(e.target)) {
        searchTypeDropdown.classList.remove('active');
    }
});

// Function to handle the "Check Status" button click
checkStatusButton.addEventListener('click', async () => {
    const selectedSearchType = searchTypeSelect.value;

    if (selectedSearchType === 'flightNumber') {
        // Redirect to the "searching_by_flight_number.html" page
        window.location.href = 'searching_by_flight_number.html';
    } else if (selectedSearchType === 'route') {
        // Redirect to the "showing_routes.html" page
        window.location.href = 'showing_routes.html';
    }
});



function useSuggestion(e, input, suggestionsContainer) {
    const selectedOptionText = e.target.innerText;
    const iataMatches = selectedOptionText.match(/\(([A-Z]+)\)$/);

    if (iataMatches) {
        const iataCode = iataMatches[1];
        input.value = selectedOptionText;
        suggestionsContainer.innerHTML = '';
        suggestionsContainer.classList.remove('has-suggestions');
    }
}

checkStatusButton.addEventListener('click', () => {
    const originIata = originInput.value.match(/\(([A-Z]+)\)$/)?.[1];
    const destinationIata = destinationInput.value.match(/\(([A-Z]+)\)$/)?.[1];

    if (originIata && destinationIata) {
        window.location.href = `routes.html?origin=${originIata}&destination=${destinationIata}`;
    } else {
        console.error("Invalid origin or destination.");
    }
});
